...
public class JsonlAnalysisRecordStore : IAnalysisRecordStore, IAnalysisRecordQuery
{
    ...
    public DailyAnalysisRecord? GetOne(string hostName, DateTime date)
    {
        return ReadAll().FirstOrDefault(r =>
            r.Date.Date == date.Date &&
            string.Equals(r.Host, hostName, StringComparison.OrdinalIgnoreCase));
    }

    private static bool Matches(DailyAnalysisRecord record, RecordQueryFilter filter)
    {
        // HostNames 為 null = 不限；空集合 = 查不到任何資料（授權範圍為空）。
        // 這個區別是授權正確性的關鍵，不能簡化成「空的就當不限」
        if (filter.HostNames != null &&
            !filter.HostNames.Contains(record.Host, StringComparer.OrdinalIgnoreCase))
        {
            return false;
        }
