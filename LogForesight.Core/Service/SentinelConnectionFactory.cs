...
/// <summary>
/// Sentinel 連線資訊組裝，批次端（<see cref="NetiqProbeCli"/>／<see cref="NetiqPipelineService"/>）
/// 共用同一份——docs/NETIQ-API-PLAN.md §2.1 說的「批次與 Web 是不同部署單元、各自一份合理」
/// 講的是跨專案；這兩個都在同一個 console 專案內，沒有理由各寫一份同樣的解密邏輯。
/// </summary>
internal static class SentinelConnectionFactory
{
