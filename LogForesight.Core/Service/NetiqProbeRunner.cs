...
/// <c>--sample-linux-ip</c>／<c>--sample-ip</c>（Web 端對應的樣本 IP 欄位）指定樣本主機才能
/// 執行，未提供時明確標示略過，不是靜默跳過；其餘步驟不需要任何參數。
/// </summary>
public static class NetiqProbeRunner
{
    ...
    /// <param name="sentinels">要 probe 的 Sentinel 清單（呼叫端已篩好範圍——console 篩
    /// Active，Web 診斷分頁只放使用者選定的那一台）。</param>
        ...
        string? sampleIp, string? sampleLinuxIp, IRunConsole console, CancellationToken ct = default)
    {
        ...
        foreach (var sentinel in sentinels)
        {
            ...
            try
            {
                allOk &= await ProbeOneAsync(server, settings, sampleIp, sampleLinuxIp, console, ct);
            }
            catch (Exception ex)
            {
        ...
        IRunConsole console, CancellationToken ct)
    {
        ...
        {
            ok &= await Step(console, 1, "認證＋小範圍查詢（近 1 小時、3 筆、全欄位——用於核對欄位對應）", async () =>
            {
            ...
            ok &= await Step(console, 3, "分頁效能（pgsize 100／500／1000，近 24 小時）", async () =>
            {
            ...
            ok &= await Step(console, 4, "IP 篩選批次大小（10／50／100 個 IP 子句，用 repip——第一輪已實證的回報者 IP 欄位）", async () =>
            {
            ...
            ok &= await Step(console, 5, "失敗路徑：非法 Lucene 語法應被拒絕（錯誤訊息應指向查詢語法，不是 JSON 格式）", async () =>
            {
                try
                {
            ...
            ok &= await Step(console, 6, "ESM eventsource 清單（能否取代「投影事件 distinct」當主機目錄／OS 判別來源）", async () =>
            {
                try
                {
                    var body = await client.RawGetAsync("/SentinelRESTServices/objects/eventsource", ct);
            ...
            {
                console.WriteLine("   [9] 頻道覆蓋（System/Application）：略過（未提供樣本 IP）");
            }
            else
            {
                ...
                ok &= await Step(console, 9, $"頻道覆蓋＋樣本（repip:{sampleIp} 的 System/Application 近 24h）", async () =>
                {
            ...
            if (string.IsNullOrWhiteSpace(sampleIp))
            {
                console.WriteLine("   [11] dt 邊界精確核對：略過（未提供樣本 IP）");
            }
            else
            {
        ...
        ok &= await Step(console, 13, "失敗路徑：錯誤密碼應回認證失敗（不影響上面已用正確密碼跑完的查詢）", async () =>
        {
