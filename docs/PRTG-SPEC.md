...
1. **純語法層**（`PrtgAddress.Normalize`，無 IO）：去前後空白、去 scheme、去路徑與 query、
   ...
   `PrtgHostMapper.NormalizeIp` 是它的對外名稱，另有二十個呼叫點（IP 排除清單的儲存鍵、
   device 分組鍵等）共用同一份判定。
...
- **與取數執行互斥（兩個方向都要處理）**：
  ...
  - 同步進行中，夜間或手動取數開始 → PRTG 日路徑**先等同步結束**（進度 phase `prtg-wait-sync`），
    然後**跳過自己的結構同步**（鏡像剛更新過，重爬一次沒有新資訊），對應照做（對昨天）。
...
2. 找 `Ip` 與位址相同的 device。**比對鍵有三段優先序，一段都不能少**：
   ① 兩邊都過 §4 的位址解析（IP 直接比；名稱先 DNS 解析成 IPv4 再比）；
   ② 都對不到時退回**主機名稱字面比對**——device 的 `Ip` 也可能填 DNS 名稱，
   而內網名稱未必進得了 DNS，兩邊填同一個名稱時仍應命中。
   **少了 DNS 那段幾乎必然全數落空**：Sentinel 慣以 DNS 名稱設定、PRTG device 慣填 IPv4；
   少了字面那段，內網名稱環境會整組失效。
