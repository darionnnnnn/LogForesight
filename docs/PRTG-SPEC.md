...
本文件描述**現況**：資料表、擷取、主機對應、設定與操作介面、狀態變更型規則、磁碟可用空間趨勢、
資料準備度／語意驗證及跨來源佐證。磁碟趨勢已有程式判定與正式每日路徑，但尚未以真實 PRTG 28 天資料及現場頻道語意驗證，暫定門檻未校準；其他值型規則（基線偏移、CPU／記憶體持續高檔）仍未實作。
...
**取數範圍可放寬**（`PrtgValueFetchScope`，預設 `triggered`＝上述行為）。需累積更多值型歷史時，
管理者可改變第 1 步的候選主機來源；第 2、3 步的收斂三種模式完全相同：
...
- **結構同步三階段各自回報進度**（`prtg-sync-devices`／`-sensors`／`-messages`）：
  ...
  感測器與狀態變更階段逐台查詢，分子分母是**台數**（逐台不印分頁進度行，500 台會洗掉執行紀錄；感測器走全站分頁模式時進度只有起訖兩點）。
  各階段完成時寫出**耗時**——沒有這個數字，「哪個階段值得優化」無從判斷。
  單一查詢的分頁必須循序；逐台的感測器與狀態變更階段在台與台之間套 `PrtgFetchConcurrency`。
  **不以固定的整段時間誤殺大型環境的合法長同步**；這不等於無上限等待。每個外部請求仍有
  `PrtgTimeoutSeconds` 逾時，分頁／逐台輪詢有頁數或批次上限，所有迴圈都接受取消訊號；中止的手段是
  §5a 的停止鈕，收斂的保證是上面的停止條件、頁數上限與取消訊號（每台各自一份）。
...
- 查詢：目標 ≤ 2000 顆（常數 `FilteredSnapshotLimit`，暫定）時依 objid 排序、每 50 顆一個 `table.json?content=sensors&columns=objid,lastvalue_raw,interval&filter_objid=…` 請求（組字串與批次大小和資源守門共用 `PrtgResourceGuardProbe.BuildObjidFilter`／`MaxBatchSize`）；每批只收該批要求的 objid（PRTG 若忽略 `filter_objid` 會每批回整站，不擋會重複累加）；取回少於要求時每輪印一行缺幾顆；單批失敗只記數、其餘照做，全部批次失敗才算本輪失敗。目標超過 2000 顆時退回單發全站 `count=50000`（實測約 10～15 秒），回傳筆數少於 `treesize` 時寫截斷警告。兩種取法共用同一段解析與累積。
...
4. 資源守門的裝置（§12）：Sentinel 與 PRTG 位址比對到的裝置、覆寫清單感測器所在的裝置、鏡像中 corehealth 感測器所在的裝置——
   **不論守門是否啟用**。這些裝置多半不在主機主檔裡，少了它們，守門的自動偵測與 corehealth fallback 會因為範圍外的裝置不在鏡像中而找不到東西，
   覆寫清單的感測器會讀不到分類而被忽略。
...
清理的對象要分開判讀：`lf_prtg_devices` 是完整的監看裝置鏡像；`lf_prtg_sensors` 依「數值取數對象」
的取數範圍保存 sensor 鏡像；`lf_prtg_state_changes`／`lf_prtg_values` 則由結構同步成功後的範圍清除與保留期共同管理。
`PrtgScopePurge.RunAfterStructureSync` 只有在結構同步與主機對應全部成功，且 `CheckScope` 通過（範圍非 null、
非部分處理、監看裝置非空、裝置鏡像本趟成功更新），再由 `CheckShrink` 通過（已有監看裝置數基準，且減少量沒有超過
`max(基準 × 30%, 50 台)`）時，才呼叫 `DeleteOutOfScopeData`，立即刪除取數範圍外的數值與狀態變更列。
首次沒有基準、範圍計算失敗、空範圍、部分處理、裝置鏡像未成功更新或縮小保護擋下時，不得自動清除；
維護頁「監看範圍外資料」提供預覽與「確認清除」人工退路。資源守門所需裝置與 sensor 不能因數值取數範圍變小而被清掉。
...
- 感測器：範圍非空、感測器階段無例外（全站模式已收斂／逐台模式零失敗）、寫入數大於 0 時，刪除 `synced_at` 早於本趟起點的列——
  一條判準同時收掉「範圍外」與「PRTG 端已刪除」。不設過半保險：首次套用取數範圍時本來就會刪掉九成以上，感測器鏡像是
  PRTG 的複本，清錯下一趟即可重建。範圍外感測器上的人工分類一併刪除。另有兩道保護：
  **範圍內沒有任何對應成功的裝置時不清**（主機主檔暫時讀到空清單會讓對應全數落空、範圍只剩守門裝置，照清會把整份鏡像清光）；
  **本趟「查詢成功但回 0 顆」的裝置給一趟寬限**——它底下 36 小時內刷新過的列先留著並在執行輸出列出，下一趟仍回 0 顆才清
  （PRTG 偶發回空陣列時，不讓當晚對那台主機的規則評估無聲失效）。
- 結構同步通過上述範圍清除條件時，`lf_prtg_state_changes` 與 `lf_prtg_values` 的範圍外列會立即刪除；
  未通過條件的列才保留給維護頁人工預覽／確認或保留期處理。快照資料仍依其自己的保留規則管理。
...
「鏡像一顆感測器都沒有」的裝置（環境探測執行中不補，免得干擾探測的效能量測），每輪最多 50 台，以同一個逐台取法補抓感測器並重算自動分類（`BackfillSensorsForDevicesAsync`，
...
這些裝置**排在待補清單最前面**一併補抓；補上後範圍的守門項就會納入該裝置，之後由結構同步持續刷新。查不到的 objid、以及補抓後仍不在鏡像的 objid 記住不再查，清空時機同上。
補抓例外只寫一行警告，不進快照退避。補上之前主機詳情的 PRTG 區塊說明「感測器清單尚未取得」，未回報提示顯示「PRTG：無資料」（hint 代碼 `unknown`）。
...
- **回填套用與每日擷取相同的三重過濾與取數範圍設定**（§3a，共用 `PrtgValueFetchScope.SelectHosts`）：
  ...
  全量回填在實機是 4 萬多個 sensor × 30 天，跑不完；對從沒出過問題的主機回填也沒有分析價值。
...
PRTG 維護頁提供兩種共用執行狀態與停止鈕的探測。「完整環境探測」唯讀、背景執行並由前端輪詢狀態，產出這個 PRTG 環境的結構統計：
...
環境步驟只需要 PRTG 連線，回答「這台 PRTG 長什麼樣」；站台對照（`PrtgProbeSiteCheck`）另外讀本機鏡像、主機主檔與設定，
回答「套用到這個站台會怎樣」：取數範圍多大、鏡像有多少感測器在範圍外、範圍內的裝置查起來怎樣、資源守門找不找得到目標。
輸出接在同一份探測輸出後面，段落標題「站台對照」。環境探測失敗（連線或認證）時不跑；站台對照自己出錯只印原因，
**不改變探測的成功與否**。全程唯讀，不寫任何表或設定。鏡像或主機對應是空的（探測本來就能在啟用前跑）時只印說明。

- **[S1] 取數範圍試算**（不打 PRTG）：以夜間同步實際使用的同一份 `PrtgScopeDevices.Compute`（鏡像來源）算範圍，
  印範圍台數與四項組成、鏡像的裝置數與感測器數、**其中範圍外幾顆**。範圍外不代表必然清除；首次基準、範圍縮小保護與守門保留集合都可能阻止清除（§3c）。
  鏡像沒有任何裝置時印「請先執行同步結構與對應」並略過 [S2]；範圍為空時指路主機對應；範圍非空但沒有任何對應成功的裝置時，
  提示「下次同步不會清除感測器鏡像」（§3c 的保護）。
- **[S2] 範圍內裝置逐台查詢實測**：從範圍內挑最多 3 台（挑法與 9d-5 同一份：底下有非 Up 感測器者優先，其次 objid 小到大；
  沒有感測器的裝置也是候選、排在後面），每台查 `content=sensors&id=<裝置>`（與鏡像顆數對照）與
  `content=messages&id=<裝置>&filter_drel=7days&count=50`（判定文字與 9d-5 (b) 共用同一個函式）。
  結論行彙總規則同 9d-5；另以平均每台耗時 × 範圍台數 ÷ `PrtgFetchConcurrency` 估感測器與狀態變更兩個階段的秒數
  （以 7 天級距量測，回望 30 天的資料量更大、實際會更久）；範圍超過逐台門檻（§3c）時感測器階段走全站分頁，該項標示不適用。
  感測器那次查詢回傳少於 `treesize` 時在該行標示被截斷。最多發 6 次 `table.json`。
- **[S3] 資源守門目標偵測**（直接查 PRTG，§12 的即時來源，裝置與感測器全表各一次）：不論鏡像是否為空、守門是否啟用都跑。
  以 `PrtgResourceGuardTargets.Detect`（與夜間守門同一份判定）印 PRTG 主機是**以位址命中**、**改以 Core Health 感測器找到**、還是找不到，
  Sentinel 主機命中的裝置，以及守門會監看的 cpu／memory／corehealth 感測器數。命中的裝置逐台對照取數範圍：
  一台都沒命中時說明無從對照（不印通過）；有不在範圍的就列出並給處置（系統設定「資源守門」頁「自動偵測並填入」存入覆寫清單，快照服務的範圍補抓會把它們帶進鏡像，§3c）。
  覆寫清單有值時另印「已在鏡像／不在鏡像」各幾顆。沒有 Sentinel、PRTG 連線網址也解析不出主機時略過。即時來源的全表查詢是同步阻塞的、中途無法取消；回應被截斷時由來源自己的截斷警告出聲（直接進探測輸出）。

### 小範圍資料流驗證

「驗證小範圍資料流」不執行上述全站探測，從最近的主機對應中挑前 20 台已成功對應、仍啟用的主機作候選，選出其中一台的一顆未暫停且符合 sensor type 白名單的感測器。對該 sensor 的昨天一天，直接呼叫正式 `PrtgFetchService.FetchValuesForSensorsAsync`（`historicdata` 每小時聚合）並冪等寫入 `lf_prtg_values`，再按 sensor 與日期讀回。共用取值路徑會在寫入前再次限制資料日期，即使 PRTG 忽略 `sdate`／`edate` 也不會寫入範圍外的列。只有本趟確實寫入且讀回至少一筆 `ok` 數值才判通過；零筆、只有 `unknown`／`no_data`、找不到對應或 sensor 都明確判未通過。只寫此 sensor 當日真實數值，不清除其他資料、不執行結構同步；取數排程、結構同步或回填進行中時拒絕啟動。即使通過，結論也只限這個樣本的**目前**主機對應、API、正式解析與資料庫路徑；昨天的歷史歸戶是否與目前相同，以及定時快照、狀態變更、規則命中與全站負載，仍需各自驗證。
...
特別是 sensor 語意分類要不要做、主機對應的實際覆蓋率，以及取數範圍套用到這個站台的結果（站台對照）。
...
功能依性質分置——**靜態設定與同步／回填的啟動入口在 PRTG 維護頁，排程作業頁只呈現動態狀態**，
...
| 頁面 | 內容 |
|---|---|
| **PRTG 維護頁 `/admin/prtg`**（權限 Maintain，側欄「系統管理」內緊鄰 NetIQ） | 六個頁籤：連線／擷取參數（**首欄「PRTG 擷取」四選一下拉＝總開關＋取數範圍**）／鏡像狀態（含「同步結構與對應」§5a）／環境探測（含校準匯出 §11 與資料搬運 §10）／指定主機補值／使用效果。各頁籤的卡片與儲存行為見 docs/WEB-SPEC.md §9.9e，此處不重複 |
| **設定頁「資源守門」頁籤** | 八個 `PrtgResourceGuard*` 欄位與「預覽／自動偵測並填入」兩顆鈕（§12）。與該頁其他設定共用整包儲存，不另開儲存鈕——分兩顆時按其中一顆，另一區未存的改動會在重載時被覆蓋回舊值 |
| **排程作業頁** | 模組狀態（未啟用時附連結指向維護頁「擷取參數」；啟用時一併顯示生效範圍）、同步／回填進度與停止鈕、每日擷取進度軌（回填天數在維護頁設定）。**這頁沒有同步／回填啟動鈕與開關**；啟動操作只從 PRTG 維護頁進入 |
...
## 9a. 磁碟可用空間趨勢（`disk_free_trend`）

F1 逐 sensor readiness 按頁查詢，每頁最多 100 顆；第 1 頁另分批抽樣最多前 100 顆計算 readiness 摘要，回應會標出摘要樣本數及是否截斷，不能解讀為全局就緒數。逐列缺小時資訊以 28×24 bitmask 表示。真實 PRTG 28 天資料及頻道語意尚未驗證。

內建規則 `builtin-prtg-disk-free-trend`（seed v8）預設停用。規則頁可編輯專用低水位、最低下降速度、
耗盡天數與有效日等門檻；不使用狀態規則的分鐘／次數門檻。正式及預覽評估窗口最多 730 個已完成日；草稿試算窗口為 1–730 日，依每批不超過 100,000 個預期歷史點分批（約 35 日 × 100 顆 sensor 或 730 日 × 5 顆）。試算回傳逐 sensor 就緒、排除原因、預期命中及依目前抑制設定估計的抑制數；估計不含問題靜音與實際派工。它只讀已落地資料，不呼叫 PRTG，也不寫 finding、風險、案件或交辦。結果按「完成日 × sensor」配對分頁；配對列與未抑制命中配對數只描述試算結果，不是正式派工上界或交辦張數（正式派工另受規則啟用、finding 與案件／派工閘門影響）。單顆合法 Probe 窗口可超過 35 日，並有 ready-hit 正向試算；這不代表實機預警已驗證。
規則維護 UI 已提供未儲存草稿唯讀試算與分頁結果；正式每日路徑僅在管理者明確啟用規則時
產生趨勢 finding，之後沿既有 PRTG finding 管線處理。
正式每日評估逐 sensor 分頁掃描完整候選集合，但只評估最新已完成日；只讀已落盤資料，不等待同一趟稍後的
觸發式取值或即時呼叫 PRTG。

判定逐顆要求目前對應有效、未暫停、白名單內、分類為 disk，並具備最近 28 日每日有效小時達標的資料及
仍有效的頻道／單位語意證據。低 coverage 的 `sampled`、未知／無資料、單位不明或語意證據失效均跳過；
不足是未知，不當作正常或零。此規則不承接狀態型 Down 的 PRTG 確認旗標。停用時不產生趨勢 finding。

批次語意驗證最多接受 5 顆，依請求順序逐顆執行；執行中取消會停止目前工作並略過後續 sensor。`requestId` 同內容重試只在目前行程內去重，去重快取最多 512 筆、保留 15 分鐘，行程重啟後不保留。語意驗證只核對有限資料的頻道／單位／值，不等於已確認未知語意、不等於 28 日趨勢或真實預警效果驗證，也不會自動啟用規則或派工。

規則首次啟用與之後重新啟用都由後端確認至少一顆 sensor 同時有 28 日有效資料及有效語意證據；只靠畫面
停用按鈕不構成保護。探測頁可對單顆 sensor 查昨天有限歷史資料，將主要頻道、單位與落地值比較；無法自動
確證時，管理者只能對最近 24 小時內有成功比對的候選補人工核實，且留下理由及稽核。主機／device／sensor
類型、頻道解析語意變動會使舊證據失效。資料準備度、語意確認、規則試算命中、實際預警效果與規則啟用是
不同狀態；程式及替身測試通過不代表正式站台已校準。

正式站台 28 日校準仍待執行。尚未以真實 PRTG 28 天資料或頻道語意驗證。已觀察的匯出約只有 6 天可用資料，且 `ValueSensorSummaries.Unit` 為 null；
因此尚無證據確認正式磁碟 sensor 的 28 日有效資料、PRTG 真實主頻道與百分比單位，也未驗證真實預警效果。readiness 摘要只抽樣最多 100 顆，不可外推全站。程式候選解析及替身測試不能取代現場核對。管理者的
實機驗收入口為「系統管理 > PRTG 維護 > 環境探測」：

1. 等候並核對指定磁碟 sensor 的逐顆 28 日有效日數及有效小時；列表摘要最多抽樣 100 顆，須按 sensor 個別判讀。若不足，使用「指定主機補值」先預覽主機、日期、請求數，再執行有限回填。局部回填停止只取消該次 run；回填只補值，不回溯建立 finding 或交辦。
2. 從準備度清單選一顆已對應、白名單內、未暫停的磁碟 sensor，執行「驗證昨天語意」；確認頻道、單位、方向及資料庫比對結果。`Unit=null` 或只顯示待人工覆核時，依 PRTG 實際 sensor 畫面核實後留下有理由的人工確認。
3. 到「規則維護」編輯停用草稿並執行唯讀試算，檢視逐日／sensor 配對的排除原因、命中與門檻；目前抑制設定估計只供參考，不含靜音或實際派工。試算及程式測試不代表真實命中率。
4. 只有在實際資料與語意核對完成、接受暫定門檻及影響範圍後，才明確啟用。之後觀察真實 finding、案件／交辦、回覆、抑制與誤報情況，再記錄效果；在這些證據出現前，校準與效果均保持待驗。

## 10. 資料搬運（匯出／匯入）

值型規則門檻仍需用**真實累積的數值**校準，但數值累積在正式機
...
1. **累積**：數值快照（§3b）持續累積白名單內、有對應主機的全部 sensor 的 `sampled` 列；激進策略另由夜間逐顆查詢寫入觸發主機的 `ok` 列。
  ...
  需要較長歷史的值型分析須按各規則自己的逐 sensor 品質與日期門檻判定；磁碟趨勢 readiness 見 §9a。
...
狀態規則的暫定門檻與磁碟趨勢門檻仍要靠正式站台資料校準。`/admin/calibration`
（頁面規格見 docs/WEB-SPEC.md §9.9f）回答既有四類資料量是否足夠並提供匯出；四項共通狀態不是磁碟規則
的就緒或啟用閘門。磁碟規則另依 §9a 的逐 sensor 28 日 readiness 與語意證據判定。程式測試及校準匯出
均不能替代正式站台頻道、單位、歷史品質與預警效果的人工驗證。
...
- **本節既有四項校準的主機涵蓋天數＝其名下 sensor 涵蓋天數的最大值**——只要有一個 sensor 累積夠久，
  這台主機就有基線可用。
...
- 值型基線卡另列：快照目標 sensor 數（`SnapshotTargets`）、近 24 小時有持久回報寫入的 sensor 數與平均 coverage（`SnapshotSensors24h`／`SnapshotCoverage24h`，含 coverage 不足的取樣列——回答聚合回報寫入情形而非「可不可用」；診斷沒有逐 sensor identity 證據，不能據此宣稱 sensor 層 100% coverage）、逐日列數（`ValueBaselineRows`，預告完整匯出大小）。
...
本節校準資料匯出閘門：四項全部達「可用」以上才解鎖，未達標需勾「仍要匯出」覆寫，兩者都寫稽核；
此匯出閘門只管本節匯出，不限制 §9a 磁碟草稿試算或規則啟用。
...
鏡像的感測器只含取數範圍內的裝置（§3c），守門比對到的裝置、覆寫清單感測器所在裝置與 corehealth 所在裝置一律在範圍內，夜間讀鏡像因此讀得到守門需要的感測器。**全新安裝**且 PRTG 位址對不到裝置時，corehealth 所在裝置不在第一趟的範圍內，夜間偵測找不到它——在維護頁以直接查 PRTG 的來源偵測一次並存入覆寫清單；
