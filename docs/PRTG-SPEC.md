...
功能依性質分置兩頁——**靜態設定在 PRTG 維護頁，動態工作在排程作業頁**。
設定頁沒有 PRTG 頁籤（只在「資料保留」頁籤留一行指路）：
...
> 設定頁已不再送出任何 PRTG 欄位，無條件寫入時設定頁存檔會把 PRTG 位址清空、
...
| PRTG 規則門檻 | 一個 sensor-日 | 狀態變更涵蓋 ≥28 天且命中 ≥30 筆 | 涵蓋 ≥56 天且 ≥100 筆 |
...
- **規則門檻**：近 56 天每規則每日命中數（依 `lf_top_issues` 的 EventKey 前綴分組——
  該表沒有 rule_id 欄，且 PRTG finding 的 EventId 恆為 0，既有聚合會讓四條規則塌成一格）
  ＋四條規則的門檻現值。
...
- **殘留判定**：每候選主機日的指標（候選組數、明細總數、前二組集中度、機械型態佔比、
  ...
  **不含任何帳號名稱**——校準只需要統計形狀。指標計算與正式判定**共用同一份實作**
  （`ResidualCredentialDetector.EvaluateMetrics`），兩邊分岔會讓匯出的數字與實際判定不一致。
