...
| 3. 狀態變更 | `table.json?content=messages&filter_drel=…` | `AppendStateChanges`（一個日期區間一次取完、依日分桶，去重） |
...
- **狀態變更一次取一個日期區間**（`PrtgFetchService.FetchStateChangesRangeAsync`）：區間＝最早目標日的前一天到今天。
  規則評估看目標日前後一天的變更，只取目標日會讓跨午夜的 down 持續時間算不準；今天的部分列之後會被冪等補齊。
  每列依時間的日期分桶寫入，區間外的列丟棄。執行輸出印讀取頁數、讀取筆數、新增筆數（「其餘已存在」——冪等重跑新增 0 是正常的）、
  多日時的各日新增數、是否提早停止與耗時。
...
- **依時間提早停止（只省查詢、不省資料）**：messages 依時間由新到舊回傳，翻到整頁都早於區間時後面的頁全是不要的資料。
  ...
  停下時算正常收斂，執行輸出寫明停在第幾頁、該頁最新時間與門檻日期。
...
- **狀態變更整趟只取一次**（區間＝最舊回填日的前一天到今天，§3a），之後逐日做數值；單日失敗不中止整趟，最後輸出成功、失敗、略過天數。
...
- **啟動閘門**（依序）：未啟用、連線不齊、環境探測執行中、鏡像沒有感測器、**取數執行中**、**同步結構與對應執行中**、**近 31 天沒有任何主機對應**。後兩類執行中的訊息帶已執行分鐘數並指路停止鈕；沒有對應的訊息指路「同步結構與對應」。與取數或同步同時跑會對同一台 PRTG 疊查詢。
...
9. **效能量測**（純診斷，結果不影響探測成敗）：各子量測獨立容錯，任一失敗只印原因、繼續下一個。
   成本固定為 11 次 `table.json` 與 87 次 `historicdata`。
