...
- **[S2] 範圍內裝置逐台查詢實測**：從範圍內挑最多 3 台（挑法與 9d-5 同一份：底下有非 Up 感測器者優先，其次 objid 小到大；
  ...
  （以 7 天級距量測，回望 30 天的資料量更大、實際會更久）。最多發 6 次 `table.json`。
- **[S3] 資源守門目標偵測**（直接查 PRTG，§12 的即時來源，裝置與感測器全表各一次）：不論鏡像是否為空、守門是否啟用都跑。
  ...
  有不在範圍的就列出並給處置（系統設定「資源守門」頁「自動偵測並填入」存入覆寫清單，快照服務的範圍補抓會把它們帶進鏡像，§3c）。
  覆寫清單有值時另印「已在鏡像／不在鏡像」各幾顆。沒有 Sentinel、PRTG 連線網址也解析不出主機時略過。
