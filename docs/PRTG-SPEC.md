...
8. **分頁語意診斷**（純診斷，結果不影響探測成敗）：對 `devices`／`sensors`／`messages`（帶 `filter_drel=7days`）
   ...
   結構同步與資源守門的分頁迴圈都以「遵守 `start`」為前提，這一步就是驗證那個前提。
...
即時來源**全量分頁抓裝置與感測器再於記憶體過濾**，不用 `filter_parentid` 逐裝置查詢：
偵測要先比對完位址才知道命中哪些裝置，逐裝置查會變成 N 次往返；全量兩次往返反而少。
分頁另有頁數上限當保險絲——終止條件是「這一頁不滿」，若 PRTG 忽略位移而每次回滿一頁，
沒有上限就會是無窮迴圈，而它跑在 HTTP 請求執行緒上。
