...
- **使用效果**：`GET api/prtg/effectiveness` 預設查最近 30 天，可指定最多 366 天；分開統計 PRTG finding、案件、交辦、已回覆、已抑制、期間靜音區間有重疊的 PRTG 問題檔案及跨來源佐證主機日。靜音數是問題檔案數，不表示期間發生 finding；各指標口徑不同，不構成轉換率或解決率；資料缺漏時以不可計算呈現。
