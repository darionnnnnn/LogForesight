...
- **擷取參數**：忽略 SSL、逾時、併發、回填天數、保留天數、sensor type 白名單，
  ...
  ——兩者現在分屬不同頁籤，不帶會讓測試結果與存檔後的實際行為分岔（秘密欄留空皆 fallback 已儲存密文；
...
- **鏡像狀態**：device／sensor 計數、各類資料最新時間點、白名單覆蓋量級、主機對應摘要、
  ...
  **IP 對多主機**（要從候選主機中挑一台），指派 modal 依型別分岔——
  ...
  指派用的主機清單走**不分頁端點**：舊寫法送 `pageSize=2000` 會被後端夾成 200 台，
  超過的主機選不到且畫面無提示。
  **未對應清單已移除**——取數與規則只認 `ok` 的對應，未對應項目不影響任何行為；
  摘要的計數保留。
...
- **狀態卡的三條進度軌**：本機／NetIQ／PRTG 三路並行，各自一條互不覆蓋的軌。
  ...
  **不清空**——清空會讓那條軌整個消失，使用者看不出那一路究竟跑完了沒。
  ...
  「PRTG 觸發式取數　已取 N 個 sensor（等待分析結果）」，不再是一片空白的「準備中」。
- **首次載入不慢半拍**：狀態查詢與 options／ai-status／settings 同時發出，
  不等那三支回來；頁面狀態文字初值為「載入中…」而非空白
  ——否則進站會先看到「沒有執行中」，過一下才跳出執行中。
- **資源守門暫停中**：`status.pausedReason` 有值時狀態卡顯示徽章
  （模組規格見 docs/PRTG-SPEC.md §12）。詳細的觸發 sensor 與數值走執行詳情的 Milestone
  ——`IRunProgress` 只帶得動 `(phase, done, total)`，刻意不為此擴充介面。
- **執行紀錄分欄**：列表與執行詳情各有「本機／NetIQ／PRTG」三欄
  （各為「分析 n（失敗 m）」與 PRTG 狀態徽章＋sensor 數＋觸發主機數），
  執行總表每日列另加一格 PRTG 狀態徽章。資料來自 `BatchRun` 的分路結構化欄位；
...
——過去它直接建立紀錄物件、沒掛 target，執行詳情永遠是空的。
