...
AD 帳戶本身的鎖定依網域原則；Web 端另對登入請求按帳號及可信來源 IP 節流，命中門檻時
不再呼叫 AD，減少密碼噴灑觸發網域鎖定。救援帳號不受共用 IP 門檻連坐，仍受本身鎖定限制。
...
無處置能力」是半套。能力進 JWT；負責人變動會推進權限版本，下一次請求重算能力並無感刷新
原工作階段的 token（可見範圍則即時生效）。
...
- **409 另外兩種語意（執行類端點）**：「被另一個執行互斥擋下」（取數執行中不放行手動 AI、
  回填與結構同步／環境探測互斥）與「沒有可停止的對象」（停止類端點找不到進行中的執行）
  一律回 409，不是 400。分界線是「你送錯了」（400，例如天數不合法、服務未設定）
  與「現在不行，等一下再試」（409）——呼叫端要分得出重試有沒有意義。
  訊息一律回可直接顯示的中文，前端不依狀態碼組字。
...
6a. **全站執行中告示**：分析與站台跑在同一個行程，執行期間整站回應變慢——告示放在共用版型
   （`core/layout.js` 組裝、`_Layout.cshtml` 留容器），**所有已登入頁面共用同一份**，
   只有一頁看得到原因等於沒有配套。內容：固定「分析進行中」、有分母時帶「第 N／M 單位」、觸發者、
   以及「畫面回應可能較慢，資料仍是完整的」。
   執行中 30 秒、閒置 60 秒輪詢 `GET api/run-activity`；**閒置只降頻不停掉**——停掉的話
   停在同一頁不動的使用者永遠等不到下一次執行的告示。閒置時容器完全空著（零高度不佔位），
   不可關閉、單行、不覆蓋主內容。API 失敗當作沒在跑、同樣不停輪詢。
   該端點**任何登入者可讀**（刻意不掛 `[Permission]`）：變慢的是所有人的畫面。
   狀態另以 `lf:` 前綴的 `CustomEvent` 廣播並保存最後一次於 `window`，頁面模組**訂閱它**
   而不是各自輪詢（比第一次回應更晚才載入的模組讀得到最後狀態，不會停在初值）。
   面板型的一次性查詢（AI 對話面板、說明書問答的「AI 回應會變慢」提示）例外：開啟當下查一次、不輪詢。
   觸發者只對具 DevMonitor／Maintain 的使用者回傳，其餘角色的告示不講是誰按的。
   **`isRunning` 是「取數或 AI 任一在跑」的聯集，只能用來決定告示出不出現**；
   要判斷「某個動作現在能不能做」必須看 `isFetchRun`——後端只在取數執行中擋主機更新，
   AI 單獨在跑時那個動作是允許的。
6b. **GET 逾時 60 秒，寫入不逾時**：讀取逾時後把骨架換成失敗狀態並可重試；已知會超過的
   查詢由呼叫端以 `timeoutMs` 覆寫（校準判定、報表匯總、帶處理狀態篩選的記錄查詢）；
   POST／PUT／DELETE **一律不設逾時**——長時間寫入若在客戶端中止，伺服器仍會做完，
   使用者再按一次就是重複執行（重複匯入、重複套用）。逾時文案要指路（縮小查詢範圍）。
6c. **載入失敗狀態附「重試」鈕**：重試只重跑那一段載入流程（整頁重載會丟掉已設好的篩選條件），
   按下後先回到骨架狀態。**找不到資料（404）不給重試鈕**：重試永遠不會成功。
...
- **分析執行中告示**：不屬於本頁，在共用版型上，所有已登入頁面共用同一份（見 §8.6「全站執行中告示」）。
...
- API：`GET api/records?hostIds=&groupIds=&from=&to=&riskLevels=&categories=&severity=&eventId=&statuses=&overdue=&sort=&dir=&page=&pageSize=`
  ...
  「風險→關聯→日期」緊急程度排序）
- **缺省日期下界分兩種**：呼叫端沒帶 `from` 時，**沒有處理狀態類篩選**（`statuses`／`overdue`／
  `unassigned`）的路徑下界為「昨天往前 90 天」；**帶了任一種**則維持整個保留期。
  分開的理由：帶狀態篩選是「依狀態找案子」，把窗口切掉會讓一件開了幾個月還沒結的案子從結果裡
  消失——那是使用者的待辦不見了，比查詢慢更嚴重。前端各頁一律會帶 `from`，受影響的只有
  直接呼叫 API 與「依問題視角展開受影響主機日」時使用者手動清空日期欄位的情況。
- **「下一筆未處理」捷徑**：`GET api/records/next-unhandled?hostId=&date=` 只回下一筆的主機與日期
  （沒有下一筆時回 null），呼叫端要的只是一個連結。判斷「下一筆是哪一筆」在後端：
  以未結案的高＋中風險日、依預設排序取清單後找目前這筆的下一筆；目前這筆不在清單裡
  （例如剛結案）取第一筆。窗口是整個保留期（同上一條的理由），成本由跨請求快取吸收
  ——鍵含可見主機集合與可見度設定，少任一維度就是把 A 的待辦端給 B。
- **依問題視角補上「時間形狀」**：
...
- **第四視角「依問題」**：一列一個問題（Source＋EventId 分組，
  ...
  **已交辦**（「N／M 台」＝已在進行中交辦單內的主機數／總主機數，連到 `/work-orders?source=&eventId=`；
  零台顯示灰字「未交辦」；N 由同一批已套檢視範圍與查詢期間的出現點計算，與 M 同一母體——
  全站計數會讓只看得到 10 台的人看到「300／10 台」並洩漏看不到的台數）；預設排序嚴重度→主機數→總次數（**不採 §9.1／§9.6 的 PriorityScore
  ...
  改以 program 比對 Linux 規則，同一個 program 命中兩條以上規則時給 null——聚合層沒有訊息
  內容可判斷是哪一種，寧可不顯示也不顯示錯的。前端在沒有說明可顯示時補一句固定文字說明
...
- **「下一筆未處理」捷徑**：處理完一天後不必回清單自己找下一筆。哪一筆是「下一筆」由後端決定
  （端點與窗口見 §9.2），前端拿到什麼就連到什麼；沒有下一筆時整顆按鈕不顯示。
  在主要內容載入完成之後才發出且不阻塞——它走的是記錄查詢的慢路徑，串在主載入裡會讓整頁陪它等。
...
- **風險日詳情處理面板**：
  ...
  12. **重點問題表格欄位合併**：「來源/Event」
     ...
     `@media print` 解除收合。
  12a. **殘留憑證徽章與登入失敗明細表**（`issueCell` 內，依此順序排在 keyDetails 之前）：
     ...
     - `IssueDto.LoginFailureDetails` 有值時（4625／4771／Linux 認證失敗簽章），
       ...
       超出 10 列時下方小字「另有 N 筆明細未顯示」；後端標記截斷
       （`LoginFailureDetailsTruncated`）時改報總次數並言明明細僅保留前 50 組
       （`LoginFailureTotalCount`）。帳號／來源為空顯示「（不明）」、類型為空顯示「—」、
       電腦帳號在帳號後加小字「（電腦帳號）」。「類型」「原因」是後端
       `LoginFailureTextFormatter` 轉好的白話字串（`LogonTypeText`／`ReasonText`）——
       **前端不持有第二份對照表**。
...
- **指定主機更新鈕**：就近原則——看著這台主機覺得資料舊了當場按。開確認 modal（可選一次性回補天數
  1~14，不落地設定）後送 `POST api/admin/schedule/run`（scope=host）；
  **取數執行中時停用（不是隱藏）並在按鈕旁與 modal 內各說明一次原因**：按鈕消失會被當成
  「權限被拿掉了」，而說明只放在 modal 裡等於看不到——按鈕一停用 modal 就打不開，
  使用者面對的仍是一顆沒有理由的灰按鈕。狀態訂閱 §8.6 6a 的全站告示事件、只看 `isFetchRun`
  （理由見該條），不自己輪詢。本機直讀主機走
...
- **進行中**：交辦單清單——單號｜問題｜處理人（停用／暫停接單徽章）｜成員（台數、狀態彙總、逐日同步中台數）｜
  逾期｜未回覆｜期限｜來源｜建立｜最近新增（最近一次加入成員的時間）｜狀態。篩選：狀態（進行中／上報中／有逾期／未回覆／已結案／全部）、
  排序、**群組**（處理人所屬的使用者群組，選項來自 `GET api/work-orders/handler-groups`，派工池群組名後加
  「（派工池）」）、「只看自靜音恢復」（問題靜音在近 7 天內結束的單）、「只看處理人已停用」（離職或停用後
  逐單改派或代為結案用）。網址參數 `source`／`eventId`／
...
- **待派**：期間（預設昨天往前 7 天）內出現、卻沒有任何進行中案件的問題，以派工試跑決定——
  SQL 只取候選出現點（排除已有進行中案件），記憶體以與夜間相同的 `WorkOrderDispatcher.Decide`（含 ⓪ 靜音）
...
**夜間掛接**（每個主機日寫入後；①②③ 在 `IssueCaseCoordinator.AttachNewDay`，剩下沒人接的問題交給 `NightlyDispatch`，由 `WorkOrderDispatcher.Decide` 判定 ⓪、閘門與 ④⑤⑥；本機、NetIQ、PRTG 三路並行共用一份
派工脈絡，決策與寫入整段在脈絡鎖內）：
⓪ 問題目前靜音中，或紀錄日落在靜音區間 → 不派新工（與讀取側單一列判定同一條規則；②③ 照常，
靜音日本來就視同已有結論）；
...
- 交辦單：問題目前靜音中的單推導為暫停。處理人清單與徽章預設排除，總覽預設列出並標「暫停」與靜音至；
  ...
  （預設）或「代為結案為不處理」。代為結案逐張寫入與稽核；中途失敗時回傳已完成、失敗及未處理單號，
  畫面保留輸入並要求先核對失敗單（該單可能已部分更新），再決定是否重試。
...
**通知**（`MailEnabled` 且 `MailNotifyWorkOrders`，預設關）：依問題視角建單、改派（新處理人收建單信、舊處理人收「已移交」）、
取消各一封；夜間續掛與自動派工**每趟每人一封**彙總，自動派工以延續性偏好派回給上次修好的人時，
該單那一行註明「其中 N 台是復發」（`DispatchDecision.Recurrence` → `NightlyDispatchOrderLine.RecurrenceMembers`）。內容列前 20 台主機名，超過寫「…等 N 台」。
詳情頁日層級指派與「立即派工」建的單不寄建單信。收件人是目前操作者時不寄；收件人沒有 email 記警告、不擲例外。站台沒有對外網址設定，信末寫「請至站台的『交辦單』頁檢視單號 N」。
...
- **異動說明的句型規則**（`PermissionChangeService.GenerateSummaryText`，未展開就要看得懂
  ...
  1. 資訊順序一律「操作者 → 動作 → 對象」；操作者缺漏時改用被動句（本機監控來源恆無操作者，
     ...
     4670「物件的權限已變更」依**物件類型**組句：物件名稱有值時「{物件名稱}（{物件類型}）」，
     常見的「物件名稱: -」則用「{物件類型} 物件」，並附處理程序的**檔名**（完整路徑不入句）。
  2. 句中帳號一律短名；**不含主機名與類別標籤**（同列已有各自的欄位；例外：`other` 與
     `other` 這個沒有專屬句型的類別，句首保留「類別標籤：對象」形式；`summary` 只保留對象，
     不重複類別標籤）；ACL 與稽核政策的
     安全性描述元（SDDL）不入句——那是展開明細的內容，塞進列表會把句子擠爆。
  3. 缺漏處降級為全形括號佔位字（對象依類別為「（未能解析群組名稱）」「（未能解析路徑）」
     「（未能解析對象）」「（未指定對象）」，成員為「（未能解析成員）」；**「（未能解析路徑）」
     只用於檔案／資料夾類**——Token、登錄機碼這類物件走 `object_acl` 的句型，說「路徑」是錯的）。舊資料的對象欄可能存著事件來源退路值，顯示層會辨識其形狀
...
- **例行同步彙總列**（`change_type` ＝`例行同步（彙總）`、類別 `summary`）：AD 自動化程序的
  ...
  null**，否則畫面會顯示「0」。展開明細另顯示「涵蓋時間：hh:mm～hh:mm（N 對）」（起訖同一主機日，故只印時分），
  資料來自 `covered_from`／`covered_to`／`pair_count`（欄位語意見
  [DB-SPEC.md](DB-SPEC.md)）；三者為 null 的列不顯示這一行。
- **異動前／異動後兩者皆空時整塊不渲染**：兩格都印「（無）」等於用兩列說「沒有內容」。
- 站台啟動時會清掉沒有明細可展開的每日總計列（`change_type`＝`權限異動（彙總）`），
  只跑一次（狀態存在 `permission_legacy_summary_cleanup`）。
...
- **排行卡的「主機｜問題」切換**：卡 header 的 toggle
  ...
  （目前靜音中的全部問題：問題、到期日、原因、設定者；為零不出現）。7 天內到期且本期仍在發生的靜音另由體檢報告提醒。
...
- 清單（Id/類別/嚴重度/Origin/Enabled/已修改徽章/種子有新版標示）；
  ...
  畫面文字只寫在 `core/alert-tools-table.js` 一處，設定頁「自動派工」段共用，連到說明書 `#alert-tools`；
...
- owners.csv：一台主機多列＝多位負責人，檔案中出現的主機**負責人整組取代**；帳號不存在
  **自動建立**（User 角色、無群組，AD 登入時補齊其他資訊）。預覽的提醒文字自「負責人不會
  ...
  使用者在下一次請求會取得處理權限（檢視範圍即時生效）」——§2b 起負責人本身即授權路徑與
...
- **掃描是背景工作（不是同步請求）**：`POST api/admin/netiq/scan` 立即回
  ...
  （回 `status`＝running／completed／failed／canceled，另有 `serverName`／`subnetPrefix`（續看前比對是否同一台）、`stage` 進度文字、
  `hostsFound` 即時台數、完成時 `result`、失敗時 `error`），並提供
  `POST api/admin/netiq/scan/{jobId}/cancel` 取消。**沒有同步掃描端點。**
  - 背景執行的理由：一輪翻頁最多 50 頁、實測約 85 秒，互動請求的秒級預算容不下
    殘差輪掃——背景工作的總預算是 900 秒。
  - **全站同時只允許一個進行中的掃描**（再啟動回「已有掃描進行中」的驗證錯誤）。
  - 工作狀態**只存在行程內記憶體**，站台重啟即消失（前端顯示「工作不存在或已逾期」
    並允許重新掃描）——掃描可重跑，刻意不引入持久化。
  - 精靈關閉時**只停輪詢、保留工作識別**；重開時若該工作仍在進行中**且屬同一台 Sentinel**
    則續看（不同台顯示提示、不接手——舊結果配上新選 Sentinel 的 OS 預設值匯入會錯配），
    已結束的舊工作則放掉識別、照常啟動新掃描。不保留識別的話，背景工作還在跑而重開精靈
    會啟動新掃描並吃到「已有掃描進行中」，使用者被鎖到工作自然結束為止。
...
- **併發下拉**（`scan-concurrency-select`，請求欄位 `concurrency`，`[Range(1,3)]`）：
  ...
  **每次掃描自選，不落地成系統設定**。展開後只有一個分段時併發沒有作用（畫面提示已明說）。
  ...
  更多段」。各段結果依**分段原順序**合併，與完成先後無關；主機清單與「掃了但沒掃乾淨」
  只採計跑完的段（沒跑完的段會被列進「未掃描的網段」，同時又貢獻半套清單會自相矛盾），
  但**警告一律保留**——段中途被預算中斷，它先前產生的涵蓋警告仍然成立。
...
- 取代原本分散在批次 appsettings.json（AI 位址）與程式碼寫死常數（未處理等級門檻、補充／留存天數）
  ...
  2d. **外觀／品牌**：主標題
     ...
     空＝不顯示副標題）、自訂圖示（`BrandIconDataUri`，**只收 PNG／JPG** 的 data URI、
     ...
     **版面**：兩處共用 `Views/Shared/_Brand.cshtml`（`.lf-brand`，尺寸差異只由
     `--sidebar`／`--login` 變體的 CSS 變數表達）。副標題的對齊由 `core/brand-align.js`
     量測後決定——自然寬度小於主標題時撐開 `letter-spacing` 與主標題左右貼齊，
     大於等於時字距歸零靠左對齊（過長由 CSS 省略號截斷）。`text-align: justify`
     對單行不生效，而兩段文字都是設定值、字數不固定，故無法純 CSS 處理。
...
六個頁籤（`ui.js` 的 `bindTabs` 手作頁籤，非作用中頁籤初始 HTML 自帶 `d-none`）。
`bindTabs` 的 `{ hash: true }` 選項讓切換同步網址 hash（`history.replaceState`，
不塞瀏覽歷史）、進站時依 hash 啟用對應頁籤；校準匯出與資料搬運藏在第四個頁籤裡，跨頁指路需要直達：
...
- **擷取參數**：首欄 **「PRTG 擷取」四選一下拉**（＝ `PrtgEnabled` 加 `PrtgValueFetchScope`，
  ...
  **資源守門不在這一頁**——它同時節制 NetIQ 取數與 PRTG 擷取兩路，設定在
  ...
  ——兩者分屬不同頁籤，不帶會讓測試結果與存檔後的實際行為分岔（秘密欄留空皆 fallback 已儲存密文；
  連線失敗回 `success=false` 就地顯示，不當系統錯誤）。
  存檔走 **`PUT api/admin/settings/prtg` 專屬端點**，只寫 PRTG 欄位——
  刻意不走整包設定更新，那會在「讀取到送出之間」覆蓋他人的改動
...
- **鏡像狀態**：**「同步結構與對應」的入口與上次結果**（docs/PRTG-SPEC.md §5a；
  ...
  **停止鈕**（只在同步執行中出現；上次同步未成功時，狀態文字要說出「鏡像可能不完整，
  ...
  device／sensor 計數、各類資料最新時間點（含一行**數值快照**：最近成功時間、sensor 數、生效間隔；退避中標 `text-warning` 並寫出連續失敗數；前置條件不成立時附「目前暫停：原因」）、白名單覆蓋量級、主機對應摘要、
  ...
  **IP 對多主機**（要從候選主機中挑一台），指派 modal 依型別分岔——
  兩種衝突要做的事不同，混在同一個介面會讓人選錯。
  指派用的主機清單走**不分頁端點** `GET api/admin/hosts/all`（只回 id／name／ip，Maintain）：
  一般主機列表的 `pageSize` 被後端夾成最多 200，拿它取全部主機會讓超過的主機選不到且畫面無提示。
  未對應清單另列於摘要下方，方便確認 PRTG 獨有裝置；取數與規則仍只認 `ok` 的對應。
  指派／排除／移除**寫入後同步重算當日對應**，否則那一列要等隔天夜間批次才消失，看起來像沒生效；
  重算失敗只回警告不讓操作失敗（人工對應本身已寫入，回報整體失敗會誘使管理者再按一次）。
- **環境探測**：包含環境探測、磁碟值型資料準備度與語意驗證、**校準數值匯出**（§9.9f）、**資料搬運（開發用）**
...
### 9.9e-1 PRTG 快照診斷、磁碟準備度與使用效果（`Maintain`）

`/admin/prtg` 除連線、擷取參數、鏡像狀態與環境探測外，提供指定主機補值及使用效果入口；各項資料操作均限 `Maintain`。

- **快照診斷**：鏡像狀態頁以持久嘗試／寫入計數，加上既有 `PeriodStart` 索引資料的 SQL 聚合，診斷最近 24 個完整小時已落盤的 quality 與 coverage；不呼叫 PRTG。可區分 coverage 25% 不足、`ok` 覆蓋後可用及寫入失敗等結果。聚合沒有逐 sensor identity 證據，不能證實每顆 sensor 成功或 100% coverage，也不代表已具 28 日歷史。系統健康 `GET api/health/detail` 另呈現最近兩小時摘要；資料庫探測失敗時會 early-return 最小 Down DTO，不再讀取其他 storage-backed 狀態（有定向測試）；正式站台故障情境仍待驗。
- **逐 sensor 磁碟準備度**：環境探測頁 `GET api/prtg/disk-readiness` 每頁最多列 100 顆；第 1 頁另以最多前 100 顆計算 readiness 摘要，不是全體 sensor 的就緒數。每顆 28×24 小時缺口以 bitmask 回傳。此為站內資料查詢，不觸發 PRTG 請求或既有四項校準重算；sensor 資料就緒與頻道／單位語意確認分開呈現。
- **磁碟語意驗證**：`api/prtg/disk-verification` 支援單顆及最多 5 顆批次啟動、查詢、取消及人工確認。批次依序逐顆執行，取消會停止目前項目並略過後續項目。`requestId` 去重限目前行程記憶體，最多 512 筆、15 分鐘，重啟即失。啟動只抽查昨天有限歷史值，和已落地數值比較；結果持久保存。只有明確百分比可用空間語意可自動通過；候選不足或 `Unit=null` 不給自動綠燈。人工確認要求最近有效比對候選、相同頻道內容、人工理由並寫入稽核。驗證只檢查有限資料的頻道／單位／值，不代表真實趨勢效果已驗證；本身不啟用規則、不派工。
- **指定主機回填**：歷史補值可選主機與日期區間，先預覽資料量與估計請求數，再開始、查詢狀態或停止；有範圍、日期、請求數及互斥守門。局部 run 可安全取消自己啟動的工作。只補寫數值，不追溯建立舊日 finding 或交辦；程式／替身測試已驗，正式站操作待驗。
- **磁碟趨勢草稿試算與啟用**：規則頁提供停用草稿唯讀試算，窗口 1–730 個已完成日，依每批約 10 萬個預期歷史點分批（約 35 日 × 100 顆或 730 日 × 5 顆）；每頁列「完成日 × sensor」配對。`UnsuppressedHitRowCount` 是本頁未抑制命中配對列數；結果分頁，一列也可能涉及多位負責人，因此此數不是派工量上界或交辦張數。試算不含問題靜音與實際派工，也不寫業務資料。合法單顆 Probe 窗口可超過 35 日，並支援 ready-hit 正向結果。正式啟用 API 與儲存路徑都檢查至少一顆有 28 日有效資料且語意證據有效的 sensor。規則預設停用，門檻仍暫定；尚未以真實 PRTG 28 天資料或現場頻道語意驗證。
- **使用效果**：`GET api/prtg/effectiveness` 預設查最近 30 天，可指定最多 366 天；分開統計 PRTG finding、僅含 PRTG TopIssue 的主機日、案件、交辦、已回覆、已抑制、期間靜音區間有重疊的 PRTG 問題檔案及跨來源佐證主機日。「僅含 PRTG TopIssue」不表示完全沒有其他較低優先級日誌；靜音數是問題檔案數，不表示期間發生 finding。各指標口徑不同，不構成轉換率或解決率；資料缺漏時以不可計算呈現。

系統設定「系統健康」面板 `GET api/health/detail` 的 PRTG 摘要以最近兩個完整小時診斷持久嘗試／寫入計數及已落盤 quality／coverage，與 PRTG 維護頁最近 24 小時檢視共用基於 `PeriodStart` 索引的有限 SQL 聚合。診斷沒有逐 sensor identity，不能宣稱 sensor 層 100% coverage；已知目標數為 0 時顯示無目標，不能當成健康或故障；`Unknown` 代表證據不足，不能解讀為沒有目標。健康摘要空白時補為 `unknown`「尚無完整小時診斷資料」。資料庫探測失敗時 HealthService 會 early-return 最小 Down DTO，定向測試涵蓋此路徑，正式站台故障情境仍待驗。摘要也不等於磁碟 sensor 已通過 28 日 readiness 或實機語意驗證。

正式站台磁碟趨勢驗收入口為「PRTG 維護 > 環境探測」的準備度及語意驗證、「指定主機補值」的範圍回填，以及規則頁的草稿試算。Readiness 摘要只在第 1 頁提供且最多抽樣 100 顆，不得外推成全站比例。實測匯出目前僅約 6 天且單位為 null；真實 PRTG 28 天資料、主要頻道／單位語意及預警效果皆尚未驗證，不可將單元／整合測試或程式解析成功標為實機校準完成。細項檢核見 [PRTG-SPEC.md §9a](PRTG-SPEC.md#9a-磁碟可用空間趨勢-disk_free_trend)。

### 9.9f 校準數值匯出（PRTG 維護頁「環境探測」頁籤，`Maintain`）

這是 §9.9e 第四個頁籤裡的一張卡，不是獨立頁面——內容與 PRTG 高度相關，
獨立一個選單項只會讓側欄變長。`/admin/calibration` 路由**保留為永久轉址**到
`/admin/prtg#probe`，舊書籤與操作說明書的連結不致 404。API 與授權不變。

回答一個問題：**下一輪要設計 PRTG 值型規則與校準幾個保守門檻，資料量夠了嗎？**
夠了就一次匯出成文字檔交給開發端分析。**本頁沒有任何可儲存的設定欄位**——
校準門檻是程式常數，尚未校準的門檻再開設定是套娃。

- 四張卡對應四個校準項：PRTG 值型基線、PRTG 規則門檻、數值取得量級、殘留判定門檻。
  卡片只顯示 `prtg-calibration.js` 標籤表列出的 KeyMetrics 鍵，後端新增鍵要同步加標籤；比例值以 0～1 顯示，標籤註明「（0～1）」。卡片 DOM id 與 `cardKey`（`triggered-fetch-magnitude`）是內部識別名，不隨標題改名。
  每張顯示**目前累積量**、**狀態徽章**（不足／可用／充足／無法取得）、
  **門檻現值**、**補充說明**。說明一律帶實際數字（目前值與所需值，例如「還需要約 N 天」），
  不寫空泛的「資料不足」。
- 判定是重查詢，**按「重新計算」才跑**，頁面載入不自動執行。
- 分母為零一律判「不足」——「什麼都沒量到」不得被當成「量到而且全部達標」。
- 匯出閘門：四項全部達「可用」以上才解鎖；未達標需勾「仍要匯出」覆寫，
  覆寫照樣寫稽核且 detail 標記。下載走 **fetch + blob**，
  不用 `location.assign`（後端擲驗證例外時使用者會看到整頁 JSON，錯誤訊息就消失了）。
- 匯出檔為自描述 JSON（UTF-8 無 BOM），含四項判定摘要、四個資料集與三個摘要資料集（docs/PRTG-SPEC.md §11）。
  匯出區有「**只匯出摘要**」勾選框（預設不勾），勾選時匯出網址加 `&detail=summary`，網址只在一處組；匯出鈕前方顯示大小提示「完整匯出約含 N 列逐日資料、約 M MB」，M 以每列約 300 位元組估算，只在繪製卡片時設一次。
  **殘留判定資料集只輸出統計形狀，不含任何帳號名稱。**
  值型基線是 per-sensor **每日聚合**，需要原始 hourly 時走 PRTG 維護頁的資料搬運。

API：`GET api/admin/calibration/status`、`GET api/admin/calibration/export`
（`override=true` 為覆寫匯出、`detail=summary` 只匯出摘要）。門檻定義與資料集內容見 docs/PRTG-SPEC.md §11。
...
- **互斥判斷跨卡，不只看自己那張卡**。AI 卡兩顆啟動鈕的隱藏條件是三個因子的 OR：
  ...
  2. **取數排程執行中**——此時待補會被 AI 的資料完整性閘門擋住，補跑只是空跑一輪，
     而「強制重新分析」更會在當日資料還沒到齊時整批重標重跑。後端 `POST ai-run` 同步擋下回 **409**；
  3. **AI 服務未設定**（`IWebAiService.Available`）——按了什麼都不會發生。後端回 **400**。
  兩顆都被藏起來時，按鈕列內顯示一行說明取代它們（未設定優先，其次才是取數執行中），
  說明**放在按鈕列容器內**：那一列若整個變空，三卡等高會塌掉。
  說明文字：未設定時指路系統設定頁；取數執行中時講明「AI 會自動跟隨取數進度判讀，結束後可手動補跑」。
- **取數狀態未知時不隱藏**（狀態尚未取得或該支請求失敗）。隱藏表達的是「暫時互斥」，
  未知就藏會讓人以為功能不見了，後端的 409 才是最後防線。
  這與下一條的 PRTG 三態**方向刻意相反**，不要把兩者統一。
- **PRTG 啟用旗標是三態，未知視同未啟用**：頁面啟動時狀態輪詢與設定查詢並行發出，
  設定回來之前旗標是「未知」。此時同步與回填兩顆鈕必須維持 `disabled`、點擊時的第二道檢查也要擋下
  ——這裡擋的是「功能未啟用」，放行會送出一個註定失敗的請求。判斷一律寫成「只認明確的啟用」，
  全檔每一個判斷點都要一致（包含輪詢寫入點與點擊時的第二道檢查）。
- **停止鈕防連點**：取數與 AI 的停止鈕送出期間停用（同頁其他動作鈕的既有慣例），
  否則第二下會拿到「目前沒有正在執行的分析」的錯誤紅字。
- **強制重新分析的 modal 防護**：後端在 AI 已執行中時會先優雅停止當前執行再整批重標重跑
  （順序與理由見 `AiAnalysisHostedService.TriggerRunAsync`）。modal 開著時狀態輪詢不會關掉它，
  因此 modal 開啟期間偵測到 AI 由「非執行中」變成「執行中」就**停用確認鈕**並說明要關閉後重新確認；
  modal 內另顯示「目前由〈觸發者〉觸發的 AI 執行將被停止後重新開始」——要中斷誰的執行，使用者得先知道。
...
- **資源守門暫停中**：`status.pausedReason` 有值時狀態卡顯示短徽章，含 sensor 與數值的說明
  出現在同卡的「最新訊息」列與執行詳情（機制與取捨見 docs/PRTG-SPEC.md §12 可觀測性）。
- **執行紀錄分欄**：列表與執行詳情各有「本機／NetIQ／PRTG」三欄
  （各為「分析 n（失敗 m）」與 PRTG 狀態徽章＋sensor 數＋觸發主機數），
...
- **排程設定卡**：頁頂新增——
  ...
  反射守衛測試斷言欄位全數傳遞）、
  **送出前的 PRTG 提示**：`PrtgEnabled` 為 false **且連線已設定**（位址＋依認證方式檢查 token／帳密／passhash
  的「已設定」旗標，與後端 `PrtgClientFactory.HasUsableCredentials` 同義）時，先跳確認框
...
- **執行進度條**：狀態卡在執行中顯示**三條**進度軌——
  ...
  **待補積壓總數**、最後訊息），執行歷程以 `JobType=ai` 的 BatchRun 進執行紀錄逐筆視角。
  ...
  取數側由 `SchedulerRunState.LatestActivity()` 單點決定（netiq 主進度優先、本機次之、PRTG 最後）；
...
`BatchRunRecorder` 掛的 NLog target 註冊的是**全行程 Warn 以上**規則。夜間批次一跑就是數小時，
這期間站台仍在服務——不過濾的話，使用者開頁面觸發的慢 SQL、儀表板互動 AI 的逾時、
AI 分析排程的失敗，全都會被記進「這趟取數執行」的執行詳情，並讓它被判成「有警告」。

因此 recorder 建立時推入 `ScopeContext.PushProperty("lf_run_id", runId)`，
target 的 `Write` **只收 scope 內 runId 相符的事件**，其餘丟棄
（完整診斷仍在 `logs\logforesight.log`）。同一行程同時有取數與 AI 兩個 recorder 時各認各的，
target 名稱也帶 runId 以免互相移除。

> **`ScopeContext` 是 `AsyncLocal`：只沿非同步呼叫鏈往下傳，不會往上傳。**
> recorder 因此**必須在涵蓋整趟執行的方法本體內建構**（現況是 `AnalysisOrchestrator.RunAsync`）。
> 若搬進一個被 `await` 的輔助方法，scope 會在該方法返回時消失，
> 整趟的 Warn 全部靜默丟失且沒有任何訊號。已有測試釘住這條。

**慢 SQL 例外**：logger 名稱為 `SqlPerformanceMonitor` 的 Warn **照常寫入執行詳情**
（排查慢查詢仍需要），但**不遞增 `WarnCount`**——資料層效能告警與「這趟分析有沒有問題」
是兩件事。Error 以上不給豁免。判定用 logger 名稱，**不得用訊息內容比對**
（訊息是給人看的文字，改文案會讓豁免靜默失效）。

**AI 分析排程同樣走 recorder**（建構時帶 `jobType`），它的 Warn 因此進得了自己的執行紀錄。

**AI 契約失敗的錯誤訊息附回覆片段**：`ChatJsonAsync` 最終失敗時，訊息在「AI 回覆不是合法 JSON 或格式不符契約」
之後附上最後回覆的前 300 字（換行與連續空白摺成單一空白、超長截斷加「…」、空回覆寫「（空回覆）」）。
