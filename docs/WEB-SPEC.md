...
8. 日期區間提供快捷鈕：昨日／近 7 天／近 30 天／近 90 天（各頁選項略有不同），
   ...
   **期間終點一律錮在昨天**（分析只產到昨天，錮在今天會讓最後一天必然沒有資料），
...
- **展開列最下方的「原始訊息」區塊**：`rawText`（未截斷全文，只有升級後寫入的資料才有）
  ...
  彙總列標註它由多則合併、沒有單一原文。等寬字、保留換行、高度上限內可捲動；
...
- 取代原本分散在批次 appsettings.json（AI 位址）與程式碼寫死常數（未處理等級門檻、補充／留存天數）
  ...
  4. **資料保留**：六個天數設定的**下限一律 90 天、上限 3650**（`SystemSettings.MinRetentionDays`，
     ...
     **原始事件內容保留天數**（`RawEventRetentionDays`，預設 120，不可大於歷史保留，
     ...
     直接跳過寫入，見 `RiskyEventSelector.WithinRetention`），舊版的兩個分鍵已合併；
...
- **本機主機的「已回補」狀態**（§3）：立即執行回補會把缺漏日的分析紀錄補到
  ...
  `TrendWindowDays`＝14 脱鉤。文案講明：檢查最近 N 天內有沒有缺漏或需補跑的日子、
