...
6a. **全站執行中告示**：分析與站台跑在同一個行程，執行期間整站回應變慢——告示放在共用版型
   ...
   只有一頁看得到原因等於沒有配套。內容：進行中字樣、有分母時帶「第 N／M 單位」、觸發者、
   以及「畫面回應可能較慢，資料仍是完整的」。
...
- **指定主機更新鈕**：就近原則——看著這台主機覺得資料舊了當場按。開確認 modal（可選一次性回補天數
  ...
  **取數執行中時停用（不是隱藏）並在按鈕旁與 modal 內各說明一次原因**：按鈕消失會被當成
  ...
  使用者面對的仍是一顆沒有理由的灰按鈕。狀態訂閱 §8.6 的全站告示事件、只看取數那個旗標
  （AI 分析單獨在跑時這個動作是允許的），不自己輪詢。本機直讀主機走
