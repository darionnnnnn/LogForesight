...
日常 2~3 倍波動很常見，只有真正的爆量才該打破閘門；絕對量門檻（100）兜底基準很小
（例如 1~2 次）時單靠 10 倍會漏掉「3 次變 30 次」這種真正該被看見的暴增。

### Low 簽章的趨勢出口

未命中任何規則的簽章（`Severity=Low`）在趨勢層有且僅有兩條能被看見的路：

| 出口 | 判定 | 說明 |
|---|---|---|
| **瞬時爆量** | `TrendAnalyzer` 爆量例外（上方，回饋十六輪 B-1） | 單日次數達基準 10 倍或絕對量 100 筆 |
...
上方兩條出口是否都還在**，否則 Low 簽章會回到完全沒有趨勢出口的狀態（回饋十六輪體檢發現3／4）。

未命中規則事件的完整涵蓋總結（四種時間尺度）：

| 時間尺度 | 出口 |
|---|---|
| 首次出現 | 無（`New` 分支要求 Severity ≥ High，Low 簽章的首次出現永遠靜音） |
| 單日爆量 | `TrendAnalyzer` 爆量例外 |
