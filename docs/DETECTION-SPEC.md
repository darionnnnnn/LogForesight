...
| 層 | 負責 | 為什麼 |
|---|---|---|
| **規則層** (`KnownIssueCatalog`) | 比對已知危險事件（Source + Event ID + 次數門檻），確定性命中；規則命中的問題同時附帶靜態知識庫內容（白話說明／常見原因／處置步驟），不需要 AI 深入分析。PRTG 監控訊號的狀態變更規則同在規則庫（`prtg` 平台，docs/PRTG-SPEC.md §9） | 已知模式用規則抓，100% 召回、零成本，不賭小模型會不會漏看；同一 Event ID 的原因/處置幾乎不變，寫死比每次重新生成更快、更一致、零幻覺 |
...
| **關聯層** (`CorrelationAnalyzer`) | 比對「多個獨立事件的已知組合模式」：攻擊鏈、故障連鎖、跨日推進（見下方清單）；事件日誌與 PRTG 同日示警的跨來源佐證在 PRTG finding 追加時由 `PrtgCorroboration` 判定 | 單一事件各自不嚴重、組合起來卻是明確故事——這種跨 log 關聯判讀正是小模型最容易漏掉的，必須程式先比對好 |
...
- **4740（帳號鎖定）交叉**：被鎖帳號與當日殘留帳號有交集時降為 Medium 並填入判定依據。
  被鎖帳號集合讀簽章的結構化欄位 `KeyAccounts`（完整集合、封頂 200），不剖析 `KeyDetails`
  顯示字串——那裡只列前 5 個帳號，第 6 個以後永遠比對不到；`KeyAccounts` 為 null（舊資料）
  時才退回字串剖析。同理【暴力破解→RDP 得手】的來源 IP 交集讀 `KeyIps`
  （欄位定義見 docs/DB-SPEC.md 的 ContentJson 一節）
  （殘留重試必然反覆鎖定帳號）。
...
> （可能是入侵者提權得手後清除稽核紀錄）。這些事件都需要 Security log 讀取權限；
...
### 問題靜音與 `Suppressed` 的一致化

**靜音**是問題層級（一個 Source＋EventId、全機房）、有期限的噤聲，事實來源是問題負責與靜音的靜音區間
（docs/WEB-SPEC.md §9.4b、docs/RULES-SPEC.md「讓告警安靜的工具」）。分析側以**紀錄日**判定：紀錄日落在任一
區間的問題標 `Suppressed=true`，與規則抑制走同一條路（不拉抬日風險、不進趨勢與即時告警）。以紀錄日而非執行時間
判定，重新分析舊日子永遠同答案；靜音期間執行的重新分析不會把區間外的舊日子也標掉。

`Suppressed` 問題（抑制或靜音）在兩個出口一律排除：
- **AI 提示詞**（`AnalysisPromptBuilder`）：不進白話總覽與敘事，Windows、Linux、PRTG 同一條規則；
  當日只剩被抑制的問題時也不說「當日無事件」。
- **問題負責人郵件路由**（`MailNotificationService.ResolvePerRecipient`）：被抑制或靜音的問題不觸發負責人通知；
  同日另有未抑制問題命中時照常通知。

報告 txt 的「已抑制」段照常列出（證據層），反查原因時靜音顯示「靜音至 yyyy-MM-dd：原因」。
`lf_top_issues` 與紀錄 JSON 都不存靜音欄位——讀取側（清單、待辦、派工）由靜音區間當場推導。
...
### NetIQ 取數與 AI 判讀脫鉤（取數排程與 AI 服務）

**本機分析路徑同樣適用**：本機也只算統計段、標 `AiPending`，AI 判讀交給 AI 分析排程。
取數執行內因此沒有任何同步 AI 呼叫：行內跑 AI 會被 JSON 重試拖長本機進度軌，
...
2. **AI 分析**（`AiAnalysisHostedService`）：兩條觸發路徑。
   ...
   **背景輪詢**（60 秒）：在執行窗口內、存量校正回填已完成、且有待補資料時自動開跑，
   ...
   完整性閘門：取數排程執行中、且**本趟 PRTG finding 尚未全部算完**時，跳過本趟 PRTG 日期範圍內（到今天為止）的待補——
   夜間只有昨天，立即執行回望 N 天時是整段範圍
   （取數正在寫入的必然是最近日期；PRTG finding 會影響日風險與敘述，太早判讀等於讓 AI
   看缺了 PRTG 訊號的半份資料）。PRTG 路徑一發佈 finding（停用或評估失敗也算發佈）就全部合格，
   **AI 在 finding 發佈後即可處理當日待補**，不以整趟取數結束為前提。閘門擋住時排程作業頁的 AI 狀態卡會說明原因
   （見 docs/WEB-SPEC.md §9.10 閒置原因）。
