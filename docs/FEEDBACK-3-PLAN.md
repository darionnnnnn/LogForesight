...
5. #7 採 NuGet `OpenCC`；實作首步驗證 net8 相容與 `tw2` 轉換品質，不合格即停下回報，
   備案（OpenCCSharp 等）另議，不自行改道。
...
**步驟二：OpenCC 簡→繁（台灣用詞）**：
- 套件 NuGet `OpenCC`（doggy8088，純 C#、零相依、MIT）；
  `OpenCC.Converter("cn", "tw2")` 建 converter。
- converter 以 `static Lazy<>` 單例持有（建構含字典載入，不逐次建）；
  **驗證其執行緒安全**（與 #2 平行化並存；不安全就包 lock——轉換是純 CPU 短操作）。
...
**實作首步（閘門）**：加入套件後先寫 3 行 spike 確認 net8 可載入、`"cn"→"tw2"`
確為「簡體→台灣正體＋台灣慣用詞」（相當 OpenCC 標準 s2twp；驗證樣本：内存→記憶體
或至少 內存→內存不劣化、网络→網路、数据→資料、默认→預設）。不合格即停下回報（定案 5）。
