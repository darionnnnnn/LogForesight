...
併 dev 後全量測試：通過 3378、略過 6、總計 3384。重掃體檢修正 commit 的 diff
（候選列快取、歷史批次載入、跨日數填入）與現行文件敘事字眼：**終檢無新發現**。
一點備註（非缺陷）：候選列快取與判定摘要共用同一個 `_cachedAt`，匯出端強制重算摘要時
候選列若仍在 TTL 內會沿用——資料以「天」為單位變動，十分鐘內不會得到不同列。
