...
- **全量測試**：4288 通過、略過 6、失敗 0（基線 dev@45b14d7 為 4128，淨增 160）。
- **併回前終檢**（兩個獨立 scan-low：程式碼 diff、文件對實作）：
  - 已修：白話說明遇多條規則時未依定案 A9 取不限分類那條（改實作＋補測試，突變紅）；佐證模式被抑制過一次後取消抑制也補不回來（改冪等判斷＋補測試，突變紅）；
    DETECTION-SPEC 把「已於 PRTG 確認」寫成不拉日風險（實際仍拉「中」）；PRTG-SPEC 已確認只限 down、跨日窗口措辭、探測輸出分類欄、升級後行為差異清單（原本漏抄）、
    五層偵測表補 PRTG 來源、敘事字眼「改為／不再」、BACKLOG 規則數；`LfDbContext` 與 `CalibrationService` 兩處過時註解；CLAUDE.md 基線與文件地圖。
  - 記進 BACKLOG：主機清單 PRTG 提示每頁讀整張對應表；前端 `hosts.js` 另有一份「未回報」判定（本輪之前即存在）。
  - 判定不修、只交接：容量佐證的 `srv` 2013 判定寫在 `PrtgCorroboration` 內（未與 seed 規則共用）；沒有測試覆蓋「第二段中途取消後由 finally 補發」路徑（正常路徑的就緒訊號只送一次有測試）；
    WEB-SPEC §9.4 沒有主機詳情 PRTG 區塊的描述（本輪之前即缺，端點與顯示寫在 PRTG-SPEC §8）。
- **未實測**：B-4、D-3 的畫面沒在瀏覽器實際操作——本機開發站台唯一的預設帳號需輸入密碼登入，Claude 不代為輸入，需使用者實機確認
  規則頁「sensor 分類」下拉、PRTG 維護頁「sensor type 分類補充對照」、主機頁 PRTG 表（狀態徽章、device 名稱）、主機清單 PRTG 徽章、儀表板未回報卡提示。
...
併 dev（a8eb953）後全量 4293 通過、略過 6、失敗 0（實作輪 4288 ＋ 體檢補測 5）。體檢兩筆 commit 的 diff 由體檢方親讀，終檢無新發現。
...
| C-1 | impl-low（Opus 5 low） | 通過（860→874；Claude 修正後全套 4232 綠） | grep 驗收全過；執行端突變「改回單段迴圈」「長期門檻 13」「批次 1000」各 1 紅；Claude 突變「資料庫舊命中不排除」→ 1 紅、「長期 Down 不封頂嚴重度」→ 2 紅 | (1) 資料庫歷史查詢區間含本趟重評日期，重跑時上一趟寫入、這一趟已不成立的命中會被算成歷史——Claude 改為本趟有評估的日期只認本趟結果，補測試；(2) 長期 Down 只關重大旗標、嚴重度仍高，死 sensor 每天仍拉「中」，與定案「不再拉風險」不符——Claude 加嚴重度封頂「中」，補測試並把 pipeline 測試斷言從「非高」收緊為「維持低」；(3) `prtg-date-range` 逐日訊號改在第二段（發佈時）送，次數與參數不變 |
| C-2 | impl-low（Opus 5 low） | 通過（12→16；全套 4236 綠） | grep 驗收全過；執行端突變「事件列不排除 PRTG」「不排除已抑制」「上限 21」皆紅 | 無落差（工作樹換行曾被轉換，但 repo `core.autocrlf=true`、索引一律 LF，commit 內容不受影響；先前據 `git show` 的 LF 誤判為整檔改動，已更正） |
| D-1 | impl-low（Opus 5 low） | 通過（943→968；全套 4261 綠） | grep 驗收全過；執行端突變六處皆紅（其中「管線不產生關聯抑制集合」初次存活，補端到端測試後紅） | (1) `IAnalysisRecordStore.AttachPrtgFindings` 多一個 `out int corroboratedCount` 供執行輸出統計（規格外，介面只有一個實作）；(2) `PrtgFinding.SensorCategory` 以 init 屬性帶，簽章新增 `PrtgSensorCategory`（JSON 相容）；(3) 補充派工時加的登錄點：三個 Id 進 `CorrelationPatternIds.All`、紀錄詳情頁觸發條件說明；(4) 已於 PRTG 確認的 finding 仍參與佐證；佐證只在有新 finding 追加時判定，既有紀錄不回溯 |
| D-2 | impl-low（Opus 5 low） | 通過（110→126；全套 4277 綠） | grep 驗收全過；執行端突變七處皆紅（含「迴圈內多查一次」由 `SqlPerformanceMonitor` 量測次數抓到） | (1) 兩份「未回報」原寫法判定結果相同，儀表板寫死 `-2` 未引用常數，已收斂成 `HostAdminService.IsSilent`；(2) `HostAdminService` 建構子新增必要參數 `ISystemSettingsStore`（讀 PRTG 總開關），`DashboardService` 新增 `EfPrtgStore`，DI 已註冊不需改；(3) 儀表板的 PRTG 失聯台數在整包摘要快取內，PRTG 鏡像寫入不推進版本戳，最多落後一個 TTL——寫進文件，不另改快取 |
| B-4 | impl-low（Opus 5 low） | 通過（全套 4284 綠；B-4 相關 191 綠） | grep 驗收全過；`node --check` 驗收寫法不適用 ES module（改前版本同樣報錯），改以 module 模式檢查三支皆過；執行端突變七處皆紅 | (1) 主機頁狀態欄改用 `lf-badge`（原為 Bootstrap `badge text-bg-*`），外觀會略有不同；頁內新增同名 `badge` 小函式，與主機清單、使用者詳情頁各一份的既有慣例一致；(2) 狀態文字一律顯示 PRTG 原字串；(3) device 名稱以新方法 `GetDeviceNamesByObjids` 一次查回，不讀整張裝置表；(4) **畫面未實測**：本機開發站台唯一的預設帳號需輸入密碼登入，Claude 不代為輸入，改以字串測試與端點測試驗收，需使用者實機確認 |
| D-3 | impl-low（Opus 5 low） | 通過（UiTests 103→105；全套 4286 綠） | grep 與 module 語法檢查皆過；執行端突變兩處皆紅 | PRTG 徽章放在停用／已併入判斷之外，依賴後端「只對啟用中的未回報主機給值」（`IsSilent` 要求 `Active`，已核對）；畫面未實測，同 B-4 |
