...
> 規劃日期：2026-08-04。狀態：**七項全部完成並通過全案體檢**（實作紀錄與規劃偏離見文末章節，
> 1324 測試綠）。使用者實測回饋第七輪版本
...
## 實作紀錄與規劃偏離（2026-08-04，七項全部完成）

依規劃順序 7→3→2→1→5→6→4 逐項落地，每項一個 commit。全數通過建置（0 警告 0 錯誤）
與全套測試（**1324 綠**，較基準 1290 淨增 34 個——涵蓋連線池字串處理、登記失敗回報、
進度計數併發、依問題視角嚴重度過濾、使用者名稱素材、觀察狀態機／日推導／逾期判定／
案件同步與掛接繼承／90 天邊界／歷程後綴／ExternalOf 收斂）。

### 與規劃的偏離（皆為刻意決定，理由如下）

1. **#4 沒有另開 `ObserveUntil` 欄位，沿用 `IssueHandling`／`IssueCase.DueDate` 當「觀察至」**
   ——語意是同一件事的推廣（「這個狀態何時需要重新處理」），且既有 DTO／案件同步／批次
   掛接本來就無條件傳遞 DueDate，多數消費端（處理人工作頁逾期紅字、`AttachNewDay` 繼承）
   **零改動自動接上**，改動範圍大幅縮小。到期後解讀從「該完成了」變成「該回頭看了」，
   單點定義 `IssueHandlingStatuses.IsObservationActive/IsObservationExpired`。
2. **#4 處理人工作頁未另立「觀察中」分組**——觀察中案件本來就出現在既有的進行中案件清單
   （`GetOpenByHandler` 回傳所有未結案案件），狀態欄顯示「觀察中」＋觀察至日期、到期轉
   逾期紅字；清單依「逾期→最新」排序，觀察到期的案件自然浮上來。另立分組是同一份資料的
   重複呈現，不做。
3. **#4 `ExternalOf` 實際上不需要功能性改動**——日層級推導（`DayHandlingDerivation.Derive`）
   已把 observing 映成 `in_progress`，`ExternalOf` 收不到 observing。仍補了防禦性映射
   （observing→in_progress）：沒有它的話任何未來漏進來的 observing 會掉進 default 被誤歸
   「已處理」，觀察中的日子就從待辦統計消失。
4. **#1 骨架列深色模式檢查不適用**——全站沒有深色模式（site.css 無 `prefers-color-scheme`
   ／theme 切換），骨架列 shimmer 動畫本身正常。聊天泡泡的三點跳動（`.lf-typing`）是
   對話情境的既有專屬樣式，不納入三出口收斂。
5. **#6 NetIQ 維護頁的更新者維持帳號**——`NetiqOptions` 是直接回傳的 Core 儲存模型，
   歷史上曾為它做過一份「零加值複本」DTO 並刻意移除（見 NetiqDtos.cs 註解），
   為單一顯示欄位重造整份 DTO 不值得。
6. **#6 的收斂範圍以本文件的逐點盤點清單為準**——負責人欄（主機清單徽章／主機詳情／
   處理面板唯讀顯示）、處理人欄（問題清單／處理面板目前狀態／歷程「處理人：○○○」）、
   案件敘事句（「由 ○○○ 處理」「已指派給 ○○○」）與權限異動確認人等**名稱清單與敘事句**
   維持現狀：這些點的 DTO 只帶名稱（或帳號）單值，全面套用需要再擴 6+ 個 DTO，且敘事句裡
   逐一附帳號會讓句子變得冗長。若使用者實測後希望全面套用，屬下一輪的明確擴充項。

### 已知限制（誠實申報）

- **#4 觀察的跨日繼承依附於進行中案件**：`AttachNewDay` 只掛進行中案件（規劃如此設計），
  案件狀態為 observing 時新產生的風險日自動繼承觀察狀態。**沒有案件時**（未經指派、
  使用者直接在某日把問題標為觀察中）標記只涵蓋該日——該問題明天再發生會作為新的未處理日
  現身。實務動線：需要跨日觀察時先由 admin 指派建案（或既有案件內操作）再標觀察即可。
  若要「無案件也跨日觀察」需要類似已知雜訊記憶的主機×簽章記憶機制，屬另輪擴充，
  本輪不做（避免同一問題出現第三套跨日協調機制）。

### 全案體檢（獨立 agent 覆核全部變更後逐項比對規劃）揪出並修正

- `SchedulerRunState.EndRun` 未把進度欄位歸零（與註解承諾不符；前端只在 isRunning 時畫
  進度條所以看不出來，但欄位不該留髒資料）→ 已補歸零＋測試。
- 觀察至日期的 90 天上限只在前端 `input max`——伺服器端補上同一範圍驗證＋邊界測試
  （恰 90 天合法、91 天拒絕）。
- 規劃明列但首輪漏做、體檢補上：`HandlerWorkloadDto` 補 `Account`（#6 工作頁標題）、
  進度條的「x / y 主機日」可見文字（#2，原本只在 tooltip）、本機跳過訊息的
  Milestone 落地（#3，NLog target 只收 Warn 以上，不記 Milestone 執行詳情查不到）、
  歷程 Note 自動補「（觀察至 yyyy-MM-dd）」（#4，`ComposeLogNote` 單點供三個歷程
  寫入點共用——歷程列沒有 DueDate 欄位）。

### 驗證方式與已知限制

驗證計畫第 1 項（build＋全測試）完成；第 2、3 項在本機 Sqlite 開發環境實測通過
（連線池關閉生效、狀態卡進度條／3 秒輪詢／停止流程、掃描精靈 spinner）。
第 4~6 項因**本機測試環境沒有真實問題資料**，改以自動化測試等價覆蓋（含 #4 完整劇本
「標觀察→儀表板不吵→到期→逾期現身」的整合測試），瀏覽器逐項點擊驗證留待使用者
實測時完成——這是誠實申報的驗證缺口，不是遺漏。
