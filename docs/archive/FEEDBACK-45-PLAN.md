...
- **四個坑**：
  ...
  - 移除類依賴方：A6 移除儀表板告示的依賴方盤點**當時做錯了**：實際上 `chat-panel.js` 與 `help-manual.js` 也各自
    打過一次 `/api/run-activity`。兩者用途不同（面板內一行「AI 回應會變慢」的提示、刻意只查一次不輪詢），
    本輪維持現狀不收斂；A8 `aiAvailable` 收斂的五個呼叫端全列於 A-12。
...
| A4 前端缺陷與收斂 | impl-low | 全綠 3999（+13） | 六條契約各有結構斷言（先擷取函式主體再斷言，不用整檔比對）；Claude 另做一次突變（AI 判讀鎖鈕退回 finally）轉紅 | 執行模式那個未定義變數採「直接移除」——先查過 modal 內十二個 id 確認沒有對應的天數包裝元素，規格禁止憑名字猜測新增 DOM |
| B1 執行紀錄下推 | impl-low | 全綠 4017（+18） | 讀取量以 SQL 攔截器計數；續號損毀、附加時間早於 cutoff 的邊界各有測試；Claude 另做兩次突變皆轉紅 | 首版 `GetLogs` 在窄化為空時退回全撈，Claude 驗收時推翻：診斷行的業務時間與附加時間同源（同一次寫入設定），退路唯一會觸發的是「零診斷行」這個最常見情況，等於把全撈變成常態路徑。移除退路並補一條前提測試 |
| B2 記錄查詢慢路徑 | impl-low | 全綠 4036（+19） | 不藏老案子（200 天前仍未處理）、三種「下一筆」語意、跨使用者不互相命中各有測試；Claude 另做兩次突變皆轉紅 | 兩處偏離採納：90 天常數在專案內無具名來源可共用（主機詳情是行內字面值且該檔不在白名單），改在服務層定義具名常數並註明兩邊要一起調；捷徑沿用 200 筆掃描上限以維持與改版前相同語意。收緊的代價：依問題視角展開受影響主機日時，若使用者手動清空日期欄位，範圍會收成近 90 天 |
| B3 設定快取與失效白名單 | impl-low | 全綠 4052（+16） | 內容只讀一次、副本反例、白名單反例（處理狀態寫入必須推進）各有測試；Claude 另做一次突變（命中時共用同一個物件）轉紅 | 白名單比規劃更保守：規劃列的「顯示偏好設定」與「認證換發」在程式碼中沒有對應的非 GET 端點，執行端查證後不列入，實際只放四個 |
| B4 可見範圍快取 | impl-low | 全綠 4062（+10） | 洩漏守門（兩使用者交錯）、失效守門（版本戳推進）、未注入時行為不變各有測試；Claude 另做一次突變（TTL 永不過期）轉紅 | TTL 與上限由規劃的 60 秒／256 改為 30 秒／64，理由是專案內同族快取全部是這個數字，自創數字會製造唯一的異類。採納 |
...
- **新增的環境相依**：前端行為測試需要 node，偵測不到時**略過**而非失敗。在沒有 node 的機器上
  總數會少 5 題並多一筆略過（xUnit 把整個 theory 收斂成一筆略過），不是紅。
- **尚未實機驗證**：全部前端改動只有結構斷言與 node 行為測試，沒有瀏覽器實測——
  尤其全站告示的版面（執行中會在頁首與主內容之間多一段間距）、五頁新增的載入指示、
  慢查詢清單掛在資料保留面板的視覺位置。
...
2. `AiCacheStore` 開啟 collection 快取 → 本輪不開（內容大、寫入頻繁、效益不明）。
3. `LatestOccurrences` 的來源名稱下推 SQL → 移出本輪（兩個資料庫後端的大小寫轉換語意不同，
