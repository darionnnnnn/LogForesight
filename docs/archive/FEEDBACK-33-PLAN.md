...
## 終檢輪（合併 dev 後，體檢方同模型執行）

- **體檢修正自身的殘餘反例**：`Write` 的認領查詢沒有排序——「id 相符列＋0 列」意外並存時
  （登記失敗夜的降級寫入可造成），`FirstOrDefault` 取到 0 列並認領成 id 相符，會與既存列撞
  唯一索引讓寫入炸掉。補上與 `Read` 對稱的 `OrderByDescending(HostId)`，殘留的 0 列交由保留期清理。
- 文件稽核：本輪 diff 引入的敘事字眼僅一句升級路徑必要脈絡；「報告年月分層」殘留僅在本 PLAN
  （隨歸檔離開現行文件）。其餘無新發現。

## 狀態：全案完成（含體檢輪＋終檢輪），已併 dev，待使用者實測後併 master
