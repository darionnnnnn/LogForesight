...
`<summary>` 為區塊標題——比照同頁 `ai-advanced` 的既有寫法。

> **實作時推翻了「執行中也不自動展開」這條**：按下探測後若重新整理頁面，進度與輸出
> 會被收在摺疊區裡，看起來像什麼都沒發生。改為 `isRunning` 時自動展開（有歷史輸出
> 但未執行中仍維持收合）。
...
委派模型：`gemini-3.7-flash-high`（整輪未切換）。基準 dev@c17f626（3148 綠）。

| 作業-階段 | 執行者 | 結果 | 驗收 | 落差與處置 |
|---|---|---|---|---|
| A-1：設定欄位與驗證 | agy | 通過 | 3161 綠（+13） | 無落差。「新存或既有皆算有」抽成共用的 `HasEffectiveSecret`，六處都補齊；順手收斂了 AI 金鑰的同型判定（等價替換，無行為變化） |
...
高嚴重度先自己讀程式碼驗證再改。實際修正：

| 嚴重度 | 問題 | 處置 |
|---|---|---|
| 高 | **密碼錯誤會連打 getpasshash**（Claude 自己審 passhash 失敗路徑時抓到）：每個 sensor 的數值擷取各自呼叫一次 `GetJsonAsync`，密碼打錯時三千個 sensor 就是三千次登入失敗，足以觸發 PRTG 端帳號鎖定 | 憑證層級失敗黏住（`_credentialFailure`＋`FailCredentials`），傳輸類失敗仍可重試；加測試斷言連續五次呼叫只打一次 getpasshash |
...
**查證後不成立、未採納**：終檢把「工作目錄有未 commit 的憑證黏住修改」列為擋路級——
那是它讀 HEAD 狀態時我正在修的東西，不是缺陷。

### 已知形狀（未處理，登記備查）

- `PrtgClient` 建構子的 `authMode`／`username`／`password` 是有預設值的選用參數。目前正式碼
