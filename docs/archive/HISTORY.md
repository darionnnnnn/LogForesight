...
> **決策 20 修訂**（2026-07-31 定案、2026-08-04 執行完成，docs/WEB-SCHEDULER-PLAN.md）：
> one-shot＋工作排程器模型推翻，改為 Web 行程內常駐 `BackgroundService` 排程引擎——
> 排程時間、手動觸發、進度可視化全部搬進 Web，批次 console 專案不再是分析執行的
> 唯一途徑。Phase 2~4（服務搬遷、排程引擎與 UI、CLI 職責搬 Web）已於 2026-07-31
> 實作完成；Phase 5（console 專案自解決方案移除）原定需 ≥5 晚實際環境驗證通過
> 才執行（定案 #5），**2026-08-04（回饋第七輪）由使用者知情豁免該閘門提前執行**，
> `LogForesight`（console）專案已自解決方案與 Git 移除。接受的殘餘風險：移除後
> Web 排程若有問題只能修、不能靠重啟 schtasks 熱回退（冷回退＝git revert＋重建
> 部署）；緩解是分析冪等——缺漏日靠既有回補機制自動補齊，不會永久缺資料。
> 通知管道仍維持原案（Web 頁面顯示，未前移為主動推播），這部分決策不變。
