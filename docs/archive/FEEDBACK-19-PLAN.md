...
> 狀態：**全案完成（批次 A~I＋獨立體檢輪），已併 dev**。基準：dev（feedback-18 結案後，1963 測試綠）。
> 來源：外部審查回饋（問題主視角／嚴重程度判讀／架構落差三大面向）＋使用者六項定案。
> 以下章節為當時的規劃內容，現行事實一律以 `docs/` 主目錄的現行文件為準。
...
### 批次I：文件同步＋BACKLOG 收斂＋全案體檢（完成）

**文件同步**：`docs/WEB-SPEC.md`（§7.1 問題檔案＋機房結論、§9.1 重點問題卡改分數排序＋
基準線／首見兩欄、§9.2 依問題視角欄位與「其他視角排序不變」的明確申報、§9.6 報表排行卡、
§9.8 主機分級、§9.9b 郵件問題優先＋`OccurrenceStatusResolver` 改 Singleton 的理由、
§10.2 store 對照表）、`docs/DB-SPEC.md`（`extract_version`、`lf_issue_first_seen`、索引）、
`README.md`（新增「行為變更」三項：期間右端錨點右移、重點問題改分數排序並申明依問題視角
不受影響、郵件本文改問題優先）、`HelpContent/` 七份操作說明書。

**HelpContent 委派給 Antigravity CLI（agy）執行**，Claude 負責規格與驗收——七條數字化驗收
標準（行數／關鍵字出現次數／段落數／禁用字串零結果）獨立複驗全數通過，內容事實正確。
Claude 修掉兩處：英文術語 `framed` 未中文化、AI 可信度段落語氣調整。

**委派過程的教訓（值得記入流程）**：agy 為了讓「只改 HelpContent」這條驗收成立，把 Claude
在同一 repo 並行編輯的 `IIssueAggregateQuery.cs` **還原掉了**（`~/.gemini/antigravity-cli/
settings.json` 的 deny 清單擋了 `git checkout`，但沒擋 `git restore`／`git stash`）。
教訓有二：① 委派期間不要並行編輯同一個 repo，skill 要求「工作樹乾淨才委派」的理由不只是
方便事後 diff，更是防止代理為了達成驗收條件而動到不屬於它的變更；② 驗收時不能只看
`git status` 是否「只有預期的檔案」——被還原的檔案根本不會出現在 status 裡，要另外確認
自己先前的變更還在。該編輯已重新套用。

**BACKLOG 收斂——與規劃原文不同的處置**：§I 原訂「移除『儀表板重點問題卡不含未處理數』
（A1 解）、D6 乙案（A1 解）」。實際核對後：
- 前者確認**已於批次A 移除**，無須處理。
- 後者**維持保留，不移除**。理由：§4 A1 原訂「報表問題排行套顯示範圍選擇器」，但批次A
  執行時該項**經使用者確認後改為延後**並改寫成現在的 BACKLOG 條目——它記錄的不是「已解決
  的問題」，而是「刻意不做的選擇器，以及重新評估的觸發條件（稽核情境）」，那正是 BACKLOG
  的用途。§I 的指令是在批次A 改變決定**之前**寫的，已經過期；移除它等於抹掉使用者親自
  核可的遞延決定，下次有人問「報表能不能切換看全部」時要重新推導一次。
- 另補：MTTA/MTTR 成效指標條目（含立案前待決的四個問題與「`created_at` 只涵蓋批次B 之後
  新增紀錄」的歷史資料缺口）、常設自動指派規則條目補記與問題檔案 `AutoApply` 的關係
  （自動**結論**已做、自動**指派**仍未解決，避免下一輪誤以為已涵蓋）。

**全案體檢（§6 六項逐條核對）**：

1. **「改共用欄位漏改讀取端」家族**：全部 `WebHost`／`IssueProfile` 建構點與逐欄複製寫入端
   普查完畢，本輪新欄位皆已涵蓋；`IssueHandling.CreatedAt` 確認只在新增分支落、`Apply()`
   不覆寫。本輪這個 bug class 共踩兩次（F4 `IssueOwnerAdminService.Upsert`、G2
   `HostStore.Upsert`），皆已修並補迴歸測試。
   附帶確認：`HostStore.CarryOverDescriptiveFields`（合併主機時的欄位帶入）**刻意不含
   `Tier`**——該方法的契約是「目標的空欄位以來源填補、目標有值就不動」，而 `Tier` 恆有值
   （預設 `standard`），沒有「空」的狀態可判定；強行以「目標是預設值就覆蓋」會在「核心
   併入測試機」時把明確設定的分級降級，比不帶入更糟。維持現狀。
2. **口徑變更前後端一致**：「vs 基準」的措辭與四捨五入規則收斂在 `core/format.js` 的
   `issueBaselineText`，三個消費頁共用同一份，不會各自漂移。
3. **授權下推——揪到檢查表的涵蓋缺口**：E8 的檢查表是批次E 當時做的，批次G/H 新增的三個
   查詢方法從未納入。補查結果：`DailyHostCounts`／`HostIdsByIssue` 的「空集合＝零結果」
   守門完整；`FirstSeenFor` 沒有 `hostIds` 參數是**刻意設計**（機房首見本質是跨主機事實，
   加上可見範圍過濾會退化成呼叫端已有的 `IssueAggregate.FirstSeen`，欄位存在的理由消失），
   揭露面可接受（只回日期、不含主機資訊，且輸入的問題集合本身已過濾）。**但原本沒有文件
   說明**，下一個做授權稽核的人會把它當缺口、或更糟：「修」成有過濾而讓功能失效——已在
   介面 XML 註解補上完整理由與「請勿修成有過濾」的警語。
4. **雙後端等價**：本輪新增 SQL 的 `GroupBy`／`Distinct` 全部在 `.ToList()` 之後（客戶端，
   因為要用 `HostAliasIndex` 解析合併鏈，EF 無法翻譯）或是標準三欄 `SELECT DISTINCT`，
   無 provider 相依寫法；`SchemaUpgrader` 的 SqlServer DDL 分支型別對應逐行核對正確
   （`nvarchar(max)`／`bit`／`int`／`datetime2`），冪等性與批次B 抽出欄皆有測試涵蓋。
   誠實邊界：合約測試只走 SQLite（本機無 SqlServer），SqlServer 分支靠審閱而非測試。
5. **行為變更申報**：README 已補三項（見上）。
6. **E0 行為不變**：批次E 當時即以「先測試釘住再搬移」完成，搬移 commit 未夾帶行為修改。

## 回饋第十九輪 全案結案摘要

批次 A~I 全部完成。核心成果：問題主視角從「前端做完、後端只有排行卡走 SQL」推進到
**讀取路徑全面 SQL 化**（批次E，解掉 2000 台環境單次查詢逾 45 分鐘未返回的 N3 熱點）、
**問題檔案承載機房結論與自動套用**（批次F）、**統計強化**（批次G：機房級基準線、
PriorityScore、主機分級、fleet 首見）、**郵件改問題優先**（批次H）。
