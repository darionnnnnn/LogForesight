...
- 首見日回填——終檢查證後確認**根本不需要**：`UpsertFirstSeen` 只往前不往後，重跑舊日以該舊日期建立/更新首見日，不會被誤判成新問題（原「接受視為新問題」的說法已被推翻，見批次 C 定案「首見日」條）。
...
- **run-preview 的重跑預估與確認框預估數**（批次 C 改動 3／批次 D.1 後半，實作中降級不做）：預估需逐主機掃既有紀錄＋處理狀態，3000 台規模下預覽查詢成本不成比例。確認框只含模式與天數；已寫進 WEB-SPEC §10.9 當現況、BACKLOG 記觸發條件。
- **執行監控頁的重跑專屬進度列**（批次 D.3 前半）：申報走執行詳情 Milestone，不另做進度列（同上，BACKLOG 記觸發條件）。
- **「郵件不重寄」的專屬測試**（批次 C 驗收末條）：重跑不清已寄鍵這件事沒有對應的新程式碼路徑（本來就沒有任何清除點），「斷言寄送清單為空」測的是不存在的行為，不寫；已寄鍵機制本身有既有測試覆蓋。
...
實作模型：Claude Opus 5（規劃＋批次 A／終檢自做，批次 B／C1／C2／D 委派 agy gemini-3.7-flash-high）。
實作完成時 2941 綠。實作方自報最沒把握處：重跑與缺日聯集後的逐日迴圈語意、四個模式在真實瀏覽器的互動未實測。

## 體檢輪修正（Fable 5 獨立體檢）

1. **趨勢基準被重跑日的舊紀錄污染（真 bug，前兩輪審查皆漏）**：`BuildStatisticalRecordAsync` 的
   `ReadRecent(targetDate, historyDays)` 窗口含錨定日，其安全前提（程式碼註解自述）是「當下這一天的
   紀錄還沒寫入」——重跑日打破此前提（舊列到寫入前才刪），舊版「今天」混進 `TrendAnalyzer`／
   `SlowTrendAnalyzer`／`ResidualCredentialDetector` 的基準：事件在舊列出現過就不再算新問題、量值墊高
   基準、暖身計數多算一天。修法：統計段讀出 history 後無條件 `RemoveAll(同日)`（正常缺日分析是 no-op，
   合併主機重複列情境反而多一層保護），一處修好本機與 NetIQ 兩條路徑。迴歸測試
   `重跑日的舊紀錄不混進趨勢基準`（以暖身計數 1/13 vs 2/13 當觀測點），突變（拿掉過濾行）轉紅確認有效。
   AI 段 prompt 的 history 屬既有行為（NetIQ 兩階段本就先寫統計列）不動。
2. **五個檔案被實作輪加上原本沒有的 UTF-8 BOM**（EfAnalysisRecordStore／AnalysisOrchestrator／
   LogAnalysisService／ScheduleDtos／SchedulerHostedService，dev 基準皆無 BOM）：實作方的 python
   `utf-8-sig` 寫入一律加 BOM，且後續比對是對著已污染的中間 commit 比、看不出來。已全部剝除對齊 dev。
3. **規劃比對缺口補記**：run-preview 預估／監控頁進度列／郵件測試三項降級未做，補進「明確不做」；
   「明確不做」的首見日條目與批次 C 定案互相矛盾（前者仍寫已被推翻的舊說法），已修正；
   批次 C「過濾查詢須批次化」實作為「每主機一次 GetMany」而非跨主機一次（WEB-SPEC 已如實記載，
   2000 台＝2000 次範圍查詢，量級可接受、留作觀察）。
4. BACKLOG「重新分析舊日期會讓首見日偏晚」半句被本輪查證推翻，已更正；CLAUDE.md 測試基線 2886→2942。
