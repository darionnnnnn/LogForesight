...
> 體檢對象：`origin/dev..dev`，重點是終檢後的手改 commit `eff42d4`。

**無擋路級或高嚴重度發現**——手改的資料請求黏住經獨立查證是正確的（雙層黏住不互相干擾、
鎖內檢查在併發下仍 load-bearing 並非死碼、併發首波的失敗上限受 `PrtgFetchConcurrency` 限制、
取消路徑不會被誤判成 401）。定案 1~7 全部落地、明確不做無偷做、跨段四條鏈全通、
文件對照表九格逐格相符。修掉三個低度：

| 嚴重度 | 問題 | 處置 |
|---|---|---|
| 低 | 新前置守衛讓兩條既有黏住測試改走前置路徑——`EnsurePasshashAsync` 鎖內那道防線（併發首波唯一的保護）失去突變覆蓋：把它刪掉單執行緒測試照綠，併發首波會各打一次 getpasshash | 補「併發首波帳密錯誤時 getpasshash 仍只打一次」測試，並以突變（移除兩道鎖內外檢查）確認會紅 |
| 低 | `PRTG_username在password與passhash模式間切換時不被清空` 名稱與強化後的內容不符 | 改名為 `..._兩模式都能寫入且token模式不動它` |
| 低 | `WEB-SPEC` 通知段殘留一整段「體檢輪修正」過程敘事（非本輪造成的既有殘留） | 改寫為純現況陳述 |

未處理（記錄備查）：`EnsurePasshashAsync` 的「passhash 模式走到換取邏輯＝程式錯誤」防禦性
throw 嚴格不可達（建構期必預填快取）——成本為零、意圖清楚，留著。
