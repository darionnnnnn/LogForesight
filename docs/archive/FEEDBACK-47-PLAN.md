...
### 15.5 B-3b 實測抓到的三個缺口（2026-09-18）

**缺口一：依問題視角的「N 個問題靜音中」在篩選下高估**
- 事實：`RecordListQueryService.SearchByIssue` 在後段篩選（來源、事件 ID、類別、問題嚴重度 chip、嚴重度門檻）**之前**呼叫 `CountCurrentlyMutedIssues`；篩單一問題時仍說「另有 2 個」。儀表板與報表沒有後段篩選，不受影響。
- **定案 50**：聚合介面新增 `CurrentlyMutedIssues`，回傳目前靜音中、期間內有列的問題（來源、事件 ID、類別、期間最高嚴重度），SQL 與 `CountCurrentlyMutedIssues` 共用同一個查詢建構；依問題視角把後段篩選抽成**單一述詞**，同時套在主清單與靜音清單，註腳數＝通過述詞的靜音問題數。儀表板與報表維持計數方法。

**缺口二：依問題視角交辦沒有範圍與續掛，也不帶頁面篩選**
- 事實：C-3d 規格把 modal 定為 `scopeKind=Hosts`、`autoAttach=false`、`hostIds=null`、`groupIds=null`（規格階段的決定，未記進設計修正）。結果介面上沒有任何路徑建出會續掛的單（夜間建的單本來就不續掛），定案 6 與劇本 S2「勾續掛」落空；頁面上的主機與主機群組篩選也沒傳給解析，交辦的主機可能多於畫面列出的。
- **定案 51**：
  - modal 帶頁面篩選：`hostIds`＝已選主機、`groupIds`＝已選主機群組。
  - 範圍由篩選推導、不另設選單：有選主機→`Hosts`；否則有選主機群組→`Groups`（那些群組）；都沒有→`All`。modal 顯示一行「範圍：全站／主機群組 A、B／指定主機」。
  - 「續掛新主機」勾選：範圍不是 `Hosts` 才顯示，預設勾；說明「之後這個範圍內新出現此問題、還沒有人處理的主機，夜間自動加入這張單」。在預覽清單**手動排除過主機**時停用並取消勾選，說明「已手動排除主機時不提供續掛（被排除的主機之後再出現會被自動加回）」。
  - 夜間 ④ 同一問題有多張可續掛的單時（群組分攤會產生），改為**負載最輕者**（進行中成員數→單數），同分才取建立最晚者；原規則「取建立最晚者」會把新主機全掛給同一人。
- 不做：範圍下拉（範圍＝目前篩選，與依問題視角其他動作同一個心智模型）。

**缺口三：交辦單詳情頁範圍欄比對錯字**
- 事實：`work-order-detail.js` 以小寫 `'groups'` 比對，模型值是 `Groups`／`All`／`Hosts`；全站與群組範圍都顯示「指定主機」。目前只因所有單都是 `Hosts` 才沒被看見。
- **定案 52**：範圍欄依三值顯示「全站」／群組名稱／「指定主機」，旁邊加「續掛：開／關」。

| 段 | 內容 | 白名單重點 |
|---|---|---|
| G-2a | 定案 50 | `IIssueAggregateQuery`／`EfIssueAggregateQuery`／測試替身／`RecordListQueryService`／測試 |
| G-2b | 定案 51、52 | `records.js`／`work-order-detail.js`／`WorkOrderDispatcher`／`WorkOrderDispatcherTests` |
...
| B-3a | impl-low | 一輪通過並完整回報（前端守門 68 綠、全套 4776 綠／略過 10） | Claude 核對三檔白名單、位元組核對；**瀏覽器實測**：依問題視角「靜音」鈕（`Maintain` 才出現）、modal 三句提示／六顆時長／原因必填／既有單區塊顯示「1 張進行中交辦單、共 20 台」與「代為結案」選項（本人具 Assign＋Handle）、送出後 toast「已靜音『Service Control Manager (7000)』至 2026-09-24」、該問題自清單消失、交辦單 #2 轉 `paused`＋`mutedUntil`、處理人摘要 `pausedWorkOrders=1` | 規格寫的 `api.del` 不存在，實際為 `api.delete`（執行端以程式碼為準並申報）。**Claude 實測抓到既有缺陷（非本段造成）**：`/api/records/by-issue` 控制器宣告回傳 `ApiResponse<PagedResult<IssueGroupDto>>`，服務實際回子型別 `IssueSearchResultDto` → JSON 依宣告型別序列化，`distinctHostCount`（R20 起）與 `mutedIssueCount`（B-2b）**靜默被吃掉**；前端 `records.js:476` 有 `Number.isInteger` 守衛，所以「共 N 台主機（去重）」一直沒出現也沒報錯，服務層測試看不到。Claude 改宣告型別並加序列化守門測試（斷言宣告型別＋序列化後兩個欄位都在），修正後實測「共 9 個問題，共 60 台主機（去重）」與 `mutedIssueCount=2` 都正常 |
| G-1 | impl-low | 一輪通過並完整回報（目標 64 綠、全套 4781 綠／略過 10） | Claude 核對七檔白名單、位元組核對 0 異常；**突變**拿掉群組交集→3 個群組測試轉紅，還原位元組一致；**瀏覽器實測**：群組下拉列 6 個啟用群組、池群組帶「（派工池）」，「維運一部」篩出 3 張、「系統管理」1 張，加總等於全部 4 張；依問題視角處理人連結為 `/work-orders?source=&eventId=&handlerId=`，點下去只列該人該問題 1 張、橫幅「篩選：DistributedCOM 10016｜處理人 愛麗絲(scale-alice)」；查無時橫幅「處理人 #5」、網址 `groupId` 預選下拉 | 網址只帶 `handlerId` 時橫幅省略「｜」；網址 `groupId` 不在選項（已停用）時忽略、不帶查詢。首屏會先等群組選項再載入清單 |
| B-3b | impl-low | 一輪通過並完整回報（前端守門 68 綠、全套 4781 綠／略過 10） | Claude 核對 11 檔白名單＋1 新檔、位元組核對 0 異常；**瀏覽器實測**：`/work-orders#muted` 直接進「靜音中（2）」頁籤，兩列依到期排序、第二個問題「1 張」暫停單、延長／解除鈕；依問題視角「共 38 個問題，共 60 台主機（去重）；另有 2 個問題靜音中（未列出）」且連到 `#muted`；儀表板「另有 24 個問題已有結論、2 個靜音中（未列入）」；報表問題排行「共 15 個問題；另有 23 個問題已有結論、2 個靜音中（未列入）」；規則頁告警抑制頁籤與設定頁自動派工段的決策表四列＋說明書連結；風險日詳情「靜音至 2026-09-24」徽章與原因／設定者 tooltip | Claude 親修兩處：①執行端把依問題視角註腳限在查出筆數 > 0，期間內問題全被靜音時畫面只剩空清單、看不出原因 → 零筆時也顯示「另有 N 個問題靜音中（未列出）」；②決策表「不再打擾」寫成「主機詳情頁的問題列」不準確 → 改為「風險日詳情頁把問題標為不處理、誤報或已知雜訊」。**實測另抓到三個缺口，開 G-2**（見 15.5）：依問題視角篩單一問題時註腳仍說「2 個」——靜音計數在來源／事件／類別／嚴重度後段篩選之前算；交辦 modal 固定 `scopeKind=Hosts`、`autoAttach=false`，也不帶頁面的主機與群組篩選；詳情頁範圍欄以小寫 `groups` 比對，全站與群組範圍都會顯示成「指定主機」 |
| G-2a | impl-low | 一輪通過並完整回報（目標 278 綠、全套 4785 綠／略過 10） | Claude 核對白名單七檔、位元組核對；**突變**讓靜音數不套後段篩選→2 條新測試轉紅，還原位元組一致 | 規格寫 `Aggregate` 類別取 `Min`，實際是「最近一天的類別」（`LatestCategories`，`Min` 只當回退）→ 執行端抽出共用 `PickLatestCategory` 照實際口徑做，沿用 `Aggregate` 同段的 `??` 回退寫法（非新語意，接受）。Claude 親修一處：新方法插在 `MapRiskLevelToSeverities` 的文件註解與方法本體之間，註解錯掛到新方法 → 歸位。留意：靜音清單多查一趟（量＝靜音問題數 × 天數）；新方法未做 SQL Server 翻譯測試（寫法同 `Aggregate`） |
| G-2b | impl-low | 一輪通過並完整回報（目標 77 綠、全套 4788 綠／略過 10） | Claude 核對白名單四檔、位元組核對；**突變**把續掛挑單改回「取建立最晚者」→ 2 條新測試轉紅，還原位元組一致；**瀏覽器實測**：依問題視角篩主機群組「機房-001」後開交辦 modal，顯示「範圍：主機群組：機房-001」與預設勾選的「續掛新主機」；取消勾選一台主機 → 續掛停用並取消勾選，勾回後恢復可用且回到勾選；實際送出建出 #5（`Groups`、`autoAttach=true`、60 台，toast「已建立 1 張、併入 0 張，共 60 台」）；詳情頁 #5 範圍欄「機房-001（續掛：開）」、舊單 #1「指定主機」 | 範圍區塊放在「交辦給」之後、說明欄之前（規格寫的「衝突處理之前」中間還有說明與期限）。續掛挑單同分時不沿用「本趟已選中者」，照規格以負載與建立時間決定 |
| 文件 | Claude 親寫 | 已提交 | 說明書相關測試 42 綠 | WEB-SPEC §7.1／§8.6a／§9.1～9.4a／新 §9.4b／§9.6～9.9c／§10～11；DB-SPEC 移除 `lf_user_host_map`、補交辦單兩表與案件新欄位、部分唯一索引理由；RULES-SPEC 決策表與 NoiseMark 是派工閘門；DETECTION-SPEC 靜音與 `Suppressed` 一致化；BACKLOG 移除三條已完成、新增本輪遞延；CLAUDE.md 文件地圖與三條「不要做」；說明書改寫處理章與新增「讓告警安靜的工具」章（`#alert-tools` 錨點） | 核對時抓到 AI 版說明書有不存在的 `/my-tasks` 路徑與舊四層優先序，整份重寫 |
| 收尾清理 | Claude | 已提交 | Sentinel 相關 65 綠；突變拿掉手動觸發→轉紅 | Sentinel 多段預算測試改注入 `TimeProvider`（根因：全套負載下執行緒池飢餓讓 1 秒計時器回呼晚於 1.2 秒睡眠，剩餘空網段先跑完）；清除 17 處 `task-47-*` 註解標記；程式碼引用「規劃 15.3」與「定案 48～52」改指向歸檔路徑 | D-2b 驗收 grep 過嚴一事屬規格指令問題，程式碼不改 |
| 收尾體檢 | scan-low ×4（授權／正確性／前端／資料層） | 見下列修正 | — | 各自獨立閱讀 diff，不給 Claude 結論 | **確定且已修**：①交辦單回填器與「刪除指定主機日」（第 31 輪既有）自開交易沒包執行策略，SQL Server 啟用重試時必擲例外 → 包進策略＋守門測試（突變驗證）；②處理歷程批次續號取「最後一筆」，SQL Server 批次插入 seq 不保證照清單順序 → 取回看窗 100 行最大值＋測試；③依問題視角「已交辦」分子是全站數字（洩漏看不到的台數、出現 300／10）→ H-1；④派工 ⓪ 只看紀錄日，今天剛靜音仍為昨天的紀錄與立即派工建單 → H-1；⑤總覽清單舊回應蓋新篩選 → 請求序號；⑥靜音 modal 單數取頁內筆數 → 取總數；⑦代為結案會觸發不再打擾沒講 → H-1 提示。**端到端劇本**另抓到 S7「篩處理人已停用」沒有入口 → 補勾選框。**不成立**：取消交辦把案件開著時使用者標的非結案日子改回 open（那些日子就是案件狀態）；靜音頁籤張數連結不帶 paused（問題靜音中時其單全為暫停）。**記 BACKLOG**：夜間派工與 Web 單操作並行競態、多單寫入無整體交易、SQLite `upper()` 只轉 ASCII、回填與協調層並行、待派試跑未模擬 ①③ |
| H-1 | impl-low | 實作通過，一條白名單外既有測試依新口徑轉紅並停下回報 | 突變拿掉「目前靜音中」→ 2 條新測試轉紅 | Claude 核對 18 檔、位元組 0 異常 | 已交辦改由同一批出現點計算並移除無用的 store 方法與建構參數（10 個建構點）；`DispatchContext.MuteToday`；代為結案提示 |
| H-2a | agy（gemini-3.8-flash-high） | 一輪通過並完整回報（相關 130 綠，+6） | Claude 核對 diff 只動白名單六檔、位元組核對；突變讓復發旗標恆為 false → 2 條轉紅，還原一致 | 補劇本 S9 的「復發」字樣：⑥ 延續性偏好命中＝復發，旗標經夜間彙總帶到摘要信該單那一行。第二次比對規劃時找到的落差 |
| H-2b | agy（gemini-3.8-flash-high） | 改動正確，但**跑測試前就結束**（stdout 停在「正在執行測試驗收指令」＋ terminating background task） | Claude 自跑驗收（grep 兩條、前端守門 24 綠）；**瀏覽器實測**總覽「進行中」出現「最近新增」欄且五張單都有值 | 補 C-2 清單列契約的「最近新增」欄。第二次比對規劃時找到的落差 |
| H-1b | agy（gemini-3.8-flash-high；使用者指示本段起改回 agy） | 一輪通過並完整回報（IssueMuteTests 35 綠，+1） | Claude 核對 diff 只動白名單兩檔、位元組核對；突變拿掉「目前靜音中」→ 3 條轉紅，還原一致；**全套 4795 綠／略過 10，總 4805** | 舊測試改以不落在任何區間的「現在」建脈絡保留原意，新增今天在區間內的反例；`IssueProfile.Mutes` 註解改為分析側只看紀錄日、讀取側與派工另算目前靜音中 |
...
- **全量測試**：4807 綠／略過 10（總 4817），全綠（見下方體檢輪）。與本輪起點 4293（略過 6）相比 +514；多出的 4 條略過是本輪壓測 `WorkOrderScaleBenchmarks`（`LF_SCALE_BENCH=1` 才跑）。
- **壓測（本機 SQLite）**：4000 台 × 10 天交辦預覽 0.83 秒、建單 2.2 秒；4000 × 60 背景逐日展開 91 秒（修正前 53 分鐘）；750 台回覆 336 ms；2000 台待派試跑 3.8 秒；4000 主機日夜間掛接含自動派工 126 秒、不派工 20 秒（差距為實際寫入上萬件案件的成本，約 7 ms／台；原目標「差 < 20%」口徑不成立）。
- **比對規劃後補做**：定案 48～52（群組篩選、chip 連單、靜音數套篩選、交辦範圍與續掛、詳情頁範圍欄）。
- **未竟事項**：全部記在 `docs/BACKLOG.md`「回饋四十七輪遞延」；SQL Server 端的執行策略與批次續號只能以程式碼審閱與守門測試保證，正式機升級後要看一次啟動回填 log（「解析 N／建單 M／成員 K／未連結 J」）。
- **待使用者實測**：劇本 S1～S9，特別是 S4／S9 夜間自動派工（demo 資料庫無法觸發真實夜間分析）與 D-2b 逐日同步告示（需大量成員才進背景）。


## 體檢輪修正（換模型：實作 Opus 5／agy，體檢 Fable 5.1；subagent 為 Opus low × 4）

四個唯讀面向（Web 服務層邏輯、前端三大頁狀態組合＋探針、收尾手改與過度設計、文件與程式碼事實核對），明列先前已查過的項目不重查。Fable 親做規劃逐條比對與修不修的取捨。

**已修（行為錯）**
- `WorkOrderCommandService` 預覽「將交辦 N 台」把他人處理中、未勾改派的主機算進去（落盤 0 台）→ 改以實際寫入的成員計；補斷言於 `衝突_單人不改派_…`，突變轉紅。
- 建單期限沒有驗證，可建出一建立就逾期的單 → 與日層級指派同一條「不可早於今天」；`驗證_期限早於今天_零寫入`。
- 續掛與手動排除互斥只在前端 → 服務層同規則（排除過主機就不續掛）；`續掛_手動排除過主機時不開`／`續掛_全站範圍且未排除時照請求開啟`，突變轉紅。
- 改派給現任處理人：協調器 no-op 卻寄「交辦」＋「已移交」兩封給同一人並寫稽核 → 服務層拒絕；`改派_給現任處理人_拒絕且不寄信不寫稽核`。
- 群組依負載分攤把會被略過的衝突成員算進虛擬負載，後續主機被推給別人 → 只計實際寫入的成員。
- 併入既有單時說明與期限不覆蓋，建單信卻寫這次填的值 → 信件改讀單上實際的值。
- 夜間採用別人剛建的單（處理人換了）時復發旗標照帶，會對錯的人說「你修好過」→ 處理人相同才保留。
- 回填器的執行策略取自立刻 Dispose 的 context（SQL Server 重試時第一次執行會回頭讀它）→ probe 活到迴圈結束。
- 交辦總覽與詳情頁「已結案」徽章提示直接顯示 `all_closed` 等代碼 → `format.js` 單一對照函式。
- 詳情頁代為結案／取消後 `reloadAll` 不清勾選，「拆出」可送出已看不見的成員 → 重新載入時清空；取消交辦補 `withBusy` 與失敗處理（原本失敗是未處理的 rejection）。
- 詳情頁成員清單、處理人頁交辦單清單沒有舊回應防護 → 請求序號（同總覽）。
- 詳情頁複製 CSV 以請求的 200 判斷最後一頁 → 改用後端回應的 `pageSize`。
- 交辦 modal：使用者手動取消的「續掛」在排除後又取消排除時被強制勾回 → 記住使用者自己的選擇；交辦對象與改派選單不列停用帳號與停用群組（後端必定拒絕）；送出失敗重複兩個錯誤 toast、被區域變數遮蔽的頁碼指派 → 移除。
- 處理人頁勾「只看暫停的單」時仍顯示「另有 N 張暫停」→ 勾選時不顯示。
- 趟末派工摘要只寫 `Log.Info`（不進執行紀錄）且略過原因是代碼；自動派工開著但派工池沒人時沒有任何訊號（規劃 A-3「夜間記一則 warn」漏做）→ `NightlyDispatch.DescribeSummary` 中文一行寫進執行紀錄＋空池警告；兩條測試。
- 守門測試會把註解裡的 `CreateExecutionStrategy()` 算成一筆 → 先去掉註解行。
- 註解與行為不符兩處（續號讀取成本、`AppendLines` 的 seq 順序）→ 改寫。

**文件**：WEB-SPEC 夜間掛接的類別歸屬（①②③ 在 `AttachNewDay`、⓪④⑤⑥ 在 `WorkOrderDispatcher.Decide`）、執行紀錄摘要文字、週報靜音段欄位（目前靜音中全部問題；7 天內到期的提醒在體檢報告）、狀態選項「有逾期」、通知不涵蓋日層級指派與立即派工、當天設定當天解除不留恢復紀錄；決策表畫面用詞「全站」統一；說明書 13 章補交辦單／續掛／派工池／待派／靜音五個名詞並修「第一位負責人」舊說法，09／13／14 章與 04 AI 版章名更新，02 章補「我的交辦單」卡，05 章「立即派工」標明需系統維護能力。

**記 BACKLOG**：詳情頁處理人回覆入口（`viewerIsHandler` 已備）、靜音歷程畫面（`muteHistory` 已回傳）、日層級指派與立即派工的建單通知、真實 DNS 計時測試偶發（Sentinel 那條已修，BACKLOG 舊條目改寫為這條）。

**不修（判斷後不成立或屬取捨）**：`NotifyEscalationAsync` 不受 `MailNotifyWorkOrders` 管（上報是既有的獨立事件信，比交辦單通知早存在）；`BulkCloseIssue` 操作者恰為處理人時 `TouchReply`（他確實在處理這些主機）；預估主機日不扣排除（標示為「期間內」的估算）；`CountCurrentlyMutedIssues` 與 `CurrentlyMutedIssues` 並存（共用同一段篩選，儀表板與報表不需要逐列）；`records.js` 與詳情頁各自把範圍翻成中文（前者描述「這次交辦的範圍」、後者描述「單上記錄的範圍」，語境不同）；④ 續掛不標復發（續掛不是延續性選人）。

**全量測試**：4807 綠／略過 10（總 4817）。全套一次跑中 `PrtgAddressResolverTests` 的真實 DNS 計時測試偶發失敗，單獨重跑 3/3 綠，本輪未碰該檔，已記 BACKLOG。

## 終檢輪

體檢修正直接進 dev（實作方已併 dev）。修正後重跑全套、瀏覽器實測詳情頁取消交辦（toast 與中文結案原因）、交辦 modal（範圍行、停用帳號不列）、處理人頁暫停註腳；主控台無錯誤。終檢無新發現。
