...
S1～S6 全數完成，測試 2086 → 2168 全綠。以下只記**與原規劃不同的決策**與**驗收所得**，
逐段的機制細節見程式碼註解與 `docs/DB-SPEC.md`。

**體檢時發現 S4 曾被整段漏做**——第一版完成紀錄寫「全數完成」時，儀表板仍在
`QueryLightweight`、群組風險仍用主機名稱建索引，原始審查的 P2 第 6 項與 P3「群組合併」
兩項其實還開著。它從進度追蹤表消失在 S1 之後、S2 之前，之後每一輪的「剩下的」清單都沒有它。
教訓：完成紀錄要對回**規劃的階段清單**逐項打勾，不能對回自己的記憶。
...
| **S4 儀表板下推繞過第二層可見範圍**（委派產出） | 未注入 `ISystemSettingsService`，`riskLevels` 從未計算——「只顯示高風險」設定在儀表板整個失效，症狀是設定不生效而不是報錯。同一段還手組 `Todo` 漏 `TotalCount`、直呼 `AggregateDayTodo` 傳空 `excludedHostIds` 讓墓碑待辦靜默消失 |
| **既有 `AggregateByHost` 兩處逐列物化**（回饋十九輪遺留） | 紀錄含 `Headline` 與整張 `lf_top_issues` 都拉回記憶體才分組，3000 台 × 366 天是 110 萬列。而 S3-a1 我又讓 agy 建了一支九成重疊的 `AggregateReportHostRisk`——規格漏查既有機制。體檢時合併成一支真正 SQL 端聚合，報表與儀表板共用 |
| 兩處呼叫端修改 `GetAll()` 取得的快取物件 | `Upsert` 未提交甚至失敗時，變更已對所有讀取者可見，而 `Active` 影響主機可見性 |

### 委派品質（agy）

十三段委派、一輪回饋（S3-a2 實作正確但測試一支未加）。另外一次刪掉 13 支既有測試
（含與任務無關的 `HasOverdueIssue`），已還原並把新測試移到獨立檔案。
**「既有測試一支不少」＋「新增 ≥ N」兩條分開的數字化驗收是唯一抓到後者的機制**
（淨總數抓不到「刪 3 加 4」那種）。S4 那段規格明寫的四件事它漏做三件（注入設定服務、
用 `GetTodoByRange`、六支測試），由 Claude 直接補完——規格越長、越後面的項目越容易被丟。
