...
體檢方：Claude Fable 5.1（實作方：Claude Opus 5＋impl-low）。三個 Opus low 子代理分掃「獵 bug（附探針實測）／架構契合／文件稽核」，取捨與修正由體檢方親做。

| # | 哪裡 | 症狀 | 怎麼修 | 迴歸測試 |
|---|---|---|---|---|
| 1 | `PrtgFetchService` 階段 2 清除＋`EfPrtgStore.DeleteSensorsNotSyncedSince` | **已實測重現**：逐台模式某台「查詢成功但回空陣列」時，該台鏡像感測器全被刪、整趟 0 failures；PRTG 偶發回空會讓當晚對那台主機的規則評估無聲失效 | 回 0 顆的裝置給一趟寬限：36 小時內刷新過的列本趟保留並出聲，下一趟仍 0 顆才清（無狀態的兩次才刪，真的移除全部感測器的裝置最多多留一趟） | `感測器同步_單台回零顆時該台舊感測器保留一趟_上次刷新已過寬限才清`（突變紅） |
| 2 | 同上 | 感測器清除沒有保險：主機主檔暫時讀到空清單 → 對應全落空 → 範圍只剩守門裝置（非空）→ 整份鏡像被清 | 前置條件加「範圍內至少一台對應成功（`Mapped > 0`）」，不成立時不清並出聲 | `感測器同步_範圍內沒有對應成功的裝置時不清除`（突變紅） |
| 3 | `FetchDayAsync` 的 `syncStructure:false` 提早返回 | 夜間沿用手動同步的鏡像、而範圍為空（鏡像無感測器）時，被回填專用的提早返回攔下：計失敗、印「無法回填」、狀態變更階段沒跑 | 提早返回只在 `fetchValues` 為 true（回填）時成立 | `不同步結構且不取數值時_鏡像沒有感測器不提早返回…`（突變紅） |
| 4 | `PrtgSnapshotHostedService.FetchFilteredAsync` | 同類風險只修一半：覆寫清單查 parentid 有「PRTG 忽略 filter 回整站」的防護，快照分批沒有——忽略時每批都回整站，同一顆一輪被累加幾十次、缺顆警告也不會響 | 分批模式每批只收該批要求的 objid（全站模式收目標集合），同一個解析入口多一個 accept 參數 | `分批快照_PRTG忽略filter回整站時_每顆只累積一次`（突變紅） |
| 5 | `BackfillScopeSensorsAsync` | 覆寫清單的感測器補抓後仍不在鏡像（所在裝置回 0 顆）時，每輪重查 parentid、重補同一台，永不收斂 | 補抓零失敗後仍不在鏡像的 objid 記入「查不到」集合（清空時機不變） | 既有 `覆寫補抓_查不到的objid下一輪不再查…` 涵蓋集合行為；本條未另加 |
| 6 | 註解 | `FetchDayAsync` 參數說明仍寫「狀態變更的縮圈在後續實作」；`EfPrtgStore.Prune` 兩處「結構鏡像永遠是現況全量」；「本段」「暫定值」過程用語；夜間 provider 註解稱「擷取整個擲例外時 provider 可能沒被呼叫」與實情不符 | 逐條改為現況敘述 | — |
| 7 | 文件 | DB-SPEC §保留期仍寫 devices／sensors「永遠保持現況全量，不清」；WEB-SPEC §9.10 進度軌仍寫三階段都是列數／treesize、回填進度寫「約 T 筆」；PRTG-SPEC 三處「縮圈後」過程敘事、§5 殘留 42,393、未回報提示用字與畫面不符（實為「PRTG：無資料」）；BACKLOG 一條缺觸發條件；README 直接寫內部代號 9d-5 | 逐條修正 | — |

判斷後**不修**的（記錄理由）：
- 逐物件併發骨架兩份（感測器／狀態變更）：同構但各自的每台狀態與結果形狀不同，抽共用要多一層泛型回呼，只有兩個呼叫者，不抽。
- `PrtgStateChangeRangeResult.Pages／StoppedEarly` 無正式消費端：回填輸出與既有測試在用，逐台後語意是加總／任一台，保留。
- 對應失敗時仍照範圍清除（設計修正 2 已定案）：`MapForDate` 最後才一次 `ReplaceHostMapForDate`，失敗時當日對應沒寫、範圍取自前一次成功的對應；加上修正 2 的保險，不另擋。
- 手動同步「收斂但 0 台裝置」顯示成功、無錯誤訊息：改動前即如此，非本輪引入。
- 守門等待時佔著併發名額：與階段 4 既有寫法一致。
- 分批快照部分批次失敗仍算成功：規劃 C 定案 3 的決定。
- 非觸發式回填逐日重查狀態變更（**已實測**：天數 × 台數）：正式路徑走不到，進 BACKLOG。
- 範圍補抓每輪 `GetAllSensors()`：鏡像已縮到範圍內，量小，進 BACKLOG。
- 「估算與校準顆數相等」「IP 排除不在範圍」未寫成測試：前者兩處同源查詢，後者 IP 排除的裝置根本不產生對應列（與 unmatched 同路徑）。

體檢後全套：**4908 綠／略過 10（總計 4918）**，基線 4807 → +101。

## 終檢輪

併 dev 後全量 4908 綠／略過 10。體檢修正 commit（b1e0e24）逐檔複看：寬限查詢在兩後端皆為單純 IN＋日期條件、快照解析入口兩個呼叫端都帶 accept、文件與程式一致。終檢無新發現。
