...
### B2 設定頁統計 UI — agy
- 行為契約：設定頁 AI 頁籤新增統計區：今日消耗、累計消耗（含起算日）、清空重算按鈕（confirm 後打 API）、單價輸入與估算金額（前端即時算，單價存後端）、展開 30 天表格。
- **實作時兩處與契約字面不同，理由如下**：
  1. 表格欄位定為「日期／呼叫次數／prompt／completion／總計」五欄，**沒有「AI 整理件數」**——
     計量掛在 `AIService` 這個 HTTP 出口，它看得到呼叫次數，看不到對應幾件 AI 整理
     （一件可能因重試多次呼叫、也可能命中快取零次呼叫）。要做得讓計量點從一處變成多處，已記進 BACKLOG。
  2. **單價沒有獨立 API**，跟著既有 `PUT /api/admin/settings` 整頁表單儲存——
     另開一支等於在同一頁做出兩套儲存語意（設定頁刻意是整頁單一 form）。
...
### F1 全主機表載入收斂 — Claude（契約值依實測修正）
- 原契約寫「至多 1 次」，實測基準是 **13 次**（不是規劃猜的 3 次），且主機清單本身早有版本
  探測快取——真正的重複工是 `EfIssueAggregateQuery` 每個聚合方法各自重建一次主機別名索引。
- **定案上限＝4 次**（實作後實測值，寫進測試常數當回歸護欄，只能往下不能往上）：
  索引重建收斂為一次之後，剩下的四次來自可見性服務、處理狀態彙總的主機索引與索引首次建立，
  分屬不同服務邊界，硬要收斂成一次得把主機快照穿過好幾層 API，代價大於效益（已記進 BACKLOG）。
- 驗收：F0 計數測試斷言載入次數不超過上限；行為測試等值；別名索引快取的失效測試。
