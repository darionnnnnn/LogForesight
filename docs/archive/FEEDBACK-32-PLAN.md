...
- **實作模型**：Claude Opus 5
- **體檢模型**：Claude Opus 5（**與實作模型相同**——收尾流程原則上要求換模型，本輪經使用者明確指示「就用同一個模型體檢」。獨立性因此較弱，這一點在採信本輪體檢結論時要納入考量）。（本輪規格、逐段驗收、手改、文件皆由它產出；實作委派 `agy` gemini-3.7-flash-high）。
- **測試**：交接時 2985 通過／略過 6／總計 **2991**；體檢輪補 2 條迴歸測試後為 2987／6／**2993**（基準 dev@a57d770 為 2942，淨增 51）。
...
對 `dev..feature/feedback-32` 全 diff 重新檢視（規劃比對／獵 bug／終檢後手改／架構契合／測試）的結果。

### 1. 死碼：`catch (AggregateException)` 確認不可能觸發，已移除

- **哪裡**：`NetiqDirectoryClient.ListHostsAsync`，包住 `Parallel.ForEachAsync` 的內層 try/catch。
- **症狀**：交接節列為「疑似防禦死碼」。實驗確認：`Parallel.ForEachAsync` 失敗時
  `await` 重擲的是**第一個內層例外本身**，不是 `AggregateException`，因此該 catch 永遠不會進去。
- **怎麼修**：移除該 catch 與它唯一存在理由的內層 try（連同 `using System.Runtime.ExceptionServices`）。
- **迴歸測試**：新增「多分段中單一分段硬失敗_擲出可顯示的NetiqDiscoveryException」——
  4 個 /26 分段、併發 3，讓其中一段的查詢被 Sentinel 回 400，斷言呼叫端收到的是可顯示的
  `NetiqDiscoveryException`（而非 `AggregateException` 或原始 `SentinelClientException`）。
  這條在移除死碼**之前**先加並確認通過，移除後仍通過——這才證明它是死碼而不是有人在擋。

### 2. 真 bug：分段中途被預算中斷時，該段已產生的涵蓋警告會整個消失

- **哪裡**：同一個方法的分段工作內部。
- **症狀**：警告先寫進區域變數 `segmentWarnings`，跑完兩段掃描才 `AddRange` 進 `outcome.Warnings`。
  段中途因預算用盡擲出例外時，這個搬移永遠不會執行——**「NOT 排除語法在這台 Sentinel 上未生效」
  「主機數超過排除上限、清單可能不完整」這類最該讓管理者看到的診斷會靜靜消失**。
  平行化之前不會發生：舊寫法是所有段共用一份 `warnings`，寫進去就在了。
  這是平行化重構帶進來的新問題，不是既有行為。
- **怎麼修**：警告直接寫進該段的 `outcome.Warnings`（每段只有一條執行緒碰它，不需要鎖）；
  合併時**警告一律保留**，主機清單與「掃了但沒掃乾淨」才只採計跑完的段
  （沒跑完的段會被列進「未掃描的網段」，同時又貢獻半套主機清單會自相矛盾）。
- **迴歸測試**：新增「多分段警告依分段原順序合併且標注正確分段」——讓第 2、4 段觸發
  collector 警告並讓第 2 段跑得比較慢，斷言警告內容標注正確分段、且輸出順序是分段原順序
  而非完成順序。**「段中途被中斷後警告仍保留」這條路徑沒有單元測試**（要同時湊出
  「已產生警告」與「隨後預算用盡」很難在替身上構造），修法本身是結構性的：警告不再有搬移步驟。

### 3. 過度設計：`openWizard` 裡重複的 `parseInt` 正規化

- **哪裡**：`netiq-import-wizard.js`。唯一呼叫端已經 `parseInt` 過並保證傳數字，
  預設值也是數字，函式開頭又做一次。已移除。

### 4. 規劃與實作的落差（實作較好，補記於此）

PLAN 作業C 的定案寫「`warnings` 為共享清單：以鎖保護寫入」。實際實作改成**每段一份、不加鎖**，
與「每段各自一份 `ScanState`」同一個理由（分段互不重疊、每段單執行緒）。少一把鎖、少一個競態面，
且順序保證更強。文件（WEB-SPEC §9.9a）已依實作更新。

### 5. 查過但沒有問題的項目

- **簽名異動的呼叫點普查**：`ListHostsAsync`／`StartScan`／`ScanAsync` 三個簽名都加了參數，
  grep 全 repo 的呼叫點——兩個正式實作、一個測試替身、一個 Controller、一個 Service 全部跟上；
  `SentinelRestDirectoryClientTests` 約 50 條既有測試全走預設值（併發 1），一條未改仍全綠，
  這就是「併發度 1 行為不變」最強的證據。
- **保留天數欄位的同型普查**：`AuditRetentionDays` 出現的檔案集合與 `ReportRetentionDays`
  逐一比對，沒有漏接的轉手點。
- **`UpdateSystemSettingsRequest` 的前端組裝點**只有一處（`settings.js` 的 `api.put`），
  漏帶新欄位會讓值變 0 而卡在 `[Range]`——已確認有帶。
- **報告年月層的文化相依**：`yyyy-MM` 的格式化與 `TryParseExact` 都吃當前文化，
  但檔名本身（`$"{date:yyyy-MM-dd}"`）也是同一套，兩者自我一致；且這是本輪之前就有的既有寫法，
  不是新引入的問題。
- **明確不做**逐條核對：沒有報告瀏覽頁、沒有把併發度落地成系統設定、沒有放寬總預算、
  上限維持 3、沒有跨欄位大小約束、沒有搬移既有報告檔——全部沒有偷做。
