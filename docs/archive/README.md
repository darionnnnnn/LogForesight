...
` 在替換端是真換行。
- `FEEDBACK-42-PLAN.md`：回饋第 42 輪——PRTG 分頁收斂保險絲、資源守門截斷警告、結構同步可取消、環境探測分頁語意與排序診斷。**根因**：兩個分頁迴圈都以「PRTG 遵守 start 且順序穩定」為前提，一個無上限、一個 50 頁靜默截斷（正式環境 42864 個感測器已中招）；實機探測證實 start 超出範圍會夾到末頁而非空頁。改法：分頁抽成 `PrtgTablePager` 三道停止條件（空頁／未滿頁／本頁無新列鍵）＋ treesize 只放大不縮小的頁數上限＋列鍵去重＋ `sortby=objid`；守門即時來源改單次 `count=50000` 並比對 treesize 出聲；補 cancel 端點與兩處停止鈕，閘門改為「等待的那趟成功」才放行跳過（docs/PRTG-SPEC.md §3／§5a／§7）。**終檢**抓到最重一條：messages 的 objid 是發出訊息的 sensor 不是列唯一鍵，按 objid 去重會讓每顆 sensor 只留第一筆並提早結束整個階段，改由呼叫端提供列鍵（objid＋時間）。**體檢（換模型）**修 10 條：排程頁停止鈕對唯讀使用者的權限退化、TryStart 取消來源空窗、探測截斷警告分母誤報與 groups 同型遺漏、取消語意文件說錯（實際當場中斷進行中的查詢）。教訓：外部系統的 objid 不一定是列的唯一鍵、單發大 count 一律比對 treesize、以 d-none 做權限隱藏的元素不能被輪詢無條件切換、閘門的「結束」與「成功」是兩件事、假守門測試（初值就等於斷言值）要主動移除。
- `FEEDBACK-43-PLAN.md`：回饋第 43 輪——PRTG 取數效能：先量測再定案。**根因**：實機探測（步驟 9）證實 `historicdata` 單次 5～72 秒是固定成本、`datetime`／`value` 在這台 PRTG 一筆都解析不到、大回應會回 200 的空白 HTML 頁、併發 8 開始排隊。改法：解析改讀 `value_raw`／`coverage_raw`，時間以顯示字串為準、`datetime_raw` 只當驗證與退路（實機兩者差 7 小時）；`PrtgClient` 辨識空白頁；分頁 5000／寫入批次 500；併發 1～8；取數策略保守（預設，15 分鐘快照、夜間不逐顆）／激進（5 分鐘＋夜間逐顆）；`PrtgSnapshotHostedService` 以 `table.json` 快照自算小時值寫 `sampled` 列（流量正規化、合併寫入、退避從上次嘗試量）；校準可用列＝`ok` 或 coverage ≥ 75 的 `sampled`、FormatVersion 2、三個摘要資料集與 `detail=summary`（docs/PRTG-SPEC.md §3b／§11）。**逐條比對規劃**抓到五個規格漏寫的缺口（策略 popover、暫停原因、快照取樣說明句、MB 估算）。**體檢（換模型 Fable、scan-low 三路）**修最重四條：顯示字串月日對調時解析成功不報錯（以 raw 差 24 小時擋下）、`value` 退路取到最後一個頻道、快照整點寫入失敗整小時樣本無聲消失且誤入退避（改待寫清單重試）、退避期間 coverage 以拉長後的間隔算讓不足的列進基線；另砍死碼與無消費端欄位、修文件七處。**終檢**見 PLAN 文末。教訓：agy 抓不到統計口徑不一致與同型重複實作、規劃逐條比對只有 Claude 做得到、突變字串別帶換行、Edit 工具會把 CRLF 正規化成 LF（若上游 commit 曾意外寫成 CRLF，會出現整檔 diff）。
