...
- `FEEDBACK-44-PLAN.md`：回饋第 44 輪——立即執行回望連動 PRTG、狀態變更區間一次取完、回填閘門與停止。**起點是使用者實測**：回望 30 天時本機／NetIQ 重跑 30 天而 PRTG 只處理昨天、每天的階段 3 都翻「約 100 萬筆」messages、回填顯示 sensor 0／0。第一原則由使用者定：資料完整才找得出異常，速度其次。改法：狀態變更以區間一次取完（最早目標日前一天到今天），每頁自驗「時間遞減且整頁早於門檻」才提早停止，否則翻到底；PRTG 路徑收日期清單逐日做規則評估與 finding 發佈，對應只重算最新一天、過去日沿用「該日或之前最近 31 天內」的對應（`HostMapLookbackDays`），沉默規則只評最新日，登錄簿按日、AI 完整性閘門涵蓋整個日期範圍、執行總表 PRTG 欄依資料日期、進度軌「第 i／N 天」；歷史回填加三道閘門（取數中、同步中、無對應）與 cancel 端點＋停止鈕，狀態變更整趟只翻一次、無對應的日子計略過；夜間結構同步也寫 `prtg_sync_status`（來源 nightly）；探測 9d-4 判定 `filter_drel`。**Claude 在 B1 驗收抓到白名單外的缺陷**：觸發式取數用「當日」對應而管線只重算最新日，多日取數會全空——改成同一個往回視窗並補回歸測試。**委派**：agy 對 C 批兩次數秒 exit 0、零輸出、零改動（簡單提示正常），改派 impl-low（Opus low）；agy 中途結束沒回報兩次（diff 完整，Claude 逐行讀 diff 驗收）、剝 BOM 一次。**體檢（換模型 Fable、scan-low 三路）**修 11 條：`LocalOnly` 也跟著回望多日、逐日統計取消時丟失（改 finally 寫）、互斥只有一個方向（同步端補擋回填）、回填無對應閘門視窗與逐日迴圈口徑不一致、`processedDays` 少算一天、未啟用算成暫停、階段 3 未收斂不印統計、稽核頁 15 個動作代碼沒有中文名稱（補反射守門測試）、死碼與無消費端欄位、AI 閘門測試偶發紅（假 AI 加閘門）。**終檢**再修 7 條：日期清單契約補「純日期」、迴圈前失敗整排記 failed、非 OCE 中斷也算打斷、提早停止句附「途中有頁不依時間」、同型文案另一處、測試等執行結束再 Dispose、§5 反向閘門補記。教訓：agy 大任務零輸出且簡單提示正常時改派同階實作代理並記錄（skill 已改）、白名單外的消費端要回頭查、批次改檔腳本先正規化 CRLF、只有一筆待補的「執行中」斷言要用閘門卡住假服務。
- `FEEDBACK-45-PLAN.md`：回饋第 45 輪——畫面狀態一致性與載入效能（A1~A4／B1~B7）：排程頁動作鈕跨卡互斥（取數執行中藏 AI 啟動鈕、API 層 409）、執行中告示搬上共用版型供全站訂閱、執行紀錄查詢下推 SQL、「下一筆未處理」專用端點與缺省 90 天下界、設定 blob 與可見範圍跨請求快取、PRTG 兩張大表區間索引、慢查詢前十支、GET 逾時與失敗重試鈕。實作期推翻七條定案（常駐投影因 store 雙實例不可行、`LatestOccurrences` 下推因雙後端 `UPPER` 語意分岔放棄…）；體檢抓到 NLog target 全域設定無互斥讓並行執行互相抹掉、續號只取尾端一筆會與既有 RunId 撞號、告示 `isRunning` 是聯集被當互斥判斷用。
- `FEEDBACK-46-PLAN.md`：回饋第 46 輪——讓 PRTG 規則真正發揮作用。**起點**：核對發現四條 PRTG 規則的 finding 進全鏈後是二等公民——映射只讀靜態表（規則頁改嚴重度／重大旗標無效、RuleId 對不上知識庫）、抑制對 PRTG 完全無效、問題排行把所有 PRTG 塌成一列、趨勢／關聯／體檢零認識、Ping 因取數白名單被排除。改法：finding 改由命中的規則物件驅動（`Source=PRTG:{代碼}`、判定看 LogName）；發佈前統一標記抑制；已於 PRTG 確認的 Down 不重大；規則母體脫離取數白名單並做同裝置失聯合併；分類加 availability／hardware 與 type 分類補充對照設定；規則可依分類覆寫（seed v7 八條）；PRTG 路徑自做跨日標註、重複升級、長期 Down 降噪（兩段式迴圈）；體檢 prompt 獨立 PRTG 段；追加時做三個跨來源佐證模式（儲存故障／磁碟容量／失聯，容量對容量、故障對故障）；未回報主機依鏡像標 PRTG 現況（判定收斂一份）；主機頁 PRTG 表修 XSS 並加全站 innerHTML 守門。**委派**：A1、A2 用 agy，自 A3 起使用者改指定 impl-low（Opus low）；每段 Claude 獨立驗收＋突變，抓到校準以代碼 ToDictionary 在 v7 丟例外、重跑時舊命中算進歷史、長期 Down 只關旗標沒封頂等。**終檢**修白話說明挑規則與佐證取消抑制補不回來。**體檢（換模型 Fable、scan-low 兩路）**補四條缺測驗收、升級注意補「簽章型抑制與處理狀態不延續」、執行輸出口徑、逐日進度搬回第一段、守門放寬後抓到 record-detail 一處跨行插值、文件冗餘與過時十二處。教訓：impl-low 也要獨立驗、子代理共用 scratchpad 會撞名、`git add -A` 收進別人的檔、autocrlf 下 `git show` 的 LF 不是被改、Git Bash heredoc 吃反斜線、驗收 grep 別寫太寬、ES module 的 node --check 要轉 .mjs。
- `FEEDBACK-47-PLAN.md`：回饋第 47 輪——交辦單、自動派工、問題靜音。**根因**：工作單位太細，每一個批次功能（批次指派、統一標記、一次回覆）都在「每台主機一件」的粒度上用迴圈硬撐，主機量一大就露出 200 台預覽上限、全有全無回覆、沒有通知、每天重複派同一種問題。改法：案件之上加**交辦單**（一個問題 × 一批主機 × 一位處理人，不存自己的狀態）、兩條不變式（進行中案件必屬一張單、同人同問題最多一張）、逐日同步就地／背景分流、夜間掛接 ⓪～⑥ 與派工閘門、派工池與暫停接單、交辦總覽四頁籤與詳情頁、我的交辦改以單為主體、郵件通知；**靜音**以問題檔案的區間為唯一事實來源，讀取側單一列判定（目前靜音中或紀錄日落在區間）、`IIssueAggregateQuery` 每個方法強制帶排除條件（反射守門）。**委派**：A～D-1、B-1／B-2 由 impl-low；E-1 起 agy；D-2 起使用者停用 agy 改 impl-low；收尾 H-1b 起改回 agy。每段 Claude 獨立驗收（位元組、突變、瀏覽器實測）。**壓測**抓到四個規模瓶頸（逐筆寫歷程讓背景展開 53 分鐘→91 秒等）。**逐條比對規劃**補定案 48～52（群組篩選、chip 連單、靜音數套篩選、交辦範圍與續掛、詳情頁範圍欄）。**實測抓到既有缺陷**：依問題端點宣告父型別讓去重主機數等欄位被 JSON 序列化吃掉。**收尾體檢（scan-low 四路）**修：回填器與刪除指定主機日自開交易沒包執行策略（SQL Server 必失敗，加守門）、處理歷程批次續號可能撞號、已交辦分子全站計數洩漏、派工 ⓪ 口徑。教訓：宣告型別吃掉子型別欄位、`Upsert` 逐欄複製會讓新旗標被清掉、自開交易要包執行策略、規格階段的簡化（固定指定主機）要記進設計修正否則比對時漏看。
