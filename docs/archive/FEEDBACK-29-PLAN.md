...
## 體檢輪修正（claude-fable-5）

1. **`AuthCookie.IsSecureConnection` 不認多層代理的逗號清單**：`X-Forwarded-Proto` 經兩層以上
   代理是「https, http」形式，整串 `Equals("https")` 恆 false，Secure 又靜默掉回去——正是
   這條路徑要修的症狀。改取第一段（最靠近使用者的協定）比對，補迴歸測試
   `多層代理的XForwardedProto清單_取第一段判定`。
2. **規劃落差（不改程式，補記理由）**：
   - 批次 B1 驗收原寫「設定頁 API/檢視測試」，實作最終是純前端即時渲染（資料取自既有
     settings DTO，該 DTO 已有測試），本專案無 JS 測試管線，故無新增測試——驗收條款
     依實作事實修訂為「AD 狀態隨勾選/清單即時更新，由使用者實測驗收」。
   - 批次 D 驗收原寫「郵件摘要問題集合 == KPI 同窗口集合」的全集合斷言，實際落地為兩條
     行為測試（隱藏嚴重度不進郵件、低風險日不進郵件）——兩者合計已涵蓋本輪改動的兩個維度，
     全集合斷言需另建跨服務對帳工具，成本大於增量價值，不補。
3. 其餘四角度（規劃逐條對應、呼叫點跟上、架構契合、測試基線 2567→2588）無新發現。

測試：2588 通過／6 略過。

## 終檢輪

終檢無新發現（實作中已含兩次獨立 Explore 終檢與換模型體檢輪，發現均已於各節記錄並修正）。
