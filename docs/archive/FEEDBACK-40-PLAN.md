...
## 4a. 規劃逐條回檢（使用者要求，收尾後補做）

把 §1 定案與各批次「契約／驗收」逐條回到程式碼驗證，**十處未達契約**，已全部補上：

| # | 出處 | 缺口 | 處置 |
|---|---|---|---|
| 1 | A 契約 | `prtg-done` 完成文字只有兩種，缺「未啟用」——PRTG 沒開與開了沒抓到在畫面上一樣 | 補第三種；前端以 `prtgModuleEnabled` 分辨，宣告移到使用點之前避免暫時死區 |
| 2 | C 契約 | 互斥被拒回 400 而非 409 | 互斥類（訊息含「執行中」）改回 `Conflict`，補端點測試＋突變驗證 |
| 3 | C 契約 | 啟用 `PrtgEnabled` 沒有「請先執行同步」的提示 | 前端切換為啟用時 toast 提示 |
| 4 | B2 契約 | 重算警告被丟棄，`HostDto` 無 `RemapWarning` | 加欄位並帶進主機儲存回應，補兩條測試＋突變驗證 |
| 5 | B2 驗收 | 「`TryRemapToday` 在 Controller 內零定義」未達成（留了委派方法） | 移除委派，四個呼叫點直接叫服務 |
| 6 | B2 驗收 | 「PRTG 停用不呼叫」沒有測試（假重算器一律計數） | 補兩條測 `PrtgHostMapRefresher` 本身的啟用閘門 |
| 7 | B3 契約 | 零對應主機「並寫里程碑」未做 | 在 `PrtgDailyPipeline` 補里程碑（fetcher 沒有 recorder） |
| 8 | B1 契約 | 訊息未說明「是哪一側解析失敗」 | 補 `UnresolvedHint`：位址本身解析不出 IP 時加註，與「解析得到但沒有 device」分開 |
| 9 | E 驗收 | 「含 `aiEnabled` 的舊 JSON 可反序列化」無測試 | 補一條（升級路徑的守門） |
| 10 | F 驗收 | 「整包只送守門欄位時其他 PRTG 設定不變」無測試 | 補一條。過程中確認產品行為正確，是 `ValidRequest()` 自己填滿了 PRTG 欄位 |

**兩處實作與契約不同，判定實作為準並改文件**：

- **device 分組鍵用解析層而非純語法層**（契約寫純語法）：填 DNS 名稱的 device 在純語法層一律回 null，
  用純語法當鍵它們根本進不了分組、也就永遠對不到主機。已在 PRTG-SPEC §4 說明代價（同名指向同位址會被判衝突、
  DNS 一改分組跟著變）。
- **IP 排除清單的比對**：儲存鍵是純語法值，device 側拿去比對的是解析後的 IP。原本文件寫「兩邊用同一把尺」
  是錯的，已改為描述實際行為並說明這是要的（使用者排除的是那個位址，不是那種寫法）。

**一處確認為刻意，補記理由**：即時來源走全量分頁再記憶體過濾，不用 `filter_parentid`——
偵測要先比對完位址才知道命中哪些裝置，逐裝置查會變成 N 次往返。已寫進 PRTG-SPEC §12。
