...
| B-1 | agy 3.8-flash-high | 通過 | 全套 3392 綠（+5）；**突變測試**：把同 IP 略過規則改成恆不成立，2 個測試轉紅，斷言有效；BOM／NUL 乾淨 | 白名單漏列 `SchemaUpgraderPrtgTests`（既有測試枚舉 PRTG 表清單），agy 補一行屬必要，判定接受。資料層呼叫 Service 層的 `NormalizeIp` 看似分層反轉，但專案內已有三處相同做法（既有慣例），不改 |
| B-2 | agy 3.8-flash-high | 通過 | 全套 3400 綠（+8）；BOM／NUL 乾淨 | Claude 補修兩處：(1) 兩個 DELETE 端點回 `bool`，把 `TryRemapToday()` 的警告丟掉，重算失敗時管理者看不到——新增 `PrtgDeleteResultDto` 讓警告到得了畫面；(2) 衝突清單分頁前無排序，而 `GetLatestHostMapWithDate` 查詢本身沒有 ORDER BY，會造成跨頁重複或漏列——在 controller 端加 `OrderBy(DeviceObjid)`，不動共用的資料層查詢 |
| B-3 | agy 3.8-flash-high | 通過（委派中斷後由 Claude 補完） | 全套 3403 綠（+3）；BOM／NUL 乾淨；`confirmAction` 三處皆物件參數 | agy 執行途中回報連線錯誤而結束（exit 0），**測試檔完全未建立**。逐項驗證已產出的 cshtml／js：驗收字串全數命中、build 零錯誤、modal 兩種型別分岔與分頁邏輯完整，判定實作非半套，改由 Claude 補寫 3 個測試而不重跑整段。其中 `confirmAction` 呼叫形式的測試是釘住「字串＋callback 誤用」的回歸——該誤用不會有任何編譯或執行期訊號 |
| B-4 | agy 3.8-flash-high | 通過 | 全套 3407 綠（+4）；BOM／NUL 乾淨 | **本段是 PLAN 定案漏抄進規格的補做**：回填執行紀錄時逐條比對 PLAN 定案與三份規格，抓到兩條從未進規格、因此完全沒實作——主機明細頁「IP 已排除」的顯示、人工對應清單「同 IP 另有 N 台已略過」。兩者成因相同：被排除／被略過的 device 根本不產生對應列，畫面看起來與「從沒設定過」無異。規格另釘住既有端點在鏡像無資料時會提前返回，排除判定必須在那之前完成（已由測試釘住）。驗收另確認 `GetAllDevices()` 未落在迴圈內 |
| C-1 | agy 3.8-flash-high | 通過 | 全套 3412 綠（+5）；BOM／NUL 乾淨；`local-done` 恰兩處 | Claude 補修：agy 把本機分析方法整個本體包進 try/finally 送完工訊號（邏輯正確），但約 190 行方法本體未跟著縮排、收尾大括號錯位，分析主流程這種關鍵檔案的可讀性明顯退化——重新縮排後以 `git diff -w` 確認實質改動僅 7 行。本階段**推翻既有契約**（done 訊號原本清空該軌欄位，改為保留數字＋Completed 旗標），規格明列哪些既有測試是刻意改寫、哪些行為必須維持成立，agy 兩條舊測試皆改為新契約斷言而非刪除 |
| C-2 | agy 3.8-flash-high | 通過 | 全套 3416 綠（+4）；**突變測試**：移除等待期回報那一行，對應後端測試立即轉紅；BOM／NUL 乾淨 | Claude 補修：agy 為滿足驗收條件 4，把 `PROGRESS_PHASE_LABEL`／`UNIT` 從模組層搬進`renderScheduleProgress`，導致執行中每 3 秒重繪都重建常數物件——移回模組層並加註理由（驗收條件寫得過死是主因，該條件的本意是限制 `updateProgressBar` 不得含 phase 知識）。另確認狀態查詢提前發出無相依問題：它只用到權限旗標，該旗標在提前發出之前已設定 |
| D-1 | agy 3.8-flash-high | 通過 | 全套 3422 綠（+5）；**突變測試**：停用 runId 比對 → 1 紅；完全移除過濾（等同舊行為）→ 3 紅；BOM／NUL 乾淨 | agy 另修一處未要求但必要的問題：兩個 recorder 並存時共用同一個 target 名稱 `batchrun`，移除時會誤刪對方的，改為帶 runId 區分（這是「兩個 recorder 各認各的」測試能成立的前提）。**Claude 失誤**：還原突變時用 `git checkout --`，把 agy 未 commit 的改動一併清掉，改以先前審查過的 diff 逐項重新套回並比對確認一致；此後突變一律先 `cp` 備份、還原走備份檔 |
| D-2 | agy 3.8-flash-high | 通過 | 全套 3427 綠（+5）；**突變測試**：移除慢 SQL 豁免 → 對應測試轉紅；BOM／NUL 乾淨 | 確認 `Finish`／`Dispose` 有 `_finished` 防重入、`FinishRun` 例外內部已捕捉，AI 排程改用 recorder 後「登記失敗不影響執行」與「取消＝已停止、exit 1」兩條既有語意皆維持 |
| D-3 | agy 3.8-flash-high | 通過 | 全套 3433 綠（+6）；`git diff -w` 確認縮排未被破壞（33 行新增）；四個 outcome 值字面值在主流程檔案零命中；BOM／NUL 乾淨 | 規格把 C-1 踩到的坑寫成限制（只加行、不重排既有區塊，並以 `git diff -w` 當驗收條件），本次未再發生。取消執行時不寫入 outcome（欄位維持 null），本機提早返回的兩條路徑亦不寫入——null 與 0 語意分離是本階段的核心，舊格式 JSON 反序列化有專屬測試守門 |
| D-4 | agy 3.8-flash-high | 通過 | 全套 3441 綠（+8）；BOM／NUL 乾淨；徽章對照表單一定義；`innerHTML` 零命中 | Claude 補修兩處：(1) **併發風險**——快取把實機 7~15 秒的 `CountPendingAi` 放在鎖內執行，而該鎖同時保護 AI 排程的 `TryBeginRun`／`EndRun`，冷快取時畫面輪詢會卡住排程狀態機十幾秒；改為查詢在鎖外、只在寫入快取時上鎖，並註明取捨。(2) **字面值散落**——「AI 回覆不是合法 JSON 或格式不符契約」在產生處、判定處、組裝處各一份，改文案會讓「是否附上回覆片段」的判定靜默失效（訊息照樣顯示、片段消失），收斂成常數。另記：`SentinelRestDirectoryClientTests.多段預算用盡回部分結果與警告_不擲例外` 以 `Thread.Sleep(1200)` 對抗 1 秒預算，全套高負載時偶發轉紅（本輪出現 2 次，單獨重跑與第三次全套皆綠），與本輪改動無關，建議收尾時改成閘門式而非睡眠式 |
| E-1 | agy 3.8-flash-high | 通過 | 全套 3448 綠（+7）；BOM／NUL 乾淨；`AnalyzeDayAsync` 在主流程零命中 | **突變抓到測試假通過**：agy 的 5 條測試全綠，但把主流程那一行改回 `AnalyzeDayAsync`（行內跑 AI）後 5 條**照樣全綠**——它們只驗到 `LogAnalysisService` 的統計段行為，完全沒覆蓋主流程選了哪個方法，本批次的核心契約等於零回歸保護。Claude 補一條原始碼守衛測試（主流程必須含 `AnalyzeDayStatisticalAsync(`、不得含 `analysisService.AnalyzeDayAsync(`），重做同一突變後轉紅。另清死碼：`HostDayPostProcessor.RecordAiCallIfApplicable` 在本機路徑移除呼叫後全專案（含測試）零呼叫端，比照 R37 刪 `GetDedupeKeys` 的先例刪除。agy 另修一處 C-2 引入的時序問題：狀態提前發出時 `aiAvailable` 尚未設定，以 `lastAiScheduleStatus` 暫存並在旗標就緒後重新套用 |
...
`SentinelRestDirectoryClientTests.多段預算用盡回部分結果與警告_不擲例外` 是**既有的時間相依測試**，
以 `Thread.Sleep(1200)` 對抗 1 秒總預算，全套高負載時偶發轉紅、單獨跑穩定全綠，
與本輪改動無關（本輪未動 Sentinel 目錄查詢）。成因與修法已記入 `docs/BACKLOG.md`。

**併回前終檢的發現**（皆已修）：

1. **規劃定案漏抄**（比對 44 條定案）：批次D「異常彙總欄位改名『執行站台』」寫進了規格
   但沒放進驗收條件，逐段驗收因此無訊號。已補改名與守衛測試。
   同型問題在批次B 也發生過一次（兩條定案完全沒進規格，補做為 B-4）。
2. **跨段產出鏈斷點**：`prtgTriggeredHosts` 後端寫入、DTO 帶出，但前端 PRTG 欄渲染
   未接該參數，畫面永遠看不到。D3 驗「有寫入」、D4 驗「有顯示 PRTG 欄」，兩段各自都綠。
   以「後端新增欄位逐一 grep 前端消費點」掃出，其餘 8 欄與 4 個狀態旗標皆有消費端。
3. **設定紅線檢查**：八個新設定鍵逐一確認在 Core 有消費端，無「有設定無行為」。

**升級注意事項**：

- 新表 `lf_prtg_ip_excludes` 走 `SchemaUpgrader` 冪等 DDL，既有 DB 自動建立。
- `BatchRun` 九個分路欄位為可空，舊 JSON-line 紀錄反序列化為 null、畫面顯示「—」。
- **資源守門預設關閉**，啟用前務必先用維護頁的「預覽受監看 sensor」確認偵測結果與
