...
併 dev 後全量 3586 通過／0 失敗／略過 6。體檢修正 commit（`9ddb361`）單獨再掃：AI 回寫的合併判斷、逐主機日鎖與「已追加」標記語意成立，共用的 `Empty` 登錄簿在 `For` 回空時即短路、不會碰到鎖表。終檢無新發現。

## 體檢輪修正（換模型獨立審查：實作 Claude Opus 5，體檢 Claude Fable 5.1）

體檢對象：`dev..feature/feedback-39` 全部 13 個 commit，兩個獨立審查（獵 bug／架構、規劃比對／文件）各掃一次全 diff，
另對最後兩個手改 commit（終檢修正、定案比對補缺）逐行再看一遍。規劃比對：每個批次的改動都有對應 diff、
「明確不做」與「定案調整」的東西都沒混進來、沒有計畫外改動。

| 哪裡 | 症狀 | 怎麼修 | 迴歸測試 |
|---|---|---|---|
| `EfAnalysisRecordStore.AttachAiResult` | AI 回寫無條件覆蓋 `RiskLevel`。AI 撿到紀錄後、回寫前若 PRTG 追加把列上等級上調，回寫會蓋回 AI 撿取時的舊等級——與「AI 只能往上拉」的既定規則相違 | 回寫改取 `MoreSevere(列上, AI)`，列上較高時連依據一起保留；抽出欄同步用合併後的值 | `AI回寫不得蓋掉PRTG上調的風險`（突變成無條件覆蓋後見紅）、`AI回寫比列上高時照常上調` |
| `PrtgFindingsRegistry`／`HostDayPostProcessor`／`PrtgDailyPipeline` | 補追加與寫入路徑對同一主機日並行：兩個交易各自讀到相同既有鍵、各插一列，`lf_top_issues` 重複、`ContentJson` 後寫蓋先寫；補追加先到時寫入路徑的記憶體不更新，執行摘要顯示低、資料庫是高 | 登錄簿新增 `AttachExclusive`（逐主機日行程內鎖）與 `WasAttached`；兩條路都經它；寫入路徑在資料庫已由另一路追加時仍併入記憶體。順手移除無呼叫端的 `PublishedHostIds` | `AttachPrtgFindings_補追加先到時寫入路徑仍併入記憶體`、`AttachExclusive_只在真的寫入時標記已追加` |
| `SchedulerHostedService`／`runs.js` | 「將於下一個窗口補跑」在最常見情境是錯的：手動執行結束時若仍在窗口內，`ShouldTriggerNow` 會立刻補觸發 | 文案改為「結束後若仍在窗口內會立即補跑，否則等下一個窗口」，WEB-SPEC 同步 | 文案，無 |
| `Runs.cshtml` | 唯讀模式把「最後更新：誰、何時」跟儲存鈕一起藏掉，與「以唯讀方式呈現設定值」的說明不符 | `data-maintain-only` 只掛在按鈕上，`span` 移出 | 既有 UI 測試涵蓋元素存在 |
| `prtg-admin.js` 估算文案 | `triggered-plus-list` 只估指定清單那半，畫面卻印成整個模式的規模 | 前綴「指定清單部分：」 | 文案，無 |
| `PrtgDailyPipelineTests.初始化失敗時仍宣告就緒` | 名實不符：`127.0.0.1:1` 只讓階段 1 失敗、流程照常走到正常的 `Publish`，沒碰到 finally 保底 | 改用 `ftp://` 讓 client 建構就擲例外，真的走外層 catch → finally | 同名測試 |
| 三處 XML 註解錯位 | 搬移程式碼時例外／驗證方法的 summary 黏到相鄰成員、`<param>` 夾在 attribute 與簽章之間 | 歸位 | 無 |

不修、只記錄：`localUseAi`（本機路徑）與補追加的 `useAi` 差一個「主機已登記」條件，補追加的主機一定已登記，兩路等價；
重疊窗口只看第一個（設定驗證已擋重疊）；被佔用訊息隨手動執行結束消失（結束後若在窗口內會立即補跑，訊息已無意義；
跨過窗口的情況走 NLog）；`runs.js` 拆檔與 RunState 基底已寫進 BACKLOG；並行負載下的不穩定測試（本次體檢又觀察到
`AiAnalysisSchedulerTests.ScheduleController_Ai端點_狀態查詢_立即執行與停止` 與 `NetiqScanConcurrencyChainTests`，
皆為 dev 既有、本輪未動、單獨重跑即綠）四條名稱一併記進 BACKLOG。

體檢後全量測試：見下方「體檢交接」節的更新數字。

## 體檢交接（實作輪收官）

- **狀態**：八個批次全部完成並 commit，併回前終檢的六項修正亦已完成。**尚未併 dev**。
- **全量測試**：實作輪收官 3582 通過；體檢輪修正後 **3586 通過 / 0 失敗 / 略過 6**（總計 3592）。
  基線 3497 → 3586，本輪淨增 **89** 個測試。
- **測試隔離的既有偶發**：全套並行執行時偶有一條紅（觀察到兩次，分別是
  `SentinelRestDirectoryClientTests.多段預算用盡回部分結果與警告_不擲例外` 與
  `BatchRunRecorderScopeTests.Scope外的Warn不會被記錄`），兩次都單獨重跑即通過、
  再跑一次全套亦全綠。與本輪改動無關（未動這兩個受測類別），屬既有的測試隔離／時間敏感問題。
  值得下輪處理，已知但不在本輪範圍。
- **建置**：0 警告 0 錯誤。JS 全部經 `node --check`。
- **編碼**：全部改動檔案的 BOM 與基準 `eb1434c` 一致，無 NUL 位元組。
- **瀏覽器實測**（批次F）：桌面 1440 無水平溢出、三張狀態卡高度接近、平板 768 正確疊直、
  設定頁籤切換時工具列隱藏、主控台零錯誤。
- **需要實機驗證的暫定項**：PRTG `table.json` 的 `treesize` 是否存在（不存在時分母為 0，
  畫不定進度但分子照走，不會壞）；`filter_drel` 是否被該版 PRTG 接受
  （被忽略時用戶端當日過濾仍在，只是慢）。
- **升級後第一晚的預期變化**：PRTG 規則命中會提升當日風險並可能觸發 AI 補跑，
  **風險日數可能上升**——這是規格原本承諾但未生效的行為。

## 定案逐條比對（併回前最後一次，Claude 親自對照）

把「## 1. 定案」與七個批次的「### 定案／測試／驗收」逐條拿到程式碼驗證。結果：

- **全部落實**：E（總表錨點、異常彙總與執行紀錄仍含今天）、A（三子 phase／treesize／每 50 頁輸出／`filter_drel`／五個閒置原因）、
  B（登錄簿在共用上下文、兩條寫入路徑都排在案件掛接之前、補追加一次而非輪詢、三種就緒路徑、AI 閘門）、
  C（單向上調、`prtg:` 依據、待補判準、補追加路徑補掛案件、prompt 獨立區塊、風險 log 暫存未動）、
  D（`forceAuto` 與自動填入鈕、偵測為空不清空、三選一設定、取數與回填共用選取器、估算端點與門檻）、
  G1（正式碼零裸 phase、反射對照測試）、G2（主流程 1236 行）、G3（三軌值型別）、G4（就緒訊號全鏈到前端）。

- **本次比對補上的三個缺口**：
  1. `PrtgTriggeredValueFetcher` 類別註解「昨日風險」（D 定案的「順手修」條目，先前漏抄進實作）。
  2. `ScheduleStatusDto` 與 `SchedulerRunState` 的進度欄位名契約測試（G3 驗收明列「status API 欄位名不變」，先前只靠既有測試間接涵蓋）。
  3. `PrtgDailyPipelineTests`（G2 改動列的新檔）：PRTG 未啟用／初始化失敗時仍宣告就緒，且就緒訊號排在收尾訊號之前。突變 `finally` 保底後 2 條見紅。

- **維持「定案調整」的四項**（理由見下節）：RunState 共通基底、儲存時 objid／主機名警告、`runs.js` 拆檔、`PROGRESS_PHASE_UNIT` 殘留條目。

## 併回前終檢（兩個獨立審查，程式碼與文件各一）

### 已修正

| 嚴重度 | 發現 | 處置 |
|---|---|---|
| 高 | **登錄簿沒有日期維度**：兩條寫入路徑都在逐日迴圈裡追加，PRTG 只評估目標日。停機三天後開跑，昨天的 `down` 會被掛到三天份的紀錄上、各自被上調成高風險並進 AI 待補；`EventKey` 不含日期、去重完全生效，重跑不會自癒 | `Publish`／`For` 帶日期並比對；補三條測試（跨日不追加、二次發佈換日、回補其他日期時零追加） |
| 中 | **忽略資料庫端回傳值**：查無當日列或 `detail_pruned` 時資料庫整段早退，記憶體卻照樣改風險——執行摘要顯示「高」、資料庫裡還是「低」 | 先看回傳值再改記憶體；補兩條測試（已精簡走真實 `PruneDetails`、當天無紀錄） |
| 中 | **`all-mapped` 的第二道防線缺失**：定案寫了雙重防線，實際只有設定層那一道。設定 blob 由匯入或人工編輯繞過驗證寫進來時，夜間批次會對全部已對應 device 的所有 sensor 取數 | `EffectiveScope` 在執行層退回 `triggered` 並在執行輸出說明；取數、回填、估算端點三處共用；估算端點補五條測試 |
| 中 | **窗口誤報**：只判「現在在不在窗口內」，沒判「這個窗口是否已觸發過」。22:00 觸發、22:40 跑完、23:00 按立即執行 → 誤報「22:00 的自動觸發被佔用」 | 抽 `ScheduleCalculator.WindowAlreadyTriggered` 與 `ShouldTriggerNow` 共用（原本是複製了它的一半），補三條測試 |
| 中 | **`filter_drel` 邊界差一**：級距是滾動時間窗（`7days` 從 `now-7d` 起算）而非日曆日，目標日剛好是第 7 天時該日凌晨到執行時刻的變更落在窗外 | 邊界一律往上跳一階（`< 7`／`< 30`／`< 365`），測試期望值同步 |
| 中 | **補追加統計語意錯誤**：`AttachPrtgFindings` 回 false 有兩種情況（紀錄未落地／已由寫入路徑加過），統計把後者也報成「尚未落地」。並行後後者是常態 | 摘要文字改為不宣稱原因 |
| 低 | **標籤完整性測試被子字串比對繞過**：`Contains("prtg-sync")` 在對照表只有 `prtg-sync-devices` 時仍為真——正好放過它要擋的漏改 | 改比對鍵本身；以突變驗證（刪掉 `prtg-sync` 鍵後見紅，舊版斷言會放過） |
| 低 | `Assert.Contains("collapse", cshtml)` 幾乎恆真 | 改為斷言收合容器與觸發器的實際接線 |
| 低 | 儲存時的 objid／主機名稱警告（D1／D2 定案）未實作 | 見下方「定案調整」 |

### 定案調整（實作事實推翻規劃，理由記於此）

- **五個 RunState 抽共通基底（G3）**：不做。四者共通部分只剩「一個 bool 與兩個時間戳」，抽出是空殼。改做三軌進度收斂，那才是「漏一個就殘留舊進度」的實際成因。
- **儲存時對未知 objid／主機名稱回警告（D1／D2）**：不做。存檔當下鏡像可能還沒同步（剛設好連線就存設定是正常流程），此時「查無此 objid」是偽陽性；執行期已有輸出列出對不到的名稱，維護頁的「自動偵測並填入」也讓 objid 不必手打。**改為在執行輸出說明**，已實作。
- **`runs.js` 拆成四個檔（F2）**：不做。版面重排後 `runs.js` 仍是 1609 行，但拆檔要處理跨檔共享的 17 個模組層狀態與輪詢生命週期，屬於獨立一輪的重構；本輪的實際痛點（版面骨架）已解決。列入下輪候選。
- **`PROGRESS_PHASE_UNIT['prtg-sync']` 殘留條目**：保留。`prtg-sync` 仍會被送出一次（total=0，畫面顯示「準備中…」不用到單位），移除會讓它 fallback 成錯誤的「主機日」。

### 明確不修（記入 BACKLOG）

- **紀錄重寫與 finding 追加之間沒有樂觀併發保護**：既有架構問題，本輪讓窗口略為變寬但未新增機制。要修得給 `lf_daily_records` 一個 rowversion 並改全部寫入路徑，影響面過大。已寫入 `docs/BACKLOG.md`。
- **估算端點在 `triggered` 模式回 0**：觸發主機數逐日變動、事前算不出來。前端已有「事前無法估算」文案，端點層不另加欄位。
