...
九項（`ReplaceHostMapForDate` 刪寫非同交易、SQLite 併發寫鎖、probe 的取消訊號被吞、
`prtg-mirror` 逐日查詢、probe 截斷警告只有一步、介面的測試用預設實作、前端與解析各有一處
逐字重複、兩個測試斷言強度不足、`StartedAt` 無消費點）已查證屬實，判斷標準是
「不會靜默造成錯誤結論」——會靜默出錯的都在本輪修掉了。全部登記於
`docs/BACKLOG.md` 的「第 1 輪終檢提出、本輪判斷不修的項目」，免得日後當成新發現重查。
