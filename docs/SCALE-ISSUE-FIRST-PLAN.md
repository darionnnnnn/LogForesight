...
## 八點五、P0 實測結果（2026-08-06，`feature/scale-issue-first`）

壓測工具位置：`LogForesight.Tests/Scale/`（`ScaleProfile`／`ScaleDataSet`／`ScaleServices`／`ScaleBenchmarks`）。
壓測案例預設**不執行**，設 `LF_SCALE_BENCH=1` 才跑；產生器本身有四條常態測試護航
（`ScaleDataSetTests`），避免它在往後幾輪改動中悄悄爛掉。

```bash
$env:LF_SCALE_BENCH=1; dotnet test --filter "FullyQualifiedName~ScaleBenchmarks"
```

### 8.5.1 `issue_handling` 整份 blob 的成長曲線（實測）

量的是「使用者標記一個問題」的實際成本——一次 `Save` → `SaveMany(1 筆)` → 一次整份
讀→反序列化→合併→序列化→寫回：

| 列數 | blob（百萬字元） | 單次標記 | 單次標記的記憶體配置 |
|---|---|---|---|
| 10,000 | 3.3 | 113 ms | 23.9 MB |
| 50,000 | 16.3 | 521 ms | 118.7 MB |
| 100,000 | 32.5 | 987 ms | 237.2 MB |
| 250,000 | 81.3 | 1,685 ms | 591.9 MB |
| 500,000 | 162.5 | 3,320 ms | 1,183.7 MB |
| 1,000,000 | 325.1 | 6,841 ms | 2,367.2 MB |

**完全線性，約每列 6.8 µs、每列 2.4 KB 配置。** 據此外推：

| 規模 | `issue_handling` 列數 | 單次標記 | 單次標記配置 | blob 字元數 |
|---|---|---|---|---|
| 2000 台 × 90 天 | 108 萬 | **≈ 7.4 秒** | **≈ 2.6 GB** | 3.5 億 |
| 6000 台 × 90 天 | 324 萬 | **≈ 22 秒** | **≈ 7.7 GB** | 10.5 億 |

**硬失敗點確認**：C# `string` 的上限是 2 GB 單一物件 ≈ **10.7 億個字元**。
6000 台 × 90 天推得的 10.5 億字元**正好貼在上限上**——序列化那一步會直接
`OutOfMemoryException`。§零 修正 2 的推算（估 341 bytes/列，實測 325 bytes/列）成立。

**P3 完成後的同一組量測**（`ScaleBenchmarks.單筆標記成本不隨資料量成長`）：

| 表內列數 | 單次標記（改版前 → 後） | 配置（改版前 → 後） | `SaveMany` 90 列（前 → 後） |
|---|---|---|---|
| 10,000 | 113 ms → **11 ms** | 23.9 MB → **0.1 MB** | — |
| 100,000 | 987 ms → **6 ms** | 237.2 MB → **0.1 MB** | — |
| 500,000 | 3,320 ms → **6 ms** | 1,183.7 MB → **0.1 MB** | — |
| 1,000,000 | 6,841 ms → **5 ms** | 2,367.2 MB → **0.1 MB** | 11,318 ms → **11 ms** |

**曲線由線性變成平的**：單筆標記與批次寫入都不再隨資料量成長，
記憶體配置從 GB 級降到 0.1 MB，2 GB 物件上限的天花板整個消失。

### 8.5.2 `SaveMany` 的 O(既有列 × 本次列)（實測，對應 N2）

在 100 萬列的 blob 上做一次「案件建案回溯 90 天」規模的批次寫入（90 列）：**11,318 ms**。

比單筆 `Save`（6,841 ms）多出的 4.5 秒就是合併迴圈的成本——`SaveMany` 對每一筆
要寫的資料都在既有清單上做一次 `FirstOrDefault` 線性搜尋。
**這證實體檢 S3／S4 的建議（「累積起來一次 SaveMany」）方向對但不足**：
`SaveMany` 本身必須先建索引，否則批次合併只是把 N 次整份重寫換成一次 O(N×K) 的掃描。

### 8.5.3 依問題視角在 2000 台規模下的實測（進行中）

`SearchByIssue` 在 2000 台 × 90 天的資料集上**單次查詢超過 45 分鐘仍未返回**。
成因是 N3：`BuildIssueGroup` 逐問題群組各做一次 `_issueHandlings.GetMany()` 與
`_cases.GetMany()`，兩者都是整份 blob 反序列化——600 種問題 ＝ 1,200 次讀 18 萬列的 blob。

**這是本規劃最重要的單一實測結果**：需求 (1) 要當**主視角**的那個畫面，
在目標規模下目前是完全不可用的，而體檢完全沒有抓到（測試環境只有 1 台主機、
blob 只有幾十列，N3 的 1,200 次讀各只要不到 1 ms）。

## 八點六、實作進度（branch `feature/scale-issue-first`）

| 階段 | 狀態 | 備註 |
|---|---|---|
| P0 壓測工具與基準 | ✅ | `LogForesight.Tests/Scale/`，預設不執行（`LF_SCALE_BENCH=1`） |
| P0.5 穩定度止血 | ✅ | Prune 分批／啟動互斥／`/api/health`／慢查詢門檻 |
| 前置 共用元件小修 | ✅ | H2/H3/H4/M9/H5/H6/M1/M4/L9 |
| P1 根因A 主機集合 | ✅ | 別名展開 O(N²)→O(N)（6000 台 8 ms） |
| P2 根因D 批次閘門 | ✅ | M10／S7／X3／X6 |
| P3 根因B 三表落地 | ✅ | S2/S3/S4/S5/N2/N4；單筆標記 6,841 ms → 5 ms |
| P4 根因C 問題聚合 | ✅ | `lf_top_issues` 擴充六欄＋`IIssueAggregateQuery`＋背景回填 |
| P5 根因E 呈現層 | ⚠️ 部分 | 見下方「P5 已完成／未完成」 |
| P6 設定面規模工具 | ⛔ 未做 | X2 授權矩陣／X5 我的交辦分頁——獨立輪次，與需求 (1)(2) 無共用程式碼 |
| 角色走查＋上帝視角體檢 | ✅ | [SCALE-REVIEW-2026-08-06.md](SCALE-REVIEW-2026-08-06.md)，14 項發現 |
| 體檢修復 A～D 批（含 D3 補做） | ✅ | [SCALE-FIX-PLAN-2026-08-06.md](SCALE-FIX-PLAN-2026-08-06.md) 各項含「實作結果」；E1 定案**不拆 worker**，S-3 三項配套落地 |

### P5 已完成

- 依問題視角：涵蓋範圍（期間跨度）／出現密度／是否仍在發生／「重大」旗標
- 儀表板重點問題卡：「新」徽章／影響率／涵蓋範圍／出現密度／變化幅度
- 側欄「我的交辦」未結案數徽章（M7）

### P5 未完成（刻意留下，需要設計決策或成本較高）

| 項目 | 為什麼還沒做 |
|---|---|
| §10.4 首頁改「一張卡三分頁」（今天有什麼不一樣／什麼最急／什麼影響最廣） | 資料已備妥（三種排序都算得出來），差的是分頁殼與三組排序的呈現決策；目前是單一排行卡帶齊全部維度 |
| §10.6 排除「已有結論」的問題 | `IssueRankingDto.OpenHostCount`／`ResolvedHostCount` 與聚合的相異簽章清單都已備妥，差把 `lf_issue_handling` 的逐簽章彙總接上（見 BACKLOG） |
| §10.7 報表趨勢圖改 Anomaly Detection 型 | 需要新的圖表型別與資料序列；氣泡圖已依準則否決（Accessibility B、不適用 mobile-primary） |
| §10.5 每列 sparkline | 規劃時即降為選配——密度與變化幅度已用文字回答同一件事 |
| X1 「只看影響 ≥N 台」「本期新增」快捷篩選 | 資料已備妥（`HostCount`／`IsNew`），差篩選 chip 與後端參數 |

## 九、下一步（六件事已全數定案）

1. **階段切分是否接受**——特別是把 S2（P3）提前到 S3／S4 之前，這與體檢的建議相反。
   → **接受**，2026-08-06「順序依據你的建議執行」；後續實測證明 S2 確實是 OOM 硬失敗（§8.5.1），提前是對的。
2. **P0 要不要先做**——不改任何行為，但它是唯一能證明「修好了」的方法。
   → **先做**。本文件所有「→」前後對比數字皆出自 P0 的基準。
3. **P0.5 要不要立刻開工**——四項止血都很便宜，且與其他階段完全無耦合。
   → **立刻做**（commit `2bd6a5a`）。
4. **前置的共用元件小修要不要先併成一個小輪次**——做完之後 P5 的新畫面天生可鍵盤操作。
   → **先併**（commit `eaecff7`）。
5. **§8.1 缺陷 3 的鍵改成 `host_id`** 是否接受——這會動到遷移邏輯與孤兒處理。
   → **不改，維持 `host_name_key`（大寫正規化）**：處理狀態的既有語意是「以現行主機名稱為鍵」，
   改鍵會動到改名行為且遷移要處理孤兒；正規化欄位已解掉真正的跨後端風險
   （SQLite BINARY vs SqlServer CI 定序），詳見 P3 的實作紀錄。
6. **E1 走甲案（維持同行程＋隔離）** 是否接受，決定點延到 P4 之後。
   → **走甲案**，使用者 2026-08-06 明示「不拆分成獨立 worker，維持在網站上執行排程」；
   三項配套（連線池隔離／平行度上限＋讓出／執行中告示）已在修復 C 批落地
   （SCALE-FIX-PLAN-2026-08-06.md S-3）。
