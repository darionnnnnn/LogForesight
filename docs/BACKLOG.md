...
- **通知管道（下一輪候選首位）**：目前只在 Web「排程作業」頁與排程狀態卡顯示，需要主動查看
  才會發現，高風險執行結果沒有主動推播。下一步可考慮接 Email／Telegram／Teams webhook；
  本系統定位為第二層縱深防禦，即時性要求不如第一層監控，本輪仍未列為優先項，但審查中被
  多次提及，列為候選首位。
...
- **`SlowTrendAnalyzer` 中位數化**：回饋十三輪 E 把 `TrendAnalyzer`（14 天窗口）的基準計算從
  平均值改中位數（單日爆量不再墊高基準），但 `SlowTrendAnalyzer`（長窗口的慢速趨勢，另一套
  獨立計算）本輪刻意不動——兩者是否該用同一套統計量、長窗口下離群值的影響本來就比短窗口稀釋
  得多，需要獨立評估，不隨這輪一併處理。
