...
三項是同一次重構，一起做才不會搬兩次。方向已定案，細節下輪規劃。
觸發條件：認證方式落地後，實機取數驗證通過（見歸檔區的 PRTG 取值測試步驟五關）。

- **PRTG 獨立維護頁 `/admin/prtg`**：與 NetIQ 維護同層級（側欄「系統管理」內、緊鄰 NetIQ），
  呈現方式參考 NetIQ 頁的頁籤結構（連線設定／鏡像狀態／探測）。設定頁的 PRTG 頁籤同時瘦身為
  純參數（併發、回填天數、保留天數）——設定頁只放參數，操作類功能不放在那裡。
- **歷史回填搬到排程作業頁**：回填是排程性操作而不是參數設定，放在設定頁是錯的位置。
- **主機頁面整合 PRTG 對應**：主機清單加「PRTG 對應」篩選（有對應／衝突／未對應），
  讓使用者從篩選就看得出哪些主機接上了 PRTG；主機明細顯示對應到的 device 與其 sensor 清單；
  並提供**手動對應 UI**（把未對應或衝突的 device 指給某台主機）。
  手動對應的結果**必須優先於每日自動對應且不被覆蓋**——同 `lf_prtg_sensors.category`
  「人工結果不被同步洗掉」的既有契約精神；需要在 `lf_prtg_host_map` 之上新增人工對應的
  持久層（自動對應逐日重算、人工對應長期有效），資料模型下輪定。
