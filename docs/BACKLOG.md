...
### 取數範圍上線後待實機確認（優先以環境探測確認）

縮到取數範圍後有三件事只有實機答得出來。環境探測是唯讀、可在啟用前跑、輸出可整段複製的工具，**能放進探測的就放進探測**，
不要靠翻夜間執行紀錄或等隔天看結果。

- **逐裝置查狀態變更是否含底下感測器的訊息**（已可用探測確認）：跑環境探測看「9d-5 逐物件查詢驗證」的結論行。
  「✓」＝現行取法成立；「✗ 只有裝置自身」＝照上方「狀態變更改逐感測器查詢」條目改 `StateChangeQueryObjects`；
  「無法判定」＝樣本裝置近 7 天沒有狀態變更，挑有告警的時段再跑一次。同一項也驗了逐裝置取感測器與 `filter_objid` 分批取值。
- **第一趟同步的清除量與各階段耗時**（已可用探測確認）：看站台對照 [S1] 的「其中範圍外 K 顆」（預期約 4 萬）
  與 [S2] 的估算行；範圍內裝置的逐台結論也在 [S2]。第一趟同步後再以狀態變更完成行的「平均每台耗時」核對估算。
- **資源守門是否仍找得到 PRTG 主機與 Sentinel 主機**（已可用探測確認）：看站台對照 [S3]——PRTG 主機是位址命中還是
  以 Core Health 找到；若列出「不在取數範圍內」的裝置，照該行的處置存入覆寫清單。
- **主機清單的未回報 PRTG 提示**（畫面確認，不適合放探測）：挑一台已對應、近期未回報的主機，確認提示不是「PRTG：無資料」；
  若是，先看該裝置的感測器是否已進鏡像（主機詳情的 PRTG 區塊），新對應的裝置要等快照服務下一輪補抓。

- **同一份資料兩種取法**（觸發條件：單次大 `count` 在實機出現截斷或 HTML 空白頁）：環境探測與資源守門即時來源用單次大 `count`、
  結構同步用分頁（感測器與狀態變更已逐裝置查）。是一致性問題，不影響正確性。
- **狀態變更改逐感測器查詢**（觸發條件：環境探測 9d-5 結論為「✗ 只有裝置自身」，或夜間執行輸出出現「範圍內 N 台裝置近期都沒有任何狀態變更」的提示）：
  只改 `PrtgFetchService.StateChangeQueryObjects` 回傳範圍內未暫停感測器的 objid；請求數會從台數變成顆數，9d-5 印有單顆耗時可估。
...
- **結構同步改 objid 清單比對**（觸發條件：取數範圍超過 500 台、感測器階段改走全站分頁後實測仍超過 5 分鐘）：先以 `columns=objid` 取清單
  （實測全欄位的 12% 時間、8% 位元組）與鏡像比對，只補新增與異動的 sensor 細節。
- **messages 改推送**（觸發條件：夜間第三階段完成行的總耗時實測超過 10 分鐘——狀態變更已逐裝置查詢，量級隨取數範圍台數成長，完成行印有平均每台耗時）：
  由 PRTG 通知（HTTP action）把狀態變更推給站台，取代每晚逐台查 messages。要動 PRTG 端設定，且只解決狀態變更、不解決數值。
...
- **伺服器硬體健康 sensor**（觸發條件：PRTG 端為伺服器加裝硬體健康類 sensor，如 Dell iDRAC、HPE iLO、IPMI）：
  實機 type 清單裡唯一的硬體健康 type 是 `SNMP Cisco System Health`，多掛在網路設備上；只對應伺服器的站台，
  hardware 規則與儲存故障佐證仍不會命中。加裝後把新 type 補進 `PrtgSensorTypeCategoryMap`（或先用補充對照）。
- **非觸發式回填逐日重查狀態變更**（觸發條件：有人以不注入 store／records 的方式呼叫 `PrtgBackfillRunner`——正式路徑一律走觸發式、整趟只查一次）：
  `FetchDayAsync(syncStructure:false)` 每一天都會跑狀態變更階段，逐裝置查詢後請求數是「天數 × 範圍台數」。要修就讓該分支比照觸發式在迴圈前取一次。
- **範圍補抓每輪讀整份感測器鏡像**（觸發條件：取數範圍成長到數千台、快照執行輸出的間隔明顯拉長）：`BackfillScopeSensorsAsync` 每輪 `GetAllSensors()`
  只為了取裝置與 objid 兩個集合，可改成投影查詢。
- **範圍補抓失敗的裝置沒有節流**（觸發條件：快照執行輸出反覆出現「N 台新進取數範圍的裝置感測器補抓失敗，下次再試」）：快照服務的範圍補抓對「查詢失敗」的裝置每輪重試（最多 50 台一輪），
  PRTG 對某幾台持續回錯時每 5～15 分鐘就有一批失敗請求。只有「取回 0 顆」的會記住不再試。
...
- **一個依賴真實 DNS 與計時的偶發失敗測試**：
  `PrtgAddressResolverTests.Resolve_真DNS解析保留網域_回null且在逾時內返回`。
  單獨執行穩定通過，全套負載下偶發（真實 DNS 查詢加上逾時上限，執行緒池飢餓時會超時）。
  修法方向同 Sentinel 預算測試：解析器的逾時改注入 `TimeProvider`、DNS 查詢改替身，斷言強度不變而時間與網路相依消失。
...
三項已落地：獨立維護頁 `/admin/prtg`、同步／歷史回填的啟動入口集中在 PRTG 維護頁（總開關現為維護頁「擷取參數」下拉的一部分，見 PRTG-SPEC §3a）、
主機頁整合 PRTG 對應（清單篩選／明細區塊／人工對應）。現況見 docs/PRTG-SPEC.md §4a 與 §7。


## 慢查詢與系統健康

系統健康頁籤與慢查詢清單已是現行入口；慢操作目前保留最慢前十支，並涵蓋紀錄查詢、問題彙總、
處理狀態、blob、PRTG 鏡像、行式日誌與權限異動的對外查詢。若未來發現 PRTG 批次寫入耗時沒有
對應埋點，再依實機慢查詢資料補埋點，不把「設定頁沒有系統健康」列為待辦。
...
- **夜間派工與 Web 單操作的並行競態**（收尾體檢，機率低、未重現）：夜間派工用開跑時的交辦單快照，執行途中管理者
  取消、代為結案或改派併入某張單，夜間仍可能把新成員掛進那張已結案的單（案件進行中、單已結案，結案掃描不會自癒），
  或在改派後以舊處理人寫入。同型：改派不併入分支撞部分唯一索引時留下「案件已換人、單未換人」；取消與背景逐日同步
  交錯時，背景已算好的逐日寫入可能在取消後落盤。**觸發時機**：出現「案件進行中但所屬單已結案」的資料；
...
- **待派試跑沒有模擬 ① 與 ③**：試跑不看「當日已有標記」與機房結論自動套用，「立即派工」可能替夜間會略過的出現點建單。
  **觸發時機**：待派清單出現已有人標過結論的主機。
- **靜音歷程畫面**：問題檔案 API 已回最近 10 筆 `muteHistory`，沒有畫面消費。**觸發時機**：需要追「這個問題以前靜音過幾次、誰設的」。
- **日層級指派與立即派工的建單通知**：兩條路徑建的單不寄信（夜間派工有彙總信、依問題視角交辦有建單信）。
  **觸發時機**：處理人反映被立即派工卻沒收到通知。
