...
- **細粒度掃描的 128 子句未實測**：`/26` 粒度一次送 64 個位址子句，加上段內排除最多再 64 個，
  合計最多 128 個 OR/NOT 子句。Lucene 預設 `maxClauseCount` 是 1024，結構上安全，但本環境的
  probe 只實證過 OR 50~100 個。試點時要核對是否被 Sentinel 拒絕（被拒的症狀是背景工作 failed，
  訊息來自 Sentinel）。**`/25` 粒度刻意不提供**：它一次要 128 個位址子句＋最多 128 個排除子句
  ＝ 256 子句，遠超實證範圍；而拆成兩個 64 子句的查詢送出後，它與 `/26` 在查詢層面完全等價，
  多這個選項只會讓人以為有第三種涵蓋粒度。
- **掃描工作只存行程內記憶體**：`NetiqDiscoveryService` 的背景掃描工作是靜態字典，
  站台重啟即消失（前端會顯示「工作不存在或已逾期」並允許重新掃描）；且**全站同時只允許一個**
  進行中的掃描。多行程／IIS 應用集區回收的情境未處理——掃描可重跑，刻意不引入持久化。
- **`/16` 配 `/26` 粒度會被段數上限擋下**：展開為 1024 段、超過 `MaxSegments`（512）。
  細粒度的設計用途是「已知某個 /24 特別吵時針對它掃」，不是拿來掃整個 /16。
...
### `lf_daily_records` 缺 `(record_date, risk_level)` 索引
...
- **權限異動量增後的效能未實測（查詢與寫入兩側）**：權限異動不設每主機日筆數上限，吵雜 DC 單日可達數萬則（實例 9.2 萬）。①`/permission-changes` 的查詢與分頁在這個量級下是否仍可接受尚未實測。②寫入側兩處的量級假設同樣失去上限保護：pipeline 的去重鍵快照（回望上限＋7 天窗口的 `ConcurrentDictionary`，數萬則/日 × 數十天＝數百萬鍵常駐記憶體）與 `AppendChanges` 整個主機日單次 `SaveChanges`。若實測出問題，處理方向是索引/查詢形狀、快照改分批或布隆式結構、寫入分批，另案一輪。
