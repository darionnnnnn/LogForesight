...
**跨來源佐證三模式**＋未回報主機 PRTG 提示＋跨後端資料搬運＋本輪磁碟趨勢規則、逐 sensor readiness、語意證據、局部回填及使用效果指標
（見 docs/PRTG-SPEC.md）。SQL Server 僅驗證 EF `ToQueryString` provider 翻譯，未連線實際 SQL Server。模擬整合／SQLite 與程式測試不代表正式站台或真實 PRTG 驗證。

**正式站台磁碟校準仍待真實 PRTG 28 天資料及頻道／單位語意驗證**：目前觀察到約 6 天資料且 `Unit=null`；真實 28 天 coverage 根因、sensor 頻道／方向與預警效果尚待確認。
...
- **值型規則**：基線偏移、CPU／記憶體持續高檔仍未實作。`disk_free_trend` 已接入日常 finding 管線，並有逐 sensor readiness、語意證據、預設停用規則、啟用後端守門及規則頁草稿唯讀試算（依目前抑制設定估計，不含靜音與實際派工）；但真實 PRTG 28 天資料、現場頻道／單位、門檻與效果仍待實機驗證。目前約 6 天、單位 null 的匯出不足以作為通過證據；readiness 摘要最多抽樣 100 顆，四項共通校準不作此規則總閘門（規則頁現況見 PRTG-SPEC §9a）。
...
- **正式站台校準後的門檻調整**（觸發條件：累積並核實真實資料，PRTG-SPEC §9a／§11）：(1) 磁碟趨勢門檻；
...
- **`PrtgProbeRunner` 只有 sensor type 分布那一步有「取樣數 < 總數」的截斷警告**，
  ...
  探測結論失真。
- **PRTG 取數與探測的 JSON 解析輔助仍有重複**：`PrtgFetchService` 與 `PrtgProbeRunner` 各自維護 `GetStringProperty` 及相關相依性判定；若後續格式／判定調整，需同步檢查兩處。
