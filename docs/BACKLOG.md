...
`ActionableOccurrences`），但它的下鑽連結帶 `view=issue&riskLevels=高,中`，而依問題視角的
chip 是**問題嚴重度**（見 WEB-SPEC §10）——同一組值在兩端指的不是同一件事，數字不會相等。
風險類型卡沒有這個問題（它帶三個等級＝不篩嚴重度，母體又與列表相同）。

要對齊得讓依問題視角能表達「只看某些日風險等級的主機日」，也就是多一個與嚴重度分開的
