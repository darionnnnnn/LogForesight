...
儀表板「未處理問題」卡的數字母體是**可行動風險日（日風險高／中）**（`IssueTodoQuery`／
`ActionableOccurrences`），依問題視角的母體則是全站「日風險等級顯示」設定允許的等級。
下鑽連結已改帶三個等級（＝不按嚴重度篩），所以在常見設定（低風險日設為不顯示）下兩者相等；
但若設定把低風險日設為顯示，下鑽會多出「只在低風險日出現」的未處理問題。

要完全對齊得讓依問題視角能表達「只看某些日風險等級的主機日」，也就是多一個與嚴重度分開的
查詢參數；那會讓同一排 chip 再多一種語意，值得連同「依問題視角要不要有獨立的日風險篩選」
一起決定，不適合順手加。
