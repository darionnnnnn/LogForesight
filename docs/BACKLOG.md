...
- **第一趟同步的清除量與各階段耗時**（目前只能看同步的執行輸出；探測待補）：預期清掉約 4 萬列範圍外感測器，
  狀態變更完成行印有查詢台數與平均每台耗時。**探測待補「取數範圍試算」步驟**：探測目前只拿得到 PRTG 連線、讀不到主機主檔與鏡像，
  要讓探測服務把 `PrtgScopeDevices.Compute` 的結果（範圍台數與四項組成）與鏡像現有感測器數傳進去，印出
  「範圍 N 台（對應 a、衝突 b、人工 c、守門 d）／鏡像感測器 M 顆、其中範圍外 K 顆（下次同步會清除）」，
  並以 9d-5 量到的每台耗時 × N 估出感測器與狀態變更兩階段的時間——升級前就看得到會清多少、要跑多久。
- **資源守門是否仍找得到 PRTG 主機與 Sentinel 主機**（目前只能到設定頁按「自動偵測並填入」的預覽；探測待補）：
  **探測待補「守門目標偵測」步驟**：以直接查 PRTG 的來源（`PrtgLiveGuardSource`）跑一次 `PrtgResourceGuardTargets.Resolve`，
  印出命中的裝置、各分類感測器數、PRTG 主機是位址命中還是走 corehealth fallback，以及這些裝置在不在取數範圍內
  （不在＝夜間讀鏡像會偵測不到，要先存入覆寫清單讓範圍補抓把它們帶進鏡像）。
