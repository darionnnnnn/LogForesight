...
| E2b snapshot service | agy (gemini-3.8-flash-high) + Claude fix | done | Claude verified: 3 files in whitelist, BOM/CRLF/NUL ok, snapshot tests 12 green, 2 mutations each 1 red, full suite green (one Sentinel timing flake, rerun isolated 47/47 x2, unrelated) | Claude caught 2 defects tests missed: (1) backoff was a no-op - interval measured from last SUCCESS, so when PRTG stays down last-success is frozen in the past and the service hit PRTG every 60s; fixed to measure from last ATTEMPT, regression test confirmed red on old code first. The existing backoff test only passed because of this bug (3 ticks at one instant); now advances the clock. (2) ExecutionOutputs unbounded -> memory leak on long-running site; capped at 100 with test. |
