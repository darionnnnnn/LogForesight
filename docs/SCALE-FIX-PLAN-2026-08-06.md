...
> **狀態：只規劃，未修改任何程式碼。** 對象是
> [SCALE-REVIEW-2026-08-06.md](SCALE-REVIEW-2026-08-06.md) 找到的 14 項問題。
> 基準：分支 `feature/scale-issue-first`，1448 測試綠。
>
...
| 批次 | 內容 | 閘門 |
|---|---|---|
| **A** | D1、D2（＋G3 測試）、D4、D5、D6 甲案 | 純錯誤修正，改完即可合併 dev |
| **B** | S-1／G1／S-2／G2 整組（§三） | **需要造大 blob 實測**才算完成；這是唯一可能丟資料的一組 |
| **C** | W1、D7＋H1、S-3 | 需要你確認 D6 走甲或乙、W1 是否接受 sticky 欄位 |
| **D** | G4、S-4（含 DB-SPEC 更新） | 上線前完成即可 |
...
2. **W1 是否接受「動作欄 sticky 固定在列末」？**
   這會改變依問題視角與主機清單的視覺行為（捲動時該欄不動）。
3. **G4 的處理歷程保留天數要跟稽核（730 天）還是執行歷程（90 天）？**
   我建議跟稽核——它是追責證據，不是執行紀錄。
4. **B 批要不要先造 100 萬列的舊格式 blob 做升級實測？**
   不做的話，S-1／G1 的修復本身也只是推論；而這一組正是唯一會丟資料的。
