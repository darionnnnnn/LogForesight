...
> **狀態：批 0／批 1／批 2 已實作完成**（2026-08-06，commit `6918d6b`／`961763b`／`b32a0ea`，
> 分支 `feature/scale-issue-first`，1531 測試綠）。丁（背景掃描）依決策暫緩，設計保留 §四。
> 實作與規劃的差異、以及過程中抓到的既有 bug 記在 §八。
>
> **決策已於 2026-08-06 兩輪回覆定案**（見 §七）：
...
| **手段二：重掃已知排除** | 重掃時 `NOT` 掉已登錄主機——無新機時兩次查詢各回 0 筆就收工 | 併入乙 | **第 1 批**（§3.3） |
...
- ~~計數查詢保留但改用**窄化 filter**，用途只剩 CoverageNote 的量級資訊與預檢~~
  → **實作時整個移除**（見 §8.1）：它的唯一用途是驅動自適應窗口，窗口固定之後
  就只剩「放一個數字到 CoverageNote」，不值得一次完整的 job 往返；真要 `found`，
  主掃描第一輪的結果本來就有。
...
3. 真實環境試點：同一網段改前/改後對照——耗時降、台數不減；
   重掃第二次（無新機）應在數秒內結束（主掃描與補充掃描各一次查詢、各回 0 筆）。
...
| 批 | commit | 內容 |
|---|---|---|
| 0 | `6918d6b` | `Program.cs` 啟動時 `Console.OutputEncoding = new UTF8Encoding(false)`，包 `IOException`（Windows 服務無主控台）。**未動 nlog.config**（如 §6.3 決策）。實測啟動輸出中文正常，含 NLog console target 的訊息 |
| 1 | `961763b` | 窄化 filter＋殘差輪掃＋補充掃描帶排除＋重掃已知排除；**移除自適應窗口壓縮整段**。測試 +21 |
| 2 | `b32a0ea` | `SentinelEsmDirectory` 防禦性解析器＋per-Sentinel `UseEsmDirectory` 開關（預設關）＋UI 與說明文字。測試 +26 |

### 8.1 與規劃的差異（各有理由）

| 差異 | 規劃原本寫的 | 實際做的 | 為什麼 |
|---|---|---|---|
| **移除計數查詢** | §3.5「計數查詢保留但改用窄化 filter」 | **整個移除** | 計數查詢的唯一用途是驅動自適應窗口，窗口固定之後它就只剩「在 CoverageNote 放一個數字」——不值得一次完整的 job 建立/輪詢/刪除往返。需要 `found` 時主掃描第一輪的結果本來就有 |
| `ListHostsAsync` 介面 | §3.3 稱「UI 零改動」 | 介面**小改**（新增選填 `knownIps`，預設 null） | 討論時說得太滿。既有呼叫端零改動（預設值），但介面簽章確實動了；精靈 UI 才是真的零改動 |
| ESM 觸發時機 | §5.3「開關開啟時每次先試」 | 同左（維持） | 但改由 `SentinelServer.UseEsmDirectory` 承載，需在 `SentinelConnectionFactory.ToConnectable` 一併帶過去 |

### 8.2 過程中抓到的既有 bug（不在原規劃內）

**`SentinelStore.Upsert` 更新分支是逐欄複製**，新增模型欄位若忘了在那裡加一行，
症狀是「**新增存得進去、編輯被靜默還原**」。更糟的是測試替身 `FakeSentinelStore`
有一份同樣漏抄的複製邏輯，所以單元測試會全綠——`UseEsmDirectory` 正是這樣漏掉，
靠瀏覽器手動驗證才發現（API 回 `useEsmDirectory: false`）。

已修 store 與替身，並新增 `SentinelStoreRoundTripTests` 對**真實 store** 做
**整物件**往返比對（不是逐欄挑幾個測）：從「全部欄位都不一樣」的既有資料更新過去，
任一欄沒被複製都當場失敗。**已實測「移掉修正 → 該測試失敗」**，確認它守得住。

同樣的形狀存在於其他 `JsonBlobCollection` store 的 Upsert（主機、群組、使用者…），
本輪沒有一併掃——那是獨立的體檢題目，記在這裡供日後參考。

### 8.3 尚未驗證的（要等真實環境）

1. **`NOT` 子句是否被此環境接受**——殘差輪掃與重掃增量都靠它。已加偵測
   （取回事件含已排除的 `repip` 即停止輪掃＋顯性警告），試點時核對有沒有跳過這則警告。
2. **窄化 filter 的實際效果**——`System`/`Application` 佔比 0.05% 來自單一主機樣本，
   不同角色的主機比例可能不同。驗收方式見 §3.7：同網段改前/改後對照，耗時降、台數不減。
3. **ESM 回應格式**——本環境拿不到 200，解析器的候選欄位清單全部未經實測。
   拿到真實輸出後要存 fixture、收斂候選、更新 `NETIQ-API-REFERENCE.md §3.5`。

### 8.4 全面體檢（2026-08-06 收尾，實作後逐項複查）

體檢抓到並修正四件事：

1. **`SentinelAdminService.SetActive` 也漏抄 `UseEsmDirectory`**——它逐欄重建
   Sentinel 物件只為了改 `Active`，新欄位沒跟上：停用再啟用一台 Sentinel，
   ESM 開關被靜默重置。這是 §8.2 那個 bug 家族的**第二個實例**（同一輪、同一個
   欄位、不同的複製點），補測試 `停用再啟用_不重置ESM開關` 釘住。
2. **`SaveSentinelRequest.UseEsmDirectory` 改為 `bool?`**——原本是 `bool`，
   編輯時省略＝被重置成 false，與 `Os`「省略＝沿用既有值」的語意不一致；
   API 呼叫端漏帶一個欄位就靜默關掉開關，是同一個病的 API 層變形。
   對齊 Os 慣例：null → 沿用既有值（新增時 false）。
3. **補充掃描的子句上限退路落實 §3.2**——原實作在主掃描已見清單超過排除上限時
   直接跳過補充掃描（整段棄守）並多發一則重複警告；規劃寫的是「退回無排除版」。
   已改為第一輪無排除照掃（混入的已知主機事件由去重吸收，仍可能補到未知主機），
   仍截斷才警告。
4. **死碼移除**：`FormatWindow`（窗口描述隨自適應窗口一起失去用途）與
   靜態版 `PickMostCommonHostName`（職責已由 `ScanState` 內建版承接）。

文件同步：WEB-SPEC §9.9a 掃描段落仍描述「自適應時間窗」為現行機制，已改寫為
涵蓋保證版；全文件掃過「自適應」殘留（BACKLOG 與 API-REFERENCE 的皆為歷史對照
語境，保留）。
