...
> **狀態：只規劃，未修改任何程式碼。四項決策已於 2026-08-06 回覆定案**（見 §七）：
> ESM 權限現階段要不到 → 甲改為「保留能力、每台 Sentinel 手動開啟」；
> 補充掃描參數依評估採 60 分鐘/10,000 筆；丁定案要做，觸發點=新增 Sentinel 後；
...
| 方案 | 一句話 | 前置條件 | 定案處置（2026-08-06） |
|---|---|---|---|
| **乙：filter 窄化** | 用低量頻道當「主機存在的探針」，成本從正比事件量變成正比主機數 | 無 | **第 1 批** |
| **丁：背景掃描＋快取** | 新增 Sentinel 即背景掃描，精靈看快取，不再互動乾等 | 建立在乙之上 | **第 2 批**（定案要做，觸發點=新增/更新 Sentinel＋每晚刷新） |
| **甲：ESM 目錄** | 一個 GET 換完整清單（含安靜主機） | ESM 唯讀權限 | **第 3 批**——權限現階段要不到，改為「保留能力」：每台 Sentinel 一個開關、預設關，有權限的環境自行開啟（§五） |
| 丙：分散取樣 | 24h 切 N 段各拉少量，涵蓋分布更好 | — | 不排——乙已把窗口撐回 24h |
| 戊：AD 電腦物件 | 權威清單在 AD 不在 Sentinel | — | 不在本規劃——不同的問題（存在 vs 有回報），獨立評估 |

順序調整的理由：丁已定案且不被權限卡（提前到第 2 批）；甲降到第 3 批——
權限要不到代表**本環境無法驗證回應形狀**，只能做防禦性實作＋手動開關，
價值兌現要等有權限的環境出現，自然排最後。三批相互獨立、可依序合併。
...
## 四、第 2 批（丁）：探索背景化＋結果快取（已定案要做）

**定案的觸發語意（2026-08-06）：「新增 NetIQ（Sentinel）後開始背景掃描」。**
...
## 五、第 3 批（甲）：ESM 目錄——保留能力、每台 Sentinel 手動開啟
...
背景掃描走同一個 `ListHostsAsync`，開關開啟時自動享受 ESM；
快取條目記錄來源（`目錄` vs `事件掃描`），精靈顯示得出來。
...
| 1 | 乙：filter 窄化＋補充掃描（60 分鐘/10,000 筆）＋文件 | 一個 | 無 |
| 2 | 丁：Sentinel 探索網段欄位＋背景掃描＋快取＋精靈快取優先 | 一個 | 批 1（背景掃的就是批 1 的掃描） |
| 3 | 甲：ESM 防禦性解析器＋per-Sentinel 開關（預設關）＋文件 | 一個 | 批 1（退路）；**驗收**要等有 ESM 權限的環境 |

### 決策紀錄（2026-08-06 使用者回覆）

1. **ESM 權限**：現階段要不到 → 甲改為「保留能力」設計：每台 Sentinel 一個
   `UseEsmDirectory` 開關、預設關；解析器防禦性實作（成功閘門＋退路），
   有權限的環境依 §5.1 的人工驗證流程開啟。原「每次掃描先試 ESM」作廢——
   不能自動信任沒驗證過的解析。
2. **補充掃描參數**：依評估採建議值 **60 分鐘窗／10,000 筆上限**（理由 §3.1）。
3. **丁**：定案要做，觸發點=**新增/更新 Sentinel（填了探索網段）即背景掃描**
   ＋每晚分析後刷新（§四）。前提是 Sentinel 登錄表單要新增「探索網段」欄位——
   系統無法猜一台新 Sentinel 管哪些網段。
