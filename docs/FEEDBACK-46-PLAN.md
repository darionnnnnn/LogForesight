...
| C-2 | impl-low（Opus 5 low） | 通過（12→16；全套 4236 綠） | grep 驗收全過；執行端突變「事件列不排除 PRTG」「不排除已抑制」「上限 21」皆紅 | `WeeklyCheckupService.cs` 在 dev 為 LF，執行端整檔轉成 CRLF 卻回報維持原樣——Claude 轉回 LF |
