...
**全量測試**：通過 **3375**、略過 6、總計 3381（基準 dev：3296 通過／略過 6，淨增 79）。
...
**尚未實作的定案項**（判定為可接受，明列以免當成漏做）：
- 校準狀態的 10 分鐘記憶體快取未做。每次按「重新計算」都重跑，且匯出時會再跑一次判定。
  以手動觸發的低頻操作而言可接受；真的嫌慢時再加。
- 分母為零時的補充說明沿用通用文案（「目前 0 天，還需要 N 天」），
  未特別區分「期間無事件，門檻無從校準」。判定結果正確，只是文案不夠精準。
