...
|---|---|---|---|---|
| 靜音 | 這個問題暫時不用看，到期自動恢復 | 問題檔案、問題查詢「依問題」 | 全機房（一個 Source＋EventId）；1～365 天 | 分析側標 `Suppressed`；讀取側整個問題不列、不進待辦、不派工；到期自動回來 |
...
**靜音與抑制的實作關係**：靜音區間存在問題檔案（`IssueProfile.Mutes`），分析側以包裝層
