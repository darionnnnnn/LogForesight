...
- **`NoiseMark`／`INoiseMarkStore`**（webdata blob，主機＋簽章為鍵，見 docs/WEB-SPEC.md §9.4）：
  **處理狀態層級**的記憶，純 Web UI 便利——標記後，之後同主機同簽章的新問題在畫面上自動顯示
  「已知雜訊（自動）」，省去每天重新標記同一個問題的處理狀態。**不影響風險判定**：
  不改變風險等級、不讓問題從「重點問題」清單消失、不影響 `Suppressed` 旗標——單純是
  「這個問題我看過了，知道是雜訊，處理狀態幫我自動填」。唯一的批次側效果是**派工閘門**：
  夜間續掛、問題負責人與自動派工都略過有 `NoiseMark` 的（主機, 簽章），不為已知雜訊派工
  （docs/WEB-SPEC.md §9.4b）。
...
四種工具的層級與期限不同，選錯會變成盲區或白做。畫面上同一份表放在規則頁「告警抑制」頁籤頂端與
設定頁「自動派工」段（文字只寫在 `core/alert-tools-table.js`）：

| 工具 | 適用情境 | 在哪裡設定 | 範圍與期限 | 效果 |
|---|---|---|---|---|
| 靜音 | 這個問題暫時不用看，到期自動恢復 | 問題負責與靜音、問題查詢「依問題」 | 全機房（一個 Source＋EventId）；1～365 天 | 分析側標 `Suppressed`；讀取側整個問題不列、不進待辦、不派工；到期自動回來 |
| 抑制 | 某些主機或群組的這個問題永遠不告警 | 規則維護「告警抑制」 | 主機／群組／全站；天數或永久 | 只關通知與風險升級，事件照常聚合與紀錄，畫面照常列出 |
| 統一標記 | 這個問題在沒人接手的主機上一次下結論 | 問題查詢「依問題」 | 查詢期間內一次性；可勾「之後新日子自動套用」 | 寫結案類處理狀態（機房結論） |
| 不再打擾 | 這台主機的這個問題已知，別再派工 | 風險日詳情把問題標為不處理、誤報或已知雜訊 | 單一主機 | 之後再出現不自動派工（resolved 除外） |

**靜音與抑制的實作關係**：靜音區間存在問題負責與靜音（`IssueProfile.Mutes`），分析側以包裝層
`MuteAwareSuppressionStore` 在讀取時合成 `IssueMute` 型抑制項目、寫入時濾除，只包在分析側的
入口（`AnalysisOrchestrator`、`PrtgDailyPipeline`）——所以 `SuppressionFilter.MarkSuppressed` 以**紀錄日**判定靜音（重新分析舊日子永遠同答案），
規則頁的抑制清單看不到它，`ActiveForHost` 等「現在生效中」的篩選也排除它（體檢與到期提醒不會把
過去的區間當成生效中）。報告 txt「已抑制」段反查原因時多一條路：「靜音至 yyyy-MM-dd：原因」。
