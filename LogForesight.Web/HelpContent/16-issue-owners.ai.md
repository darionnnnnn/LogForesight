...
- **視窗標題（`#issue-owner-modal-title`）**：新增時為「新增問題設定」，編輯時為「編輯問題設定」。
...
- **儲存送出邏輯（`form.submit`）**：
  ...
  5. 顯示 Toast「已儲存問題負責與靜音」，關閉彈窗並重新載入列表。

## 系統業務運作與聯鎖機制

1. **夜間批次分析自動交辦**：
   - 當夜間批次分析於某主機某日產出問題、這台主機還沒有人處理且沒有續掛單接手時，若該問題命中 `IssueProfile` 且設有 `OwnerUserIds`，系統把這台主機掛進負責人此問題的進行中交辦單，沒有就建一張來源為「負責人規則」的交辦單（進該員「我的交辦」頁）。
   - 多位負責人時交給手上進行中台數最少的人；停用、沒有處理能力、暫停接單的負責人不選，全部不可用時交給自動派工。
   - 派工閘門同樣適用：問題靜音中、命中告警抑制、該主機標過已知雜訊、嚴重度不在未處理計算等級、上次以不處理類結案（不再打擾）時都不派。
2. **警報郵件路由優先權**：
...
4. **機房結論自動套用（`AutoApply`）**：
   - 若問題負責與靜音設定了機房結論並開啟 `AutoApply`，夜間分析於新主機日產出該問題時，會自動將該問題的狀態標記為指定結論（如 `known_noise`），並填入設定的原因與標註系統自動套用。
   - 該問題在報表與儀表板的重點排行計算中，若所有受影響主機均已有結論，會被自動排除出重點排行版面，減少背景雜訊干擾。
5. **問題靜音**：
   - 靜音區間存在問題設定中（`Mutes`：起日、迄日、原因、設定者），`PUT/DELETE /api/admin/issue-owners/{source}/{eventId}/mute`，需 `Maintain`。
   - 時長 1～365 天；今天已在靜音中再設定＝重設迄日；解除＝迄日改成昨天。
   - 靜音中：問題不列在清單、卡片、排行，不進待辦，不派工，分析側標為已抑制（不拉抬日風險、不進 AI 敘事、不觸發負責人通知）。到期後，區間內的日子維持已有結論，區間前未處理的日子回到待辦。
   - 既有進行中交辦單可選暫停（到期自動恢復）或代為結案為不處理（需 `Assign`＋`Handle`）。
6. **解除結論之非回滾特性**：
...
7. **稽核軌跡**：
   ...
   - 刪除問題設定：記錄動作 `issue_owner_delete`，摘要註明原負責人名單。

## 常見問答與邊界狀況（Q&A）

- **Q: 為什麼新增問題設定時沒有看到設定機房結論的選項？**
  - **A**: 負責人管理與機房結論是兩支職責分立的 API。新增模式下系統尚未持有該問題的 `(Source, EventId)` 主鍵，因此機房結論區塊在新增時隱藏。請先儲存負責人規則，建立問題設定後，再次點擊列表的「編輯」按鈕即可在彈窗下方看到並設定機房結論。
...
- **Q: 刪除問題設定後，該問題產生的告警會由誰處理？**
  - **A**: 刪除問題設定後，該問題不再有跨主機的專屬負責人，夜間分析產生的告警會落回「主機負責人」規則處理（若該主機有設定負責人）。若該問題曾設定過機房結論，刪除問題設定時機房結論也會一併被移除。
