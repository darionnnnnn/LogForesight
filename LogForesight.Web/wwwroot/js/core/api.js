 ...
 *
 * 回傳三態：true／false 是後端明確回答；**null＝還沒問到或這次沒問到**（失敗不寫入快取，
 * 同一頁之後還有機會取到）。只做真值判斷的呼叫端把 null 當不可用——AI 是加值功能，寧可少顯示
 * 一顆按鈕，也不要讓使用者按下去才發現後端根本沒設定；排程頁比對明確的 false，網路偶發失敗
 * 才不會被畫成「AI 服務未設定」還附上設定頁連結。
 */
