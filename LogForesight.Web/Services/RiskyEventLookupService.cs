...
/// <summary>
/// 詢問 AI 現場事件取得：先查風險 log 暫存（<see cref="IRiskyEventStore"/>，毫秒級、
/// 本機與 NetIQ 主機皆有），查無才 fallback 既有的 Sentinel 即時查詢
/// （<see cref="ISentinelEventFetcher"/>，僅 NetIQ 主機、開關/節流語意不變）。
///
/// 暫存資料只涵蓋「規則命中或趨勢異常」的簽章（見 <see cref="RiskyEventSelector"/>）且有
/// 保留天數上限，查無不代表沒有訊號，只代表這裡沒有——正確的下一步是照舊 fallback，
/// 不是回傳「找不到」。
/// </summary>
public class RiskyEventLookupService
{
