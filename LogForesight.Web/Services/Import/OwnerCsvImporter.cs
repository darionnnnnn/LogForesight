...
public class OwnerCsvImporter : ICsvImporter
{
    ...
    public ImportPlan BuildPlan(CsvTable table, string fileName)
    {
        ...
        if (resolved.Count > 0)
        {
            // 帳號自動建立的明細走 NewUsers（預覽以資訊框呈現）；這裡講的是套用後對方會拿到什麼。
            // 改版前這則警告寫的是相反的事（「負責人不會自動取得檢視權限」）——§2b 起
            // 負責人本身即授權路徑（IVisibilityService）與能力來源（IdentityService），
            // 把它留著會讓管理員以為還要去授權矩陣補一刀。
            plan.Warnings.Add(
                "套用後，負責人會自動取得所負責主機的檢視權，並具備處理狀態維護權限；" +
                "已在線上的使用者需重新登入才會取得處理權限（檢視範圍即時生效）。");
        }
