...
public class WorkOrderStoreTests : IDisposable
{
    ...
    [Fact]
    public void 已交辦主機數_只算進行中且已連單的案件()
    {
        var efCases = new EfIssueCaseStore(_fx.NewContext);
        var ef = Store();
        var fakeCases = new FakeIssueCaseStore();
        ...
        foreach (var (orders, cases) in new (IWorkOrderStore, IIssueCaseStore)[] { (ef, efCases), (fake, fakeCases) })
        {
            var diskOrder = orders.Insert(NewOrder(1, "Disk", 153));
            var netOrder = orders.Insert(NewOrder(1, "Network", 99));

            void AddCaseMember(string caseId, string hostName, string source, int eventId, int entryType, long? workOrderId, DateTime? closedAt)
            {
                cases.Save(new IssueCase
                {
                    CaseId = caseId,
                    HostName = hostName,
                    IssueKey = $"System|{source}|{eventId}|{entryType}",
                    IssueLabel = $"{source} {eventId}",
                    Status = IssueHandlingStatuses.InProgress,
                    HandlerId = 1,
                    FirstLinkedDate = Base,
                    LastLinkedDate = Base,
                    CreatedAt = Base,
                    CreatedByAccount = "admin",
                    UpdatedAt = Base,
            ...
            // 1. 進行中且連單的主機 A
            AddCaseMember("c1", "hostA", "Disk", 153, 1, diskOrder, null);
            // 2. 進行中且連單的主機 B
            AddCaseMember("c2", "hostB", "Disk", 153, 1, diskOrder, null);
            // 3. 同一主機 A 的第二個簽章（entryType 2）且主機名大小寫不同（HOSTA）：只算一台，不重複計數
            AddCaseMember("c3", "HOSTA", "Disk", 153, 2, diskOrder, null);
            // 4. 已結案案件：不算已交辦
            AddCaseMember("c4", "hostC", "Disk", 153, 1, diskOrder, closedAt: Base);
            // 5. work_order_id 為 null 的案件：不算已交辦
            AddCaseMember("c5", "hostD", "Disk", 153, 1, null, null);
            // 6. 別的問題（Network 99）：進行中且已連單，不混入 Disk 153
            AddCaseMember("c6", "hostE", "Network", 99, 1, netOrder, null);

            var queryIssues = new[] { ("disk", 153), ("Network", 99), ("Other", 404) };
            var counts = orders.CountAssignedHostsByIssue(queryIssues);

            var diskKey = (EfWorkOrderStore.SourceKeyOf("disk"), 153);
            var netKey = (EfWorkOrderStore.SourceKeyOf("Network"), 99);
            var otherKey = (EfWorkOrderStore.SourceKeyOf("Other"), 404);

            Assert.Equal(2, counts[diskKey]);
            Assert.Equal(1, counts[netKey]);
            Assert.False(counts.ContainsKey(otherKey));
            Assert.Equal(2, counts.Count);
        }
    }

    [Fact]
    public void 已交辦主機數_空集合不查詢()
    {
        var ef = CountingStore();
        var fake = new FakeWorkOrderStore(new FakeIssueCaseStore());
        ...
        {
            _counter.Readers = 0;
            var counts = orders.CountAssignedHostsByIssue(Array.Empty<(string, int)>());
            Assert.Empty(counts);
        }

        Assert.Equal(0, _counter.Readers);
    }
}
