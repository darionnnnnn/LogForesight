...
/// 可跑在資料庫後端，驗證「換 EfJsonBlobStore 底層、store 邏輯不變」逐位一致。
/// </summary>
public sealed class EfSqliteFixture : IDisposable
{
    ...
    public EfJsonBlobStore Blob(string key) => new EfJsonBlobStore(NewContext, key);

    public EfJsonLogStore LogStore(string key) => new EfJsonLogStore(NewContext, key);
