...
├── Persistence/        持久層抽象：讀寫介面＋三種後端實作（Sqlite/SqlServer/Jsonl，見下）。
...
│                       儲存後端三選一（Sqlite 預設/Jsonl/SqlServer，見 docs/WEB-SPEC.md §10.5）
...
`Program.cs` 的 `RetentionDays = 120`，每次啟動先清除超過 120 天的舊紀錄，檔案不會無限增長。
...
| `Storage.Type` | `Sqlite` | 儲存後端三選一，預設 `Sqlite`（測試/開發用單一 `.db` 檔真資料庫，不寫 JSON，現為主要測試方式）／`Jsonl`（現行檔案格式，單機檔案相容模式）／`SqlServer`（正式環境，2000 台量級）。SQL 模式下全部資料走 DB；`StorageFactory` 是唯一路由點，分析邏輯不需異動。批次與 Web 兩端須設相同值。詳見 docs/WEB-SPEC.md §10.5 |
...
- 27B/31B 級模型單次回應可能需數分鐘。首次執行回補 120 天時每天都呼叫 AI（總覽＋風險日的
  深入分析），總時間可能達數小時甚至更久，屬預期行為（品質優先）。
...
- **DB 後端（2026-07-23 完成；2026-07-24 起 Sqlite 改為預設與主要測試方式）**：`Storage.Type` 三選一——
  `Sqlite`（測試/開發，預設）／`Jsonl`（檔案，單機相容模式）／`SqlServer`（正式，2000 台量級）。
  SQL 模式下**全部資料**（分析紀錄＋webdata）走資料庫：分析紀錄以
