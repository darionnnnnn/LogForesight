...
透過本機 AI 模型（llama.cpp + Gemma 26B/27B 級 MoE 小模型）分析 Windows Server 的 Event Log，
...
LogForesight/
├── Program.cs
├── Service/         有狀態、會做 I/O 的服務：呼叫 AI API、讀 Event Log、每週體檢、權限監控
...
LogForesight.Tests/    規則層/趨勢層/關聯層的單元測試（xUnit），規則表新增規則時測試自動涵蓋
```
...
```
┌─────────────────┐   ┌──────────────┐   ┌────────────────────┐
│ EventLogService  │──▶│ LogAggregator │──▶│ KnownIssueCatalog   │
│ System/App/      │   │ 分組聚合統計   │   │ 規則分類 + 嚴重度    │
│ Security 三來源   │   └──────────────┘   └─────────┬──────────┘
└─────────────────┘                                  │
                                                     ▼
                       ┌──────────────────┐   ┌────────────────────┐
                       │ LogHistoryService │──▶│ TrendAnalyzer       │
                       │ history.txt(JSONL)│   │ 頻率比對 vs 前日/平均 │
                       └────────▲─────────┘   └─────────┬──────────┘
                                │                       ▼
                                │             ┌────────────────────┐
                                └─────────────│ LogAnalysisService  │
...
三個日誌來源平行掃描、一次取回整個區間的事件並按日分桶 → 逐日分析：聚合統計 →
規則標記已知危險訊號 → 與歷史做頻率比對（首次出現/頻率上升自動升級嚴重度）→
連同比對結果與近 14 天歷史組成 prompt → AI 判讀（JSON 格式/內容檢查未過自動重問）→
解析風險等級 → 寫回歷史資料庫 → console 示警**。
...
所有能確定性計算的判斷都由程式先做掉，模型只負責語意解讀）：

| 層 | 負責 | 為什麼 |
|---|---|---|
| **規則層** (`KnownIssueCatalog`) | 比對已知危險事件（Source + Event ID + 次數門檻），確定性命中 | 已知模式用規則抓，100% 召回、零成本，不賭小模型會不會漏看 |
...
| **AI 層** (Gemma) | 解讀前三層算好的結果（為什麼上升、關聯代表什麼）、評估未涵蓋的新型態事件、產生人話摘要與建議 | 這些需要理解語意與上下文，是規則做不到的 |

四層結論取較嚴重者：規則或關聯鏈命中 Critical → 風險強制「高」；
趨勢層有頻率異常或關聯層有任何訊號 → 風險至少「中」。即使 AI 判斷輕忽也不影響告警。
...
會壓縮成一行 JSON 存入歷史，下次分析時把近 14 天的統計餵給 AI，讓它回答：
**這是新問題？重複發生？還是正在惡化？**
...
## 每週體檢（WeeklyCheckupService）

除了每日分析，另外每週做一次「週對週」回顧，找出單看每天都不明顯、但整週合起來是持續累積
或緩慢惡化的訊號——補上 `TrendAnalyzer` 單日 2 倍門檻比對的盲點：如果攻擊者或硬體劣化採取
每天小幅加量的節奏，會一直躲在每日比對之下，但週對週看得出斜線。

- **觸發時機**：`appsettings.json` 的 `Analysis.WeeklyCheckupDay`（預設週六）；
  任何一次執行若發現距上次體檢已超過 7 天（週末停機、排程失敗導致錯過），會自動補跑，
  體檢不會因為錯過週六而消失。
- **AI 失敗不消耗額度**：體檢當次 AI 呼叫若失敗，該次**不寫入歷史**（`WeeklyCheckupResult.Completed = false`），
  讓下次執行的補跑機制重試，而不是把這一週的體檢額度用掉——否則一次暫時性 AI 中斷就會讓整週沒有體檢。
- **輸入塑形**：不是把 7 天歷史原樣塞給模型——程式先彙整成「每個問題簽章一行、含週內逐日次數」，
  依嚴重度取前 40 行，控制 prompt 在小模型可負擔的範圍內；同時帶入上次體檢結論，讓模型知道
  「上週說要觀察的那件事後來如何」。
- **輸出**：結論寫入當日歷史紀錄的 `WeeklyCheckup` 欄位；**有發現才**輸出
  `export\{日期}_週檢.txt`，無累積性異常的日子不產生檔案。
...
關聯層純函數邏輯，印出每一條規則、每個趨勢分支、每個關聯模式的「應命中/實際命中」比對結果，
一分鐘內確認四層偵測邏輯在新環境正常，而不是等真的出事才發現某條規則沒動。exit code 0 = 全部通過。
...
5. **JSON 契約 + grammar 強制 + 低溫度** — prompt 指定回傳
   `{risk_level, summary, trend, recommendations}` 的 JSON 結構，並透過 llama.cpp 的
   `response_format: json_object`（grammar 約束解碼）從 server 端**保證**輸出合法 JSON，
   temperature 0.2 降低發散。解析端仍有容錯（剝除圍欄、擷取大括號區段），雙保險。
...
10. **依任務性質拆分呼叫，而不是把 prompt 對半切** — 拆分的原則：
    ...
    - **前置掃描**（事件種類超過主 prompt 上限時才觸發）：超出的低嚴重度項目分批
      （每批 20 項）給獨立呼叫逐項篩選，值得注意的帶著「掃描意見」回流主分析、
      其餘以「已檢視 N 項屬雜訊」一行帶過——沒有任何事件是 AI 沒看過的，
      且掃描發生在主判斷**之前**，發現的異常能影響當日風險等級
    - **主呼叫**（每日一次）：全局判讀，看重點問題＋前置掃描結果＋歷史趨勢
    - **深入分析呼叫**（風險日才觸發，**每個問題類別一次**）：只帶該類別已確認的問題＋
      原始 log 證據，聚焦根因與處置；主分析摘要作為全局脈絡帶入，跨類別資訊不遺失
...
3. **一次抓齊**：單次倒序掃描取回整個缺漏區間的事件（不是每個日期各掃一遍），
   且 System/Application/Security 三個日誌來源**平行掃描**，抓完按日期分桶放記憶體
...
**深入分析按類別拆**——類別內的事件本來就是同一個故事該一起看
（disk/Ntfs/storahci 常是同一顆硬碟的連鎖反應），類別間則互相干擾注意力。

分開呼叫**沒有整合問題**，因為不需要合併結論：風險等級已由主分析定案，
各類別的深析結果是報告中並列的區塊、不是要調和的多份判決；
且每次類別呼叫都帶入主分析摘要作為「全局脈絡」，跨類別資訊不遺失。

重點問題的挑選：嚴重度 High 以上、頻率上升中、或首次出現的 Medium 以上，
每類別最多 4 項；原始 log 總預算 20 筆平均分配給各類別、類別內再按問題分配，
避免單一高頻事件佔滿。深入分析失敗時（模型未啟動），該類別區塊註明從缺，
統計資訊與原始 log 照常輸出。
...
- **通知管道**：目前只寫 console。排程執行時沒人盯著畫面，建議下一步接 Email / Telegram / Teams webhook，高風險時主動推播。
...
- **規則表維護**：`KnownIssueCatalog.Rules` 是純程式碼表，觀察一段時間後把貴公司環境特有的
  雜訊（可忽略）與重要訊號（要加嚴）補進去，準確度會持續提升。`LogForesight.Tests` 逐條規則
  自動產生測試案例，新增規則時測試自動涵蓋，不用手動補案例。
- **多台伺服器（NetIQ Sentinel 整合）**：目前是單機直讀。規劃中的下一階段是接上 NetIQ Sentinel
  取得數百台主機的 Event Log 做集中分析——分級分析（規則/趨勢/關聯全量跑，AI 只判讀有訊號的主機）、
  跨主機關聯層、機房總覽報告等設計已在 `docs/PLAN.md` 中規劃完成，`Persistence/` 的讀寫介面
  也已預留，實作時不需要更動既有分析邏輯的架構。
- **DB 後端**：`history.txt`／`export\` 目前是唯一實作（`JsonlAnalysisRecordStore`／`FileReportSink`）。
  查詢介面（`IAnalysisRecordReader`/`IReportSink`）已抽出，未來要接 SQLite 或其他 DB 供查詢 UI 使用時，
  只需新增一個實作類別並在 `Storage.Type` 切換，不需要異動分析邏輯。
