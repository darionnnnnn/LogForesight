...
**AI 分析改為自動啟用**：原本 AI 分析排程有自己的開關且預設關閉，本版起 AI 服務只要
設定好就一律啟用——取數執行產出結果後 AI 立刻跟上，其餘時間在「背景補跑窗口」內消化積壓。
排程作業頁的「啟用 AI 分析排程」勾選已移除。原本刻意把它關著的站台升級後 AI 會開始運作，
...
**PRTG 裝置位址的比對放寬**：裝置的位址欄位填「IP 加通訊埠」（例如 `10.1.2.3:8080`）
或 DNS 名稱時，本版都能正確對應到主機清單的 IP（名稱會做 DNS 解析）。
升級後第一次對應可能會比以往多對上一批主機，這是預期的。

**PRTG 規則行為變更**：升級後請到規則維護頁套用內建規則更新，才會有依 sensor 分類的規則。
一般 Down 只把日風險拉到「中」，只有連通性（Ping）類的 Down 拉到「高」；Ping 開始參與規則判定；
同裝置失聯時其他 sensor 的 Down 合併為一筆；已於 PRTG 確認、連續 14 天以上的 Down 不再拉高風險；
既有對 PRTG 規則建立的規則型抑制開始生效，舊 PRTG 問題的處理狀態與簽章型抑制不延續；舊紀錄的 PRTG 問題在問題排行會短期多一列舊格式「PRTG」。
完整清單見 docs/PRTG-SPEC.md §9「升級後的行為差異」。

**PRTG 只抓有對應主機的裝置**：感測器鏡像、狀態變更、數值快照改為只處理「取數範圍」內的裝置
（有對應到主機的、衝突但與主機有關的、人工對應的、資源守門用到的），狀態變更改為逐台查詢，不再翻整台 PRTG。
升級後第一次同步會清掉範圍外與 PRTG 端已刪除的感測器（數萬列，屬預期）以及 PRTG 端已刪除的裝置——
原本掛在這些已刪除感測器上、一直顯示為長期 Down 的 PRTG 問題之後不再出現；
鏡像狀態的感測器數、校準各卡的樣本數、資料匯出筆數都會大幅下降。尚未對應到任何主機的站台，
感測器與狀態變更會整段略過——先完成主機對應。**升級後請先跑一次環境探測**：效能量測最後一項「9d-5 逐物件查詢驗證」的結論應為「✓」
（若為「✗ 只有裝置自身」，請回報以改為逐感測器查詢）；輸出最後的「站台對照」會列出取數範圍大小、
下次同步會清掉多少感測器、範圍內裝置的查詢實測與預估耗時，以及資源守門找不找得到 PRTG 主機——有「不在取數範圍內」的守門裝置時照該行處置。sensor 分類內建對照新增 Port、HTTP 等連通性 type，
它們開始適用 availability 規則，可能出現新的 PRTG 問題。退版後下一次同步會把全站感測器重新抓回來。
