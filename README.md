...
```mermaid
flowchart TD
    ELS["EventLogService<br/>System / Application / Security 三來源平行掃描"]
    AGG["LogAggregator<br/>分組聚合統計，上千筆原始 log 壓成數十組簽章"]
    RULES[("rules.json<br/>規則庫（可維護，不需重新編譯）")]
    CAT["KnownIssueCatalog<br/>規則分類 + 嚴重度 + 靜態知識庫"]

    subgraph DET["確定性偵測層（不依賴 AI，AI 失效也照常運作）"]
        direction LR
        TREND["TrendAnalyzer<br/>當日 vs 前日 vs 14 日平均"]
        SLOW["SlowTrendAnalyzer<br/>近 7 天 vs 前 7 天總量"]
        CORR["CorrelationAnalyzer<br/>攻擊鏈 / 故障鏈組合比對"]
    end

    RISK["LogAnalysisService<br/>風險等級確定性判定（AI 只能往上拉，不能往下壓）"]
    AI["AIService<br/>llama.cpp / KoboldCpp OpenAI 相容 API"]
    HIST[("history.txt (JSONL)<br/>IAnalysisRecordStore")]
    REPORT["RiskReportService<br/>export/日期_風險等級_類別.txt"]
